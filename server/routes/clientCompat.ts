...
// ---------- Helpers ----------
type Tenant = "bf" | "slf";
const getTenant = (req: any): Tenant => (String(req.headers["x-tenant"] || req.query.tenant || "bf").toLowerCase() === "slf" ? "slf" : "bf");

async function pickTable(cands: string[]): Promise<string | null> {
...
let T: { Lender: string; Product: string } | null = null;
async function ensureTables() {
  if (T) return T;
  const lender = await pickTable(["Lender", "lenders"]);
  const product = await pickTable(["LenderProduct", "lender_products"]);
  if (!lender || !product) throw new Error(`Missing tables. Lender:${lender} Product:${product}`);
  T = { Lender: lender, Product: product };
  return T;
}

function asBool(x: any, def = true) {
  if (x === undefined || x === null || x === "") return def;
  const s = String(x).toLowerCase();
  return ["1", "true", "yes", "y"].includes(s);
}
function asNum(x: any) { if (x === undefined || x === null || x === "") return null; const n = Number(x); return isFinite(n) ? n : null; }
function normStr(x: any) { return (x ?? "").toString().trim(); }
...
r.get("/lenders", async (req: any, res: any) => {
  try {
    ...
    const geos = ([] as string[]).concat(req.query.geography || []).filter(Boolean).map(String);
    const inds = ([] as string[]).concat(req.query.industries || []).filter(Boolean).map(String);
    ...
    const where: string[] = [`1=1`];  // temporary - show all products regardless of tenant
    const params: any[] = [];  // Remove tenant parameter since we're not using it
    let i = params.length;

    if (isActive) { where.push(`(p.is_active IS NULL OR p.is_active = true)`); }
    if (lenderName) { params.push(`%${lenderName.toLowerCase()}%`); i++; where.push(`(lower(coalesce(l.company_name, l.contact_name)) LIKE $${i})`); }
    if (productType) { params.push(productType.toLowerCase()); i++; where.push(`(lower(p.category::text) = $${i} OR lower(p.name) LIKE '%' || $${i} || '%')`); }
    ...
    if (geos.length) { params.push(geos.map(g => g.toLowerCase())); i++; where.push(`(p.geography IS NULL OR lower(p.geography::text) SIMILAR TO '%(${geos.join("|").toLowerCase()})%')`); }
    if (inds.length) { params.push(inds.map(g => g.toLowerCase())); i++; where.push(`(p.industries IS NULL OR lower(p.industries::text) SIMILAR TO '%(${inds.join("|").toLowerCase()})%')`); }
...
r.get("/lenders/:id", async (req: any, res: any) => {
  try {
    ...
    const r = rows[0];
    if (r.amount_min != null) r.min_amount = Number(r.amount_min);
    if (r.amount_max != null) r.max_amount = Number(r.amount_max);
    if (r.term_max != null) r.term_months = Number(r.term_max);
...
r.get("/lenders/categories/summary", async (req: any, res: any) => {
  try {
...
r.get("/lenders/required-documents/:category", async (req: any, res: any) => {
  try {
    const { Product } = await ensureTables();
    const tenant = getTenant(req);
...
r.post("/public/applications", async (req: any, res: any) => {
  try {
    const tenant = getTenant(req);
    const body = req.body || {};
    ...
    if (!contactEmail) return res.status(400).json({ ok: false, error: "missing_contact_email" });
    ...
    if (!contactTable || !appTable) throw new Error("Contact/Application tables missing");
