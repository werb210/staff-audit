...
r.get("/scheduling/settings", async (req: any, res) => {
...
r.post("/scheduling/settings", async (req: any, res) => {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ error: "unauthorized" });
...
r.get("/scheduling/availability", async (req: any, res) => {
  const userId = req.user?.id;
  if (!userId) return res.status(401).json({ error: "unauthorized" });
...
r.post("/scheduling/book", async (req: any, res) => {
