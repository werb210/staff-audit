...
router.post("/send", async (req: any, res: any) => {
  ...
  if (!contactId || !toNumber || !body) return res.status(400).json({ error: "Missing fields" });
  ...
  try {
    ...
    `, [tId, body, JSON.stringify({ sid: msg.sid }), (req as any).user?.id]);
...
router.post("/webhook/inbound", async (req: any, res: any) => {
  ...
  try {
    ...
    if (!threadId) {
      const [newThread] = await q<{ id: string }>(`
        INSERT INTO comm_threads (contact_id, channel) 
        VALUES ($1, 'sms') 
        RETURNING id
      `, [contactId]);
    ...
    if (stopList.includes(text)) {
      if (contactId) await q(`UPDATE contacts SET sms_opt_out=true WHERE id=$1`, [contactId]);
    } else {
