...
router.post("/start", async (req: any, res: any) => {
  ...
  if (!contactId || !toNumber) return res.status(400).json({ error: "Missing fields" });
  ...
  try {
    ...
    const [row] = await q<{ id: string }>(`
      ...
      RETURNING id
    `, [contactId, call.sid, COMMS_CONSTANTS.CALL_FROM, toNumber, (req as any).user?.id]);
...
// Basic TwiML
router.post("/twiml/basic", (req: any, res: any) => {
...
router.post("/status", async (req: any, res: any) => {
  ...
  try {
    ...
    const [call] = await q<{ contact_id: string }>(`
