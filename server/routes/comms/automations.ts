...
router.get("/reminders", async (req: any, res: any) => {
  try {
    ...
    const reminders = await q<any>(query, params);
    res.json(reminders);
  } catch (error: unknown) {
...
router.post("/contacts/:contactId/sms-optout", async (req: any, res: any) => {
  try {
...
router.post("/triggers/missing-docs", async (req: any, res: any) => {
  try {
    const { applicationId } = req.body;
    
    // Find applications with missing documents
    const apps = applicationId 
      ? await q<any>(`SELECT id, contact_id FROM applications WHERE id=$1`, [applicationId])
      : await q<any>(`
