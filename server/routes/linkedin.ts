...
const mockLinkedInDb = {
  ...
  getSettings: (userId: string) => mockLinkedInDb.settings.get(userId),
  ...
  getPost: (id: string) => mockLinkedInDb.posts.get(id),
  ...
  getAd: (id: string) => mockLinkedInDb.ads.get(id),
...
router.get('/auth', limiter, async (req: any, res: any) => {
  try {
...
router.post('/auth/callback', limiter, async (req: any, res: any) => {
  try {
    ...
    const settings = mockLinkedInDb.saveSettings((req as any).user?.id, {
...
// Settings management
router.get('/settings', limiter, async (req: any, res: any) => {
  try {
    const settings = mockLinkedInDb.getSettings((req as any).user?.id) || {
      connected: false,
...
router.post('/settings', limiter, async (req: any, res: any) => {
  try {
...
// Organic Posts
router.get('/posts', limiter, async (req: any, res: any) => {
  try {
...
router.post('/posts', limiter, async (req: any, res: any) => {
  try {
...
router.post('/posts/:id/publish', limiter, async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
// LinkedIn Ads
router.get('/ads', limiter, async (req: any, res: any) => {
  try {
...
router.post('/ads', limiter, async (req: any, res: any) => {
  try {
    const { 
      name, 
...
router.post('/ads/:id/launch', limiter, async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
router.get('/leads', limiter, async (req: any, res: any) => {
  try {
...
router.post('/leads/sync', limiter, async (req: any, res: any) => {
  try {
...
router.get('/analytics', limiter, async (req: any, res: any) => {
  try {
...
router.post('/disconnect', limiter, async (req: any, res: any) => {
  try {
    const settings = mockLinkedInDb.saveSettings((req as any).user?.id, {
      connected: false,
      access_token: null,
