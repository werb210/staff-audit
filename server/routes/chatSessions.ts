...
import { db } from '../db';
import { chatSessions, chatMessages, users, contacts } from '../../shared/schema';
...
router.get('/', async (req: any, res: any) => {
  try {
    ...
    if (priority && priority !== 'all') {
      conditions.push(eq(chatSessions.priority, priority as string));
    }
    
    if (assigned_to) {
      conditions.push(eq(chatSessions.assignedStaffId, assigned_to as string));
    }
...
router.patch('/:id', async (req: any, res: any) => {
  try {
    ...
    if (assigned_staff_id !== undefined) updateData.assignedStaffId = assigned_staff_id;
