...
type Coverage = {
  inbound_total: number; normalized_total: number; card_total: number; pdf_total: number; lender_total: number;
  normalized_hit_rate: number; card_hit_rate: number; pdf_hit_rate: number; lender_hit_rate: number;
  missing_in_normalized: string[]; missing_in_card: string[]; missing_in_pdf: string[]; missing_in_lender: string[];
};
...
router.post("/intake", (req:Request, res:Response)=>{
  ...
  ensureDir(base);
  try { fs.writeFileSync(path.join(base,"incoming.json"), JSON.stringify(req.body,null,2)); } catch {}
  LAST_DIR = base;
  ...
  try { fs.writeFileSync(path.join(base,"normalized.json"), JSON.stringify(normalized,null,2)); } catch {}
  ...
  try { fs.writeFileSync(path.join(base,"card.json"), JSON.stringify(card,null,2)); } catch {}
  const cardFlat = flatten(card); const cardKeys = Object.keys(cardFlat);
  ...
  const pdf:any = {
    ...
    sections: [
      { name: "Business", fields: ["legalName","industry","timeInBusiness","revenue","employees"].map(k=>({ key:k, value:(normalized.answers as any)?.[k] ?? null })) },
      { name: "Loan", fields: ["loanAmount","useOfFunds","term","rate","security"].map(k=>({ key:k, value:(normalized.answers as any)?.[k] ?? null })) },
      { name: "Contact", fields: ["email","phone","address","city","province","postalCode","country"].map(k=>({ key:k, value:(normalized.answers as any)?.[k] ?? null })) },
    ]
  };
  try { fs.writeFileSync(path.join(base,"pdf.json"), JSON.stringify(pdf,null,2)); } catch {}
  const pdfFlat = flatten(pdf); const pdfKeys = Object.keys(pdfFlat);
  ...
  try { fs.writeFileSync(path.join(base,"lender.json"), JSON.stringify(lender,null,2)); } catch {}
  const lenderFlat = flatten(lender); const lenderKeys = Object.keys(lenderFlat);

  // 6) Coverage analysis: which inbound fields make it into each surface
  const inboundSet = new Set(inboundKeys);
  const hitRate = (keys:string[])=> keys.filter(k=> inboundSet.has(k) || inboundSet.has(k.split(".").pop()||"")).length / Math.max(1,inboundKeys.length);
  const missing = (keys:string[])=> Array.from(inboundSet).filter(k=> !keys.includes(k) && !keys.includes(k.split(".").pop()||""));
...
// Helpers
router.get("/intake/last", (_req:Request, res:Response)=>{
