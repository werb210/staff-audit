 ...
 */
router.post("/voice/transcript", async (req: any, res: any) => {
  try {
    ...
    res.json({ ok: true });
  } catch (e:any) { console.error("[TRANSCRIPT] error", e); res.json({ ok: true }); }
});
...
router.post("/search", async (req: any, res: any)=>{
  ...
  let comms: Array<{text?:string; subject?:string}> = [];
  try {
