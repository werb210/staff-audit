...
router.post("/flags/:key/override", async (req:any,res)=>{
  const { scope, value, tenant_id=null, user_id=null, role=null } = req.body || {};
  if (!['global','role','user','tenant'].includes(scope)) return res.status(400).json({ error:"bad scope" });
...
router.get("/flags/:key/eval", async (req:any,res)=>{
...
router.get("/experiments", async (_req,res)=>{
  ...
  res.json((e.rows||[]).map((x:any)=> ({ ...x, variants: byExp[x.key] || [] })));
});
...
router.post("/experiments/:key/status", async (req:any,res)=>{
  const { status } = req.body || {};
  if (!['draft','running','paused','stopped'].includes(status)) return res.status(400).json({ error:"bad status" });
  await db.execute(sql`UPDATE experiments SET status=${status} WHERE key=${req.params.key}`);
  res.json({ ok:true });
});
router.get("/experiments/:key/assign", async (req:any,res)=>{
  const ctx = ctxFromReq(req);
...
router.post("/experiments/:key/event", async (req:any,res)=>{
