...
import { placeCall, sendSms } from "../services/twilioSvc";
import { graphSendMail, graphCreateTask } from "../services/graphMail";

export const appDetailRouter = Router();

// Development-only bypass for testing (disable auth for demo)
if (process.env.NODE_ENV !== "production") {
  appDetailRouter.use((req: any, res: any, next: any) => {
...
const db = (req: any) => req.app.locals.db;

// 1) LEFT: full application info (condense in UI if needed)
appDetailRouter.get("/:appId/overview", async (req: any, res: any) => {
...
appDetailRouter.get("/:appId/timeline", async (req: any, res: any) => {
  const { appId } = req.params;
  try {
...
appDetailRouter.post("/:appId/actions/email", async (req: any, res: any) => {
  const { appId } = req.params;
...
appDetailRouter.post("/:appId/actions/task", async (req: any, res: any) => {
  const { appId } = req.params;
...
appDetailRouter.post("/:appId/actions/call", async (req: any, res: any) => {
  const { appId } = req.params;
...
appDetailRouter.post("/:appId/actions/sms", async (req: any, res: any) => {
  const { appId } = req.params;
...
appDetailRouter.get("/:appId/deal", async (req: any, res: any) => {
  const { appId } = req.params;
  try {
    // Mock deal summary
    const row = {
...
appDetailRouter.get("/:appId/email-attachments", async (req: any, res: any) => {
  const { appId } = req.params;
  try {
