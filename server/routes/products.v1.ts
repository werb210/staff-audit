...
function stamp(res: Response) {
  ...
  if (!res.headersSent) {
    ...
    if (process.env.GIT_SHA) res.setHeader("X-Git-SHA", process.env.GIT_SHA);
  }
}

export function productsV1Router(): Router {
  const r = makeRouter();
  r.get("/", async (_req: Request, res: Response) => {
    ...
    } catch(e:any){ stamp(res); res.status(500).json({ok:false,error:e?.message||String(e)}); }
