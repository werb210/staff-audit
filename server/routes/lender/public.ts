...
router.post("/upload", async (req:any, res)=>{
  ...
  await db.execute(sql`
    INSERT INTO documents(application_id, filename, s3_key, category, source, partner_id)
...
router.post("/download", async (req:any, res)=>{
  ...
  const doc = (await db.execute(sql`SELECT id, filename, s3_key FROM documents WHERE id=${documentId} AND application_id=${req.lender.applicationId} LIMIT 1`)).rows?.[0];
