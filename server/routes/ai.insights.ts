...
function groupDocs(docs:any[]){
  ...
  const out:any={}; for(const c of cats) out[c]=[];
  for(const d of docs){ (out[d.category] ||= []).push({ id:d.id, name:d.name, status:d.status, v:d.version }); }
  return out;
}
function computeBanking(tx:any[]){
  const last90 = tx.filter((t:any)=> (Date.now()-new Date(t.posted_at).getTime()) < 90*864e5);
  const inflow = last90.filter((t:any)=> t.amount_cents>0).reduce((a:number,b:any)=>a+b.amount_cents,0);
  const outflow = last90.filter((t:any)=> t.amount_cents<0).reduce((a:number,b:any)=>a+Math.abs(b.amount_cents),0);
  const nsf = last90.filter((t:any)=> (t.type||"").toLowerCase()==="nsf").length;
...
function findConflicts(groups:any){
  // Simple heuristic: missing pairs (e.g., IS without BS) or inconsistent versioning
  const conflicts:any[]=[];
  if ((groups.IS?.length||0)>0 && (groups.BS?.length||0)===0) conflicts.push({ field:"Statements", issue:"Income Statement present but no Balance Sheet" });
  return conflicts;
}
function scoreRisk(groups:any, banking:any){
  let s = 50;
  if ((groups.Taxes?.length||0)===0) s+=15;
  if (banking.nsf_90d>1) s+=20;
  if (banking.avg_balance_cents<0) s+=10;
