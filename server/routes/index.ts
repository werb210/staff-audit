import express, { type Application, type Router as ExpressRouter } from "express";
...
import {
  ANALYTICS_BASE_PATH,
  ADMIN_CODEX_BASE_PATH,
  API_ROOT,
  AUTH_BASE_PATH,
  CLIENT_BASE_PATH,
  COMMUNICATIONS_BASE_PATH,
  PIPELINE_BASE_PATH,
  STAFF_BASE_PATH,
  TWILIO_BASE_PATH,
} from "../../shared/apiRoutes";

type RegisterTarget = Application | ExpressRouter;
...
function stripApiRoot(fullPath: string) {
...
function registerRoute(
  ...
  if (useGuard) {
    ...
    (target as ExpressRouter).use(path, router);
  }
}

export function registerRoutes<T extends RegisterTarget>(
  target: T,
...
export function createApiRouter(options: CreateRouterOptions = {}) {
