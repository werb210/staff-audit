...
// Bulk assign lender
r.post("/assign-lender", async (req: any, res) => {
  try {
...
// Bulk archive/delete
r.post("/archive", async (req: any, res) => {
  try {
...
async function validateStageGate(applicationId: string, targetStage: string): Promise<{valid: boolean, error?: string}> {
  // Stage gate rules
  switch (targetStage) {
    case 'lender':
