...
const {
  ...
  TWILIO_NUMBER = process.env.TWILIO_NUMBER || "+18254511768",
...
r.post("/voice/token", auth, (req: any, res: any) => {
  try {
    const identity = `agent:${(req as any).user?.id || "dev"}`;
    
    if (!TWILIO_ACCOUNT_SID || !TWILIO_API_KEY || !TWILIO_API_SECRET) {
      return res.json({ ok: false, error: "Twilio credentials not configured" });
    }
    
    const token = new TwilioJWT.AccessToken(
...
r.post("/voice/outbound", auth, (req: any, res: any) => {
  try {
...
r.post("/voice/incoming", (req: any, res: any) => {
  try {
    res.set("Content-Type", "text/xml");
    const twiml = new twilio.twiml.VoiceResponse();
...
r.get("/voice/sse", auth, (req: any, res: any) => {
  res.writeHead(200, {
...
r.post("/voice/events", (req: any, res: any) => {
...
r.post("/voice/simulate", auth, (req: any, res: any) => {
  const payload = { 
    type: "voice:ring", 
