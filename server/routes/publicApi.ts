...
router.get('/test-new', (req: any, res: any) => {
...
router.get('/application/:id', (req: any, res: any) => {
  const { id } = req.params;
...
async function resolveUUIDFromAppKey(appId: string): Promise<string | null> {
  try {
...
router.get('/upload-test', (req: any, res: any) => {
...
router.post('/documents/:id', diskUpload.any(), async (req: any, res) => {
  ...
  try {
    ...
    try {
      const updateResult = await db
        ...
        .where(and(
          eq(expectedDocuments.applicationId, applicationId),
          eq(expectedDocuments.category, storageType),
          eq(expectedDocuments.status, 'pending')
        ))
; // Update expected documents for this category
...
router.post('/applications/:id/documents', s3Upload.single('document'), async (req: any, res) => {
...
async function createApplicationFromClient(data: any) {
