...
router.patch("/:id/tags", async (req: any, res: any) => {
  try {
...
router.patch("/:id/description", async (req: any, res: any) => {
  try {
    const documentId = req.params.id;
  ...
  try {
    ...
    if (tags) {
      ...
      docs = docs.filter(doc => 
        doc.tags && searchTags.some(tag => doc.tags?.includes(tag as string))
      );
    }
    ...
    if (description) {
      docs = docs.filter(doc => 
        doc.description && doc.description.toLowerCase().includes((description as string).toLowerCase())
      );
    }
