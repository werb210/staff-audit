...
import { db } from "../db";
import { documents, applicationDocuments, applications } from "../../shared/schema";
...
function shouldProcessOCR(filename: string, category: string): boolean {
  const ocrCategories = [
    'bank_statements',
    'tax_returns', 
...
const fileFilter = (req: any, file: Express.Multer.File, cb: multer.FileFilterCallback) => {
  ...
  console.log(`🔍 [UPLOAD MIME DEBUG] File: ${file.originalname}, MIME: ${file.mimetype}, Size: ${(file as any).size || 'unknown'} bytes`);
    ...
    for (const file of files) {
      ...
      const [document] = await db
        ...
        .values({
          ...
          fileSize: file.size,
          documentType: documentType as any, // Use the actual document type from request
          filePath: file.path,
