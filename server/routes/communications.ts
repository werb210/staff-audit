...
router.get("/", async (req: Request, res: Response) => {
  try {
    const contactId = (req.query.contactId as string | undefined)?.trim();
    if (!contactId) return res.status(400).json({ error: "missing_contactId" });
...
router.get("/templates", async (req: Request, res: Response) => {
  try {
    const kind = ((req.query.kind as string | undefined)?.toLowerCase()) as ("sms"|"email"|undefined);
...
type SendSmsBody = { contactId: string; to: string; body: string; templateId?: string|null };
type SendEmailBody = { contactId: string; to: string; subject: string; text?: string; html?: string; templateId?: string|null };
type StartCallBody = {
  contactId: string;
  to: string;                 // E.164 number of customer
  agentNumber?: string;       // optional staff number to bridge
};

// POST /api/communications/sms
router.post("/sms", async (req: Request, res: Response) => {
  try {
    const { contactId, to, body } = req.body as SendSmsBody;
    if (!contactId || !to || !body) return res.status(400).json({ error: "missing_fields" });
...
router.post("/email", async (req: Request, res: Response) => {
  try {
    const { contactId, to, subject, text, html } = req.body as SendEmailBody;
    if (!contactId || !to || !subject) return res.status(400).json({ error: "missing_fields" });
...
router.post("/call", async (req: Request, res: Response) => {
  try {
    const { contactId, to, agentNumber } = req.body as StartCallBody;
    if (!contactId || !to) return res.status(400).json({ error: "missing_fields" });
 ...
 */
router.get("/contact-by-phone", async (req: Request, res: Response) => {
  try {
    ...
    if (!contact || !contact.id) return res.json({ found: false });
