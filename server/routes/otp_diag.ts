...
function requireDiag(req: any, res: any, next: any) {
  if (!allow) return res.status(403).json({ ok:false, error:"diag disabled" });
  ...
  if (!token) return res.status(403).json({ ok:false, error:"auth required - no token" });
...
function assertE164(p: string) {
  if (!/^\+?[1-9]\d{1,14}$/.test(p)) throw new Error("Invalid E.164 phone");
}
...
router.get("/health", requireDiag, async (req: any, res: any) => {
  try {
    ...
    try {
      ...
      const line = (l as any)?.lineTypeIntelligence?.type || "unknown";
      ...
      health.lookup = {
        ...
        lineType: line, carrier: (l as any)?.carrier?.name || null,
        smsCapable
      };
      if (!smsCapable) health.likelyProblem = "not_sms_capable_try_whatsapp";
    } catch (e:any) {
...
router.post("/send", requireDiag, async (req: any, res: any) => {
  try {
    const user = (req as any).auth?.user;
    if (!user?.phone) return res.status(400).json({ ok:false, error:"User phone missing" });
    if (!TWILIO_VERIFY_SERVICE_SID) return res.status(500).json({ ok:false, error:"Verify service not configured" });
    const to = String(user.phone);
    assertE164(to);

    const output:any = { toMasked: mask(to), attempts: [] };
...
router.get("/last", requireDiag, async (req: any, res: any) => {
  try {
    const mins = Math.max(1, Math.min(60, Number(req.query.mins) || 10));
    const to = (req as any).auth?.user?.phone as string | undefined;
    if (!to) return res.status(400).json({ ok:false, error:"User phone missing" });
    assertE164(to);
