...
function chunk(md:string, max=800): string[] {
  const paras = md.split(/\n{2,}/).map(s=>s.trim()).filter(Boolean);
  const out:string[] = []; let buf = "";
  for (const p of paras){
    if ((buf + "\n\n" + p).length > max){ if (buf) out.push(buf); buf = p; } else { buf = buf ? (buf+"\n\n"+p) : p; }
  }
  if (buf) out.push(buf);
  return out;
}
...
r.post("/training/docs", async (req:any,res)=>{
  ...
  let vecs: number[][]|null = await embedText(chunks);
...
r.get("/training/search", async (req,res)=>{
  ...
  if (qvec && qvec[0]){
    ...
    const scored = rows.filter(r=>Array.isArray(r.embedding?.[0])||Array.isArray(r.embedding))
      .map((r:any)=>({ ...r, score: cosine(qvec[0] as any, (r.embedding as any) ) }))
      .sort((a:any,b:any)=>b.score-a.score).slice(0,30);
    return res.json({ ok:true, items: scored.map((r:any)=>({ doc_id:r.doc_id, chunk_id:r.id, text:r.text, score:Number(r.score||0) })) });
  } else {
    const { rows } = await db.execute(sql`
      ...
      limit 30
    `).catch(()=>({ rows: [] as any[] }));
    if (rows.length) return res.json({ ok:true, items: rows });
    // super fallback
    const all = (await db.execute(sql`select id, doc_id, idx, text from training_doc_chunks`)).rows;
    const items = all.filter((r:any)=>r.text.toLowerCase().includes(q.toLowerCase())).slice(0,30).map(r=>({ ...r, score: 0.1 }));
