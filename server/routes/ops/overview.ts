...
router.get("/overview", async (_req,res)=>{
  const out:any = { ready:null, counts:{}, queues:[], billing:null, health:null, release:null };
  ...
  try {
    ...
    if (isQueuesOn()){
      ...
      for (const n of names){
        try { out.queues.push({ name:n, counts: await getCounts(n) }); } catch { out.queues.push({ name:n, counts:null }); }
      }
    }
  } catch {
