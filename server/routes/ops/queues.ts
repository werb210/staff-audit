...
router.get("/queues/status", async (_req,res)=>{
  if (!isQueuesOn()) return res.json({ enabled:false, queues: [] });
...
router.get("/queues/:name/jobs", async (req,res)=>{
  if (!isQueuesOn()) return res.status(503).json({ error:"queues_disabled" });
...
router.post("/queues/:name/:jobId/retry", async (req,res)=>{
  try { res.json(await retryJob(req.params.name, req.params.jobId)); }
  catch(e:any){ res.status(400).json({ error:String(e?.message||e) }); }
});
router.post("/queues/:name/:jobId/remove", async (req,res)=>{
  try { res.json(await removeJob(req.params.name, req.params.jobId)); }
  catch(e:any){ res.status(400).json({ error:String(e?.message||e) }); }
});
router.post("/queues/:name/:jobId/promote", async (req,res)=>{
  try { res.json(await promoteJob(req.params.name, req.params.jobId)); }
  catch(e:any){ res.status(400).json({ error:String(e?.message||e) }); }
});

router.post("/queues/:name/retry-failed", async (req,res)=>{
  try { res.json(await retryAllFailed(req.params.name, Number(req.body?.limit||500))); }
  catch(e:any){ res.status(400).json({ error:String(e?.message||e) }); }
});
router.post("/queues/:name/drain", async (req,res)=>{
  try { res.json(await drainQueue(req.params.name, !!req.body?.delayed)); }
  catch(e:any){ res.status(400).json({ error:String(e?.message||e) }); }
});
