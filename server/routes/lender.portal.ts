...
const requireLenderAuth = (req: any, res: any, next: any) => {
...
r.post("/lender-portal/register", async (req: any, res: any) => {
...
r.post("/lender-portal/api/login", async (req: any, res: any) => {
  ...
  const lender = demoLenders[email as keyof typeof demoLenders];
...
r.get("/lender-portal/api/demo", (req: any, res: any) => {
  res.json({ 
    ok: true, 
...
r.get("/lender-portal/api/my-products", requireLenderAuth, async (req: any, res) => {
  try {
...
r.post("/lender-portal/api/my-products", requireLenderAuth, async (req: any, res) => {
  try {
    const lenderId = req.lender.lenderId;
    const productData = {
      ...req.body,
...
r.get("/lender-portal/api/my-products/:id", requireLenderAuth, async (req: any, res) => {
  try {
    const lenderId = req.lender.lenderId;
...
r.put("/lender-portal/api/my-products/:id", requireLenderAuth, async (req: any, res) => {
  try {
    const lenderId = req.lender.lenderId;
    const productId = req.params.id;
...
r.delete("/lender-portal/api/my-products/:id", requireLenderAuth, async (req: any, res) => {
  try {
    const lenderId = req.lender.lenderId;
    const productId = req.params.id;
...
r.get("/lender-portal/api/reports", requireLenderAuth, async (req: any, res) => {
  try {
    const lenderId = req.lender.lenderId;
...
r.get("/lender-portal/dashboard", async (req: any, res) => {
...
r.get("/lender-portal/reports", async (req: any, res) => {
  const userEmail = req.user?.email;
  if (!userEmail) return res.status(401).json({ error: "unauthorized" });
