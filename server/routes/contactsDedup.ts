...
router.get('/duplicates', limiter, async (req: any, res: any) => {
  try {
    const by = (String(req.query.by || 'email')).toLowerCase() as 'email'|'phone'|'both';
    ...
    const push = (key:string, c:Contact) => {
      if (!key) return;
    ...
    for (const c of all.rows || all) {
      if (by === 'email' || by === 'both') push((c.email || '').trim().toLowerCase(), c);
      if (by === 'phone' || by === 'both') push((c.phone || '').replace(/\D/g,''), c);
    }
...
router.post('/merge', limiter, async (req: any, res: any) => {
  try {
    const { survivorId, mergeIds = [], picks = {}, dryRun = false, note } = req.body || {};
    if (!survivorId || !mergeIds.length) return res.status(400).json({ ok:false, reason:'missing_params' });
    ...
    if (rowsData.length !== ids.length) return res.status(404).json({ ok:false, reason:'not_found' });
