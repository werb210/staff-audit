...
import { db } from "../db/drizzle"; import { sql } from "drizzle-orm";
const r = Router();
...
r.get("/analytics/roi/summary", async (_req,res)=>{
  ...
  `);
  const m = new Map<string, any>(); for (const s of spend) m.set(s.campaign_id||'unknown', { campaign_id: s.campaign_id||'unknown', cost_micros: Number(s.cost_micros||0), conversions: Number(s.conversions||0), revenue_cents: 0 });
  for (const r2 of rev){ const k=r2.campaign_id||'unknown'; m.set(k, { ...(m.get(k)||{campaign_id:k,cost_micros:0,conversions:0}), revenue_cents: Number(r2.revenue_cents||0) }); }
  const rows = [...m.values()].map(x=>{
    const cost = x.cost_micros/1_000_000; // to currency
    const conv = x.conversions||0; const revenue = (x.revenue_cents||0)/100;
