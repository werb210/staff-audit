...
const mockDb = {
  ...
  getCampaign: (id: string) => mockDb.campaigns.get(id),
  ...
  getRecipients: (campaignId: string) => 
    Array.from(mockDb.recipients.values()).filter(r => r.campaign_id === campaignId),
    
  countRecipients: (campaignId: string, status?: string) => {
...
// Utility functions
function buildUtm(url: string, opts: {source?: string; medium?: string; campaign?: string; content?: string}) {
  if (!url) return url;
  const u = new URL(url);
...
function unsubscribeLink(contactId: string) {
...
function render(template: string, ctx: Record<string, any>) {
  return (template || '').replace(/\{\{\s*([^}]+?)\s*\}\}/g, (_, path) => {
    const parts = String(path).split('.').map((p: string) => p.trim()).filter(Boolean);
    let cur: any = ctx;
    for (const p of parts) cur = (cur && p in cur) ? cur[p] : '';
    return cur ?? '';
  });
}

function stateToAudience(state?: string) {
...
// List campaigns
router.get('/', limiter, async (req: any, res: any) => {
  try {
...
router.get('/:id', limiter, async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
router.post('/:id/prepare', limiter, async (req: any, res: any) => {
  try {
    const { id } = req.params;
    const campaign = mockDb.getCampaign(id);
...
// Campaign controls
router.post('/:id/pause', limiter, async (req: any, res: any) => {
  try {
...
router.post('/:id/resume', limiter, async (req: any, res: any) => {
  try {
...
router.post('/:id/cancel', limiter, async (req: any, res: any) => {
  try {
...
router.get('/:id/progress', limiter, async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
router.get('/:id/preview', limiter, async (req: any, res: any) => {
  try {
    const { id } = req.params;
