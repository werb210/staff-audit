...
router.post("/link/exchange", async (req:any,res)=>{
  const { contactId, public_token } = req.body || {};
  if (!contactId || !public_token) return res.status(400).json({ error:"contactId and public_token required" });
...
router.post("/sync/balances", async (req:any,res)=>{
...
router.post("/sync/transactions", async (req:any,res)=>{
