 ...
 */
r.get("/products", async (req: any, res: any) => {
  const { country, category, lender, q } = req.query as Record<string, string | undefined>;
  const params: any[] = [];
  const where: string[] = [];
...
r.get("/", async (req: any, res: any) => {
  const { country, category, q } = req.query as Record<string, string | undefined>;
  const params: any[] = [];
  const where: string[] = [];
