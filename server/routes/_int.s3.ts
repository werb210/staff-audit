...
function env(name: string, def?: string) {
  ...
  if (!v) throw new Error(`[s3:env] Missing ${name}`);
  return v;
}
 ...
 */
r.post("/roundtrip", async (req: any, res: any) => {
  try {
    ...
    // Presign GET
    const getUrl = await getSignedUrl(s3 as any, new (require("@aws-sdk/client-s3").GetObjectCommand)({ Bucket: bucket, Key: key }), { expiresIn: 60 * 10 });
