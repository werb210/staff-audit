...
router.post('/restore-actual-documents/:applicationId', async (req: any, res: any) => {
  try {
    ...
    const actualFiles = allFiles.filter(file => 
      file.match(/\.(pdf|xlsx|docx|jpg|jpeg|png|gif|txt)$/i) && 
    ...
    for (const placeholder of placeholders) {
      try {
        ...
        const matchingFiles = actualFiles.filter(file => 
          file.startsWith(placeholder.id) && !file.endsWith('.txt')
        );
