...
type Check = () => Promise<{ok:boolean; info?:any; err?:string}>;
const r = Router();

async function tryFetch(url: string, init?: RequestInit): Promise<{ok:boolean; status?:number; headers?:any; text?:string; json?:any; err?:string}> {
  try {
...
async function tableHas(cols: string[], table: string) {
  try {
    const q = `
...
function summarize(features: {status:"installed"|"partial"|"error"}[]) {
...
r.get("/full-audit", async (req: any, res: any) => {
  const base = `${req.protocol}://${req.get("host")}`;

  // Auth checks
  const authDev: Check = async () => {
  ...
  const ui = (path:string): Check => async () => {
  ...
  // API checks
  const apiOK = (path:string, expectArrayKey?:string): Check => async () => {
    const r = await tryFetch(`${base}${path}`);
  ...
  // ENV presence
  const need = (keys:string[], label:string): Check => async () => {
  ...
  // ---------- Tabs & Features ----------
  const tabs: { name:string; features: {key:string; check:Check; notes?:string}[] }[] = [
    {
      name: "Auth",
      features: [
  ...
  for (const tab of tabs) {
    ...
    for (const feature of tab.features) {
      ...
      const status: "installed"|"partial"|"error" = out.ok ? "installed" : "error";
  ...
  const totals = results.reduce((acc:any, t)=> {
