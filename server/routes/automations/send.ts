...
import { getContactMergeVars, mergeVars } from "../../services/mergeFieldsV2";
...
router.post("/send-sms", async (req: any, res: any) => {
  const { contactId, templateId } = req.body || {};
  if (!contactId || !templateId) return res.status(400).json({ error: "Missing fields" });
  const [t] = await q<any>(`SELECT * FROM comm_templates WHERE id=$1 LIMIT 1`, [templateId]);
  if (!t || t.channel !== "sms") return res.status(400).json({ error: "Bad template" });
...
router.post("/send-email", async (req: any, res: any) => {
  ...
  if (!orgId || !templateId || !toEmail) return res.status(400).json({ error: "Missing fields" });
  const [t] = await q<any>(`SELECT * FROM comm_templates WHERE id=$1 LIMIT 1`, [templateId]);
  if (!t || t.channel !== "email") return res.status(400).json({ error: "Bad template" });
  ...
  await sendUserEmail({ userId: (req as any).user?.id, toEmail, subject, htmlBody: body });
  res.json({ ok: true });
});
