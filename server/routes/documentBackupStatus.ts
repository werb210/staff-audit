...
router.post("/:id/backup", async (req: any, res: any) => {
  try {
...
function getDocumentBackupStatus(hasLocal: boolean, hasBackup: boolean, backupAccessible: boolean): string {
  if (hasLocal && hasBackup && backupAccessible) return "fully_backed";
  if (hasLocal && !hasBackup) return "local_only";
  if (!hasLocal && hasBackup && backupAccessible) return "backup_only";
  if (!hasLocal && !hasBackup) return "missing";
  if (hasBackup && !backupAccessible) return "corrupted";
  return "unknown";
}
