...
import { db } from '../db';
import { applications, lenderProducts, lenders } from '../../shared/schema';
...
function calculateMatchScore(
  ...
  if (application.requestedAmount >= product.minimum_lending_amount && 
    ...
    const deviation = Math.min(
      Math.abs(application.requestedAmount - product.minimum_lending_amount),
      Math.abs(application.requestedAmount - product.maximum_lending_amount)
    );
...
router.get('/matches', async (req: any, res: any) => {
  try {
    ...
    const lenderId = req.query.lenderId as string || 'demo-lender-uuid';
    const minScore = parseFloat(req.query.minScore as string) || 0.3; // 30% minimum match
    ...
    const matches: MatchedApplication[] = [];
...
router.get('/matches/stats', async (req: any, res: any) => {
  try {
    const lenderId = req.query.lenderId as string || 'demo-lender-uuid';
...
router.post('/matches/:matchId/express-interest', async (req: any, res: any) => {
  try {
