...
const securityTestHistory: SecurityTestLog[] = [];

// Log security test results
router.post('/security/log', async (req: any, res: any) => {
  try {
...
router.get('/security/history', async (req: any, res: any) => {
  try {
...
router.get('/security/dashboard', async (req: any, res: any) => {
  try {
    ...
    const averageGrade = Object.entries(gradeValues).find(([grade, value]) => 
      averageGradeValue >= value
    )?.[0] || 'F';
...
router.get('/security/test/:testId', async (req: any, res: any) => {
  try {
