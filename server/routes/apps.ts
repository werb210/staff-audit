...
r.get("/", (req: any, res: any) => {
  ...
  const filtered = q ? rows.filter(x =>
    ...
    x.contact?.email?.toLowerCase().includes(q)
  ) : rows;
  res.json(filtered);
});
...
r.get("/:id/twilio-lookup", async (req: any, res: any) => {
  try {
...
r.get("/:id/bank/metrics", async (req: any, res: any) => {
  try {
    res.json({
      ok: true,
      metrics: {
...
r.get("/:id/bank/transactions", async (req: any, res: any) => {
  try {
    res.json({
      ok: true,
      transactions: [
...
// Financials endpoint
r.get("/:id/financials", async (req: any, res: any) => {
  try {
    res.json({
      ok: true,
      periods: [
...
r.get("/:id/documents", async (req: any, res: any) => {
  try {
    res.json({
      ok: true,
      categories: [
...
r.post("/:id/documents", upload.single('file'), async (req: any, res: any) => {
  try {
    const { id } = req.params;
...
r.patch("/:id/documents/:docId", async (req: any, res: any) => {
  try {
...
r.get("/:id/documents.zip", async (req: any, res: any) => {
  try {
...
r.get("/:id/lender/match", async (req: any, res: any) => {
  try {
    res.json({
      ok: true,
      matches: [
...
r.post("/:id/lender/send", async (req: any, res: any) => {
  try {
