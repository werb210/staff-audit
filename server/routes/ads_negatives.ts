...
async function fetchSearchTerms(start: string, end: string) {
...
r.post("/ai/ads/negatives/suggest", async (req: any, res: any) => {
  ...
  const agg = new Map<string, { campaignId: string; cost: number; clicks: number; term: string }>();
  for (const r of rows) {
...
Return JSON: [{"campaignId":"123","negatives":["term one","term two"]}] and group by campaignId. Terms:\n` + JSON.stringify(list);
  ...
  const j: any = await fetch("https://api.openai.com/v1/chat/completions", {
    method: "POST",
    headers: { 
