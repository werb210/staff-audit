...
router.get('/logs', async (req: any, res: any) => {
  try {
    ...
    const logs = await retryQueue.getRetryLogs(parseInt(limit as string));
...
router.post('/retry', async (req: any, res: any) => {
  try {
...
router.delete('/jobs/:applicationId/:jobType', async (req: any, res: any) => {
  try {
