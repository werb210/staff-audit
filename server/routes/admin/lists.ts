...
function qp(q:any){ return { search: String(q.search||"").trim(), offset: Number(q.offset||0), limit: Math.min(200, Math.max(1, Number(q.limit||50))) }; }
function daterange(q:any, col:string){
...
router.post("/applications/bulk/status", async (req,res)=>{
  const ids = (req.body?.ids||[]).map((x:any)=>String(x)).filter(Boolean);
  const status = String(req.body?.status||"");
  if (!ids.length || !status) return res.status(400).json({ error:"ids_and_status_required" });
