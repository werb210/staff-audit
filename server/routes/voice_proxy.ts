...
r.get("/voice/recording/:callId/audio", async (req: any, res: any) => {
  const call = await db.call.findUnique({ where: { id: req.params.callId } });
  if (!call?.recordingUrl) return res.status(404).send("not found");
  ...
  if (!twRes.ok) return res.status(502).send("twilio_fetch_failed");
