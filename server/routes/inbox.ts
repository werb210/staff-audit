...
r.get("/threads", async (_req:any,res)=>{
  const rows = await db.select().from(contacts);
  res.json({ ok:true, items: rows.map((c:any)=>({ id:c.id, name:c.fullName, email:c.email, phone:c.phone })) });
});

// Thread detail: merge SMS/Chat from DB + Email from Graph (read-only) with optional kind filters
r.get("/thread/:contactId", async (req:any,res)=>{
  ...
  let rows = await db.select().from(comms).where(eq(comms.contactId, contactId as any));
  
  // Filter by kinds if specified
  if (kinds.length > 0) {
    rows = rows.filter((r:any) => kinds.includes(r.kind));
  }
  ...
  if (includeEmail) {
    try {
      ...
      if (c.email) {
        ...
        emails = emails.concat((inJ.value||[]).map((m:any)=>({ kind:"email", direction:"in", body:m.subject, meta:{ id:m.id, snippet:m.bodyPreview }, createdAt:m.receivedDateTime })));
        ...
        emails = emails.concat((outJ.value||[]).map((m:any)=>({ kind:"email", direction:"out", body:m.subject, meta:{ id:m.id, snippet:m.bodyPreview }, createdAt:m.sentDateTime })));
      }
    } catch { /* not connected → show SMS/Chat only */ }
  }

  const merged = [...rows, ...emails].sort((a:any,b:any)=> new Date(a.createdAt||0).getTime() - new Date(b.createdAt||0).getTime());
  res.json({ ok:true, items: merged });
});

// Mark all messages as read in a thread (read receipts)
r.post("/thread/:contactId/read", async (req:any,res)=>{
  ...
  if (rows?.[0]) require("./../routes/realtime"); // no-op import to ensure side effects
