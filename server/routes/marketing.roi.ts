...
r.post("/roi/convert", async (req: any, res: any) => {
  const b = req.body || {};
...
r.get("/roi/summary", async (req: any, res: any) => {
  const expId = req.query.experimentId as string;
  if (!expId) return res.status(400).json({ ok: false, error: "experimentId required" });
