...
r.post("/:id/preview", async (req: any, res: any) => {
  ...
  const { whereContacts, whereApps } = buildWhere(a.definition_json as any);
...
r.get("/:id/export", async (req: any, res: any) => {
  const a = await db.linkedInAudience.findUnique({ where: { id: req.params.id } });
  if (!a) return res.status(404).json({ ok: false, error: "not_found" });
...
r.post("/:id/sync", async (req: any, res: any) => {
  const a = await db.linkedInAudience.findUnique({ where: { id: req.params.id } });
  if (!a) return res.status(404).json({ ok: false, error: "not_found" });
  // compute size
...
// ---- helpers ----
function buildWhere(def: any) {
  const logic = (def?.logic || "AND").toUpperCase();
  const rules: any[] = def?.rules || [];
  const excludes: any[] = def?.exclude || [];

  const contactClauses: any[] = [];
  const appClauses: any[] = [];
...
function toPrismaClause(field: string, r: any) {
  const op = String(r.op || "eq").toLowerCase();
  const val = r.value;

  const text = (v: any) => ({ contains: String(v), mode: "insensitive" as const });
  const eq = (v: any) => ({ equals: v });
  const isin = (arr: any[]) => ({ in: arr });
...
async function fetchPreviewCount(aud: any) {
