 ...
 */
router.post('/', developmentAuth, requireRoles('admin', 'staff'), async (req: Request, res: Response) => {
  try {
    const { 
      email, 
      firstName, 
 ...
 */
router.patch('/:id', developmentAuth, requireOwnershipOrAdmin(), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { 
      firstName, 
 ...
 */
router.get('/', developmentAuth, requireRoles('admin', 'staff'), async (req: Request, res: Response) => {
  try {
    ...
    if (role) whereConditions.push(eq(users.role, role as string));
    if (isActive !== undefined) whereConditions.push(eq(users.isActive, isActive === 'true'));
 ...
 */
router.delete('/:id', developmentAuth, requireRoles('admin'), async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
