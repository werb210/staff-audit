 ...
 */
async function upsertLenderProduct(product: any) {
  try {
    ...
    const existing = await db
      ...
      .from(lenderProducts)
      .where(and(
        eq(lenderProducts.name, product.name),
        eq(lenderProducts.lenderName, product.lenderName),
        isNull(lenderProducts.deletedAt)
      ))
      .limit(1);
 ...
 */
function broadcastProductUpdate(productId: string, operation: string) {
