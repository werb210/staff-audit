...
export function applicationsValidateRouter(): Router {
  const r = makeRouter();

  // Helpful GET (kept for diagnostics)
  r.get("/applications/validate-intake", (_req: Request, res: Response) => {
    res.json({ 
      ok: true, 
  ...
  r.post("/applications/validate-intake", async (req: Request, res: Response) => {
    const body = req.body || {};
    const errors: string[] = [];
    const details: any = {};
    ...
    if (!productId) errors.push("Missing: productId");
    if (!country) errors.push("Missing: country");
    if (!Number.isFinite(amount)) errors.push("Missing/invalid: amountRequested");

    if (errors.length) return res.status(400).json({ ok: false, errors, details });
    ...
    try {
      ...
      // Basic checks
      if (prod.active === false) errors.push("Product is inactive");
      ...
      // Result
      if (errors.length) return res.status(200).json({ ok: false, errors, details }); // 200 with ok:false = validated but not eligible
