...
const canon = (e:string) => {
  if (!e) return "";
  const [u,d] = e.trim().toLowerCase().split("@"); if (!d) return e.toLowerCase();
  if (d==="gmail.com"||d==="googlemail.com"){ const b=u.split("+")[0].replace(/\./g,""); return `${b}@gmail.com`; }
  return `${u}@${d}`;
};

r.get("/contacts/build", async (req: any, res: any) => {
