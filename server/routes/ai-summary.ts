...
import { 
  generateAICreditSummary, 
  saveDraftSummary, 
  submitFinalSummary 
} from '../services/aiCreditSummaryService';
import { db } from '../db';
 ...
 */
router.post('/:summaryId/draft', async (req: any, res: any) => {
  try {
 ...
 */
router.post('/:summaryId/submit', async (req: any, res: any) => {
  try {
    const { summaryId } = req.params;
    const { content } = req.body;
