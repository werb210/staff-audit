...
function userToken(req:express.Request){ return (req.headers["x-ms-user-token"] as string) || undefined; }

/** EMAIL (O365) */
r.post("/email/send", async (req: any, res: any, next: any)=>{
  try{
...
r.post("/doc-kit", async (req,res,next)=>{
  try{
    ...
    const links = await Promise.all((files||[]).map(async (f:any)=> presignUpload(`uploads/${contact?.id||"anon"}/${Date.now()}-${f.name}`, f.contentType||"application/pdf")));
...
r.get("/linkedin/links", async (req,res)=>{
  const { profileUrl="", companyDomain="", companyUrn="" } = req.query as any;
