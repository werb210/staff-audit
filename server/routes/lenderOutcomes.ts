...
function createIdempotencyKey(appId: string, outcome: string, fundsDisbursed: boolean, amount?: number, key?: string): string {
 ...
 */
router.post('/outcome', async (req: any, res) => {
  try {
 ...
 */
router.get('/outcomes/health', (req: any, res: any) => {
  res.json({
    ok: true,
