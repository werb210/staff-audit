...
applicationsV1.post("/", async (req: Request, res: Response) => {
  ...
  // business rules
  const errors: string[] = [];
  if (data.years_in_business < 12) errors.push("Minimum 12 months in business required.");
  if (data.monthly_revenue < 15000) errors.push("Minimum $15000 monthly revenue required.");
...
applicationsV1.post("/:id/docs", appDocsUpload("id"), async (req: Request, res: Response) => {
  // NOTE: Do NOT add any other multer or body-parser on this route.
  const id = req.params.id;
  const files = (req.files as Express.Multer.File[]) || [];
  if (!files.length) return res.status(400).json({ ok: false, error: "No files received. Field must be 'files'." });
...
applicationsV1.get("/:id", async (req: Request, res: Response) => {
