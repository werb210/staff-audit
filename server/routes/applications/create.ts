...
import { db } from '../../db';
import { applications, businesses, users, expectedDocuments, lenderProducts, documents } from '../../../shared/schema';
...
router.post('/test-logging', (req: any, res: any) => {
...
router.post('/', async (req: any, res: any) => {
  ...
  const isTestApplication = 
    submittedBusinessName.includes('Test Corp') ||
  ...
  try {
    fs.appendFileSync(
      ...
      `[${new Date().toISOString()}] Application received: ${JSON.stringify(req.body)}\n`
    );
  } catch (logError) {
  ...
  console.log("🔍 TOP-LEVEL KEYS ANALYSIS:", {
    ...
    legacyFieldsDetected: Object.keys(req.body).filter(key => 
      ['legalName', 'firstName', 'lastName', 'businessName', 'applicantFirstName', 'applicantLastName'].includes(key)
    )
  });
  ...
  if (req.body.legalName || req.body.applicantFirstName || req.body.applicantLastName || req.body.businessName) {
    ...
    return res.status(400).json({
      ...
      rejectedFields: Object.keys(req.body).filter(key => 
        ['legalName', 'applicantFirstName', 'applicantLastName', 'businessName', 'applicantSSN', 'operatingName'].includes(key)
      )
    });
  }
  ...
  try {
    ...
    if (documentRequirements.length > 0) {
      const expectedDocsData = DocumentRequirementsService.mapRequirementsToExpectedDocuments(
    ...
    try {
      fs.appendFileSync(
        "./logs/application_log.txt",
        `[${new Date().toISOString()}] Application successfully stored: ${applicationRecord.id}\n`
      );
    } catch (logError) {
    ...
    try {
      fs.appendFileSync(
        "./logs/application_log.txt",
        `[${new Date().toISOString()}] Application storage failed: ${error instanceof Error ? error instanceof Error ? error.message : String(error) : String(error)}\n`
      );
    } catch (logError) {
