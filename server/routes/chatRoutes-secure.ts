...
import { 
  chatEscalations, 
  chatIssueReports, 
  insertChatEscalationSchema, 
  insertChatIssueReportSchema, 
  chatSessions, 
  chatMessages, 
  contacts 
} from '../../shared/schema';
 ...
 */
router.post('/start', chatStartLimiter, async (req: any, res: any) => {
  try {
 ...
 */
router.post('/message', chatMessageLimiter, async (req: any, res: any) => {
  try {
...
`;
    ...
    try {
      ...
      escalated = escalationKeywords.some(keyword => 
        validatedMessage.toLowerCase().includes(keyword) || aiResponse.toLowerCase().includes('escalate')
      );
 ...
 */
router.get('/history/:sessionId', chatHistoryLimiter, async (req: any, res: any) => {
  try {
 ...
 */
router.post('/leads', leadsLimiter, async (req: any, res: any) => {
  try {
    const { 
      name, 
      email, 
    ...
    let validatedPhone: string | null;
    let validatedMessage: string;
 ...
 */
router.post('/escalate', escalationLimiter, async (req: any, res: any) => {
  try {
    ...
    let validatedUserEmail: string | null;
    let validatedMessage: string;
 ...
 */
router.post('/report', reportLimiter, async (req: any, res: any) => {
  try {
 ...
 */
router.get('/escalations', dashboardLimiter, async (req: any, res: any) => {
  try {
    ...
    const maxLimit = Math.min(parseInt(limit as string, 10) || 50, 100); // Cap at 100
    const allowedStatuses = ['pending', 'assigned', 'resolved', 'cancelled'];
    const validatedStatus = status && allowedStatuses.includes(status as string) ? status as string : null;
 ...
 */
router.get('/issue-reports', dashboardLimiter, async (req: any, res: any) => {
  try {
    ...
    const validatedStatus = status && allowedStatuses.includes(status as string) ? status as string : null;
    const validatedSeverity = severity && allowedSeverities.includes(severity as string) ? severity as string : null;
 ...
 */
router.patch('/escalations/:id/status', generalLimiter, async (req: any, res: any) => {
  try {
    const { id } = req.params;
 ...
 */
router.get('/handoff-queue', dashboardLimiter, async (req: any, res: any) => {
  try {
 ...
 */
router.post('/request-staff', escalationLimiter, async (req: any, res: any) => {
  try {
    ...
    let validatedUserEmail: string | null;
    let validatedMessage: string;
    let validatedRequestType: string;
 ...
 */
router.post('/log-contact', generalLimiter, async (req: any, res: any) => {
  try {
    ...
    let validatedAction: string;
    let validatedDetails: string;

    try {
      if (sessionId) validatedSessionId = validateSessionId(sessionId);
      validatedUserName = userName ? validateName(userName) : 'Anonymous User';
      if (userEmail) validatedUserEmail = validateEmail(userEmail);
 ...
 */
router.post('/user-message', chatMessageLimiter, async (req: any, res: any) => {
  try {
