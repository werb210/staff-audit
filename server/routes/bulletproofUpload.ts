...
router.post('/upload/:applicationId', upload.single('document'), async (req: any, res: any) => {
  ...
  try {
    ...
    const result = await bulletproofUploadDocument(
      file.buffer,
      file.originalname,
      documentType,
      applicationId,
      'public-api' // TODO: Use actual user ID when auth is implemented
    );
 ...
 */
router.get('/documents/:id/preview', async (req: any, res: any) => {
  const { id } = req.params;
...
router.get('/health', async (req: any, res: any) => {
  ...
  try {
    ...
    const [pendingBackups] = await db.select({ count: count() }).from(documents).where(eq(documents.backupStatus, 'pending' as any));
    const [failedBackups] = await db.select({ count: count() }).from(documents).where(eq(documents.backupStatus, 'failed' as any));
    const [completedBackups] = await db.select({ count: count() }).from(documents).where(eq(documents.backupStatus, 'completed' as any));
