...
interface SmartReplyRequest {
  ...
  replyType: 'sms' | 'email' | 'professional' | 'casual' | 'followup';
  context?: {
    contactName?: string;
...
router.post('/smart-reply', async (req: any, res: any) => {
  try {
    const { 
      threadHistory, 
      replyType, 
...
router.post('/reply-options', async (req: any, res: any) => {
  try {
    const { 
      threadHistory, 
      replyType, 
      context = {},
      count = 3
    }: SmartReplyRequest & { count?: number } = req.body;
...
router.post('/followup-suggestions', async (req: any, res: any) => {
  try {
    const { threadHistory, context = {} }: { threadHistory: string; context?: any } = req.body;
...
async function generateSmartReply(
  ...
  const systemPrompt = systemPrompts[replyType as keyof typeof systemPrompts] || systemPrompts.professional;
...
async function generateFollowupSuggestions(threadHistory: string, context: any): Promise<string[]> {
...
function generateFallbackReply(replyType: string, context: any): string {
  const fallbacks = {
  ...
  return fallbacks[replyType as keyof typeof fallbacks] || fallbacks.professional;
}
