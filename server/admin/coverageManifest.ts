// Endpoints we care about + where their UI lives + the DOM selector to find
// Edit paths/selectors to match your app if different.
export type UiCoverageItem = {
  label: string;
  method: "GET" | "POST";
  path: string;
  uiPath: string;              // route to visit in the app
  selector: string;            // button/link/visual element selector to assert
  adminOnly?: boolean;
};

export const uiCoverageManifest: UiCoverageItem[] = [
  // OTP (Twilio Verify)
