...
const {
  ...
  MS_CLIENT_SECRET,
  MS_REDIRECT_URI,
  MS_SCOPES = "email,Mail.ReadWrite,Mail.Send,Calendars.ReadWrite,Contacts.ReadWrite,Tasks.ReadWrite,Files.ReadWrite,Teams.ReadBasic.All,Chat.ReadWrite,Presence.Read,Presence.Read.All",
} = process.env;

// TODO: Replace with actual database calls using your ORM
async function getTokens(userId: string): Promise<{ access_token: string; refresh_token: string; expires_at: Date } | null> {
...
async function saveTokens(userId: string, tokens: { access_token: string; refresh_token: string; expires_at: Date }): Promise<void> {
  // Placeholder: upsert into o365_tokens
  console.log(`[O365] Saving tokens for user ${userId} - TODO: implement DB call`);
}

async function refreshAccessToken(userId: string): Promise<{ access_token: string; refresh_token: string; expires_at: Date }> {
...
export async function graphForUser(userId: string): Promise<Client> {
...
export async function logGraph(userId: string | null, action: string, target?: string, ok = true, error?: string): Promise<void> {
  try {
    const logEntry = {
      userId,
      action,
...
export async function exchangeCodeForTokens(code: string): Promise<{ access_token: string; refresh_token: string; expires_at: Date }> {
