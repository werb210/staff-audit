let openai:any = null;
try { const { OpenAI } = await import("openai"); openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY || "" }); } catch {}
...
async function gpt(prompt:string){
  ...
  try{
    const r:any = await openai.chat.completions.create({ model: process.env.OPENAI_MODEL || "gpt-4o-mini", messages:[{role:"system", content:SYS},{role:"user", content:prompt}], temperature:0.6 });
    return r.choices?.[0]?.message?.content?.trim() || null;
  }catch{ return null; }
}

export async function draftReply(ctx:any){
...
export async function wrapUp(obj:any){
...
export async function cueCards(obj:any){
...
export async function inmailDraft(obj:any){
...
export function docKit(contact:any){
