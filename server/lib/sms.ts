...
try {
  if (acct && auth) twilioClient = require("twilio")(acct, auth);
} catch {}

export async function sendStageSms(to: string, template: string, vars: Record<string,string> = {}) {
...
export function wantsSms(req: Request) {
...
export async function sendSMS(to: string, body: string) {
  if (!to || !/^\+\d{10,15}$/.test(to)) throw new Error("invalid_msisdn");
  if (twilioClient && from) {
