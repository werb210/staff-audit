...
const gaql = async <T>(query: string): Promise<T[]> => {
...
async function token(): Promise<string> {
  ...
  if (!j.access_token) throw new Error("google_oauth_failed");
  return j.access_token;
}

async function mutate(resource: string, body: any) {
  const c = cfg(); 
  ...
  if (!res.ok) throw new Error(`google_mutate_failed ${res.status} ${JSON.stringify(j)}`);
  return j;
}

export async function setCampaignBudget(campaignId: string, dailyMicros: number) {
  // fetch budget resource name for campaign
  const rows = await gaql<any>(`SELECT campaign.resource_name, campaign.campaign_budget FROM campaign WHERE campaign.id = ${campaignId}`);
  const budgetRes = rows?.[0]?.campaign?.campaignBudget;
  if (!budgetRes) throw new Error("budget_not_found");
...
export async function addNegativeKeywords(campaignId: string, negatives: string[]) {
  ...
  const j = await res.json(); 
  if (!res.ok) throw new Error("negatives_failed " + JSON.stringify(j));
  return j;
}