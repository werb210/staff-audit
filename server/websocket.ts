...
export const broadcastProductSync = (products: any[]) => {
...
export function emitChatRequest(data: any) {
  console.log("[WebSocket] Emitting chat request");
  io?.emit("chat:request", data);
}

export function emitChatAssigned(data: any) {
  console.log("[WebSocket] Emitting chat assigned");
  io?.emit("chat:assigned", data);
}

export const broadcastPipelineUpdate = (applicationId: string, event: string = 'update') => {
