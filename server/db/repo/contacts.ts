...
export const contactsRepo = {
  ...
  async getAll(tenantId?: TenantId): Promise<Contact[]> {
    return q<Contact>(`SELECT * FROM contacts ORDER BY created_at DESC`);
  },
  async create(c: Partial<Contact> & { id: string }): Promise<Contact> {
    const rows = await q<Contact>(
      `INSERT INTO contacts (id, full_name, email, phone, role, company_name, source, status, created_at, updated_at)
  ...
  async update(id: string, updates: Partial<Contact>): Promise<Contact | null> {
    ...
    values.push(id);
    const sql = `UPDATE contacts SET ${fields.join(',')}, updated_at=NOW() WHERE id=$${paramIndex} RETURNING *`;
