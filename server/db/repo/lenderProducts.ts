...
export const lenderProductsRepo = {
  async list(tenantId: TenantId, lenderId?: string) {
    if (lenderId) {
      return q<LenderProduct>(`SELECT * FROM lender_products WHERE lender_id=$1 ORDER BY updated_at DESC`, [lenderId]);
    }
    return q<LenderProduct>(`SELECT * FROM lender_products ORDER BY updated_at DESC`);
  },
  ...
  async create(p: Partial<LenderProduct> & { id: string; name: string; lender_id: string }): Promise<LenderProduct> {
    const rows = await q<LenderProduct>(
      `INSERT INTO lender_products (id, lender_id, name, category, min_amount, max_amount, interest_rate, term_months, active, created_at, updated_at)
  ...
  async update(id: string, updates: Partial<LenderProduct>): Promise<LenderProduct | null> {
    ...
    values.push(id);
    const sql = `UPDATE lender_products SET ${fields.join(',')}, updated_at=NOW() WHERE id=$${paramIndex} RETURNING *`;
