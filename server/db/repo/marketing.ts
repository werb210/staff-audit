...
export const marketingRepo = {
  async logIntake(e: Partial<IntakeEvent> & { id: string; source: string }) {
    ...
    return q<IntakeEvent>(
      `INSERT INTO communication_logs (id, contact_id, type, direction, content, createdAt)
  ...
  async listIntakeBetween(tenantId?: TenantId, from?: string, to?: string) {
    if (from && to) {
      return q<IntakeEvent>(`SELECT * FROM communication_logs WHERE type='marketing' AND createdAt BETWEEN $1 AND $2 ORDER BY createdAt DESC`, [from, to]);
    }
    return q<IntakeEvent>(`SELECT * FROM communication_logs WHERE type='marketing' ORDER BY createdAt DESC`);
  },
