import { Application, AppDoc, Note, Audit, Stage } from "./types";
const now = () => new Date().toISOString();

export const apps: Record<string, Application> = {
...
export const docs: Record<string, AppDoc[]> = {
...
export const notes: Record<string, Note[]> = { A1:[], A2:[], A3:[], A4:[], A5:[] };
export const audit: Audit[] = [];

export function groupedBoard() {
  const stages: Stage[] = ["new","requires_docs","in_review","lender","accepted","declined"];
  const lanes = Object.fromEntries(stages.map(s => [s, [] as Application[]]));
  Object.values(apps).forEach(a => lanes[a.stage].push(a));
  return lanes as Record<Stage, Application[]>;
}
