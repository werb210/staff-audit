...
const FLAT=(o:any,p:string[]=[] as string[],out:any={})=>{
  const isObj=(v:any)=> v && typeof v==='object' && !Array.isArray(v);
  if(Array.isArray(o)){ o.forEach((v,i)=> FLAT(v,[...p,String(i)],out)); return out; }
  if(isObj(o)){ Object.entries(o).forEach(([k,v])=> FLAT(v,[...p,k],out)); return out; }
  out[p.join(".")] = o; return out;
};
export function traceIntake(req:Request,res:Response,next:NextFunction){
  const tid = (req.header("X-Trace-Id") || (req.body?._trace?.id) || "");
  (req as any).__traceId = tid;
  ...
  (res as any).json = (data:any)=>{
    try{
      ...
      const inFields = new Set<string>(((req.body?._trace?.fields)||Object.keys(FLAT(req.body||{}))));
