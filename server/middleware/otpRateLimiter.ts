 ...
 */
export const otpGenerationRateLimit = (req: Request, res: Response, next: NextFunction) => {
 ...
 */
export const otpVerificationRateLimit = (req: Request, res: Response, next: NextFunction) => {
  const clientIP = req.ip || req.connection.remoteAddress || 'unknown';
  const currentTime = new Date();
 ...
 */
export const recordFailedVerification = (req: Request) => {
  const clientIP = req.ip || req.connection.remoteAddress || 'unknown';
  const currentTime = new Date();
  const ipKey = `otp_verify_ip_${clientIP}`;
 ...
 */
export const clearRateLimit = (req: Request, email?: string) => {
  const clientIP = req.ip || req.connection.remoteAddress || 'unknown';
  const ipKey = `otp_verify_ip_${clientIP}`;
