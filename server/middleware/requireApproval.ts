 ...
 */
export async function ensureApprovalOrQueue(
  req: TenantRequest, 
  res: Response, 
  next: NextFunction
) {
 ...
 */
export async function requireSMSApproval(
  req: TenantRequest, 
  res: Response, 
  next: NextFunction
) {
  // Set default channel and action for SMS
  if (!req.body.channel) req.body.channel = 'sms';
  if (!req.body.action) req.body.action = 'send_message';
 ...
 */
export async function requireEmailApproval(
  req: TenantRequest, 
  res: Response, 
  next: NextFunction
) {
 ...
 */
export async function requireLinkedInApproval(
  req: TenantRequest, 
  res: Response, 
  next: NextFunction
) {
  // Set default channel and action for LinkedIn
  if (!req.body.channel) req.body.channel = 'linkedin';
  if (!req.body.action) req.body.action = 'send_message';
...
export function requireHumanApprovalForContact(req: any, res: any, next: any) {
  return ensureApprovalOrQueue(req, res, next);
}