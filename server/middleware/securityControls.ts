...
type RateBucket = { tokens:number; updated:number };
const memBuckets: Record<string, RateBucket> = Object.create(null);

function now(){ return Date.now(); }
function getId(req:Request){
  const apiKey = (req.headers["x-api-key"] as string) || "";
  if (apiKey) return "key:"+apiKey.slice(0,16);
  return "ip:"+clientIp(req as any);
}

async function getIpVerdict(req:Request){
  const ip = clientIp(req as any);
  if (!ip) return { allow:true, reason:"no_ip" };

  // env lists (fast path)
  const allowEnv = parseIps(process.env.IP_ALLOWLIST||"");
  const denyEnv  = parseIps(process.env.IP_DENYLIST||"");
  if (denyEnv.some(v=> v.includes("/") ? inCidr(ip, v) : ip===v)) return { allow:false, reason:"deny_env" };
  if (allowEnv.length && !allowEnv.some(v=> v.includes("/") ? inCidr(ip, v) : ip===v)) return { allow:false, reason:"not_in_allow_env" };
  ...
  try {
    ...
    for (const row of (r.rows||[])){
      ...
      if (hit) return { allow: row.action==="allow", reason: row.action+"_db" };
    }
  } catch {}
  return { allow:true, reason:"default" };
}

export async function ipGate(req:Request, res:Response, next:NextFunction){
  const verdict = await getIpVerdict(req);
  if (!verdict.allow) return res.status(403).json({ error:"ip_blocked", reason: verdict.reason });
  next();
}

export function hashKey(secret:string){
...
export async function apiKeyAuth(req:Request, res:Response, next:NextFunction){
  ...
  const row = r.rows?.[0];
  if (!row || row.revoked_at) return res.status(401).json({ error:"invalid_api_key" });
  (req as any).apiKey = { id: row.id, name: row.name, scopes: row.scopes || [] };
...
export async function maintenanceGate(req:Request, res:Response, next:NextFunction){
  ...
  try {
    ...
    if (s.rows?.[0]?.value != null) on = !!s.rows[0].value;
  } catch {}
  if (!on) return next();
  ...
  if (isBypass || isAdmin || safePaths.some(p=> req.path.startsWith(p))) return next();
