...
export interface PipelineStageEvaluation {
  currentStage: string;
...
export class PipelineService {
   ...
   */
  static async evaluateApplicationStage(applicationId: string): Promise<PipelineStageEvaluation> {
    try {
      ...
      const docResult = await db
        .select({
          totalDocs: count(),
          acceptedDocs: sql<number>`COUNT(CASE WHEN ${sql.raw(`status`)} = 'accepted' THEN 1 END)`,
          rejectedDocs: sql<number>`COUNT(CASE WHEN ${sql.raw(`status`)} = 'rejected' THEN 1 END)`,
          pendingDocs: sql<number>`COUNT(CASE WHEN ${sql.raw(`status`)} = 'pending' THEN 1 END)`
        })
        .from(documents)
      ...
      const evaluation: PipelineStageEvaluation = {
        currentStage,
        suggestedStage,
        reason,
        shouldUpdate,
   ...
   */
  static async evaluateAndUpdateStage(applicationId: string) {
    try {
      ...
      if (evaluation.shouldUpdate) {
        ...
        try {
          const smsTemplateMap: { [key: string]: string } = {
          ...
          if (templateType) {
            ...
            import('../routes/enhancedSmsTemplates').then(({ sendEnhancedSMS }) => {
              sendEnhancedSMS(applicationId, templateType as any)
                .then((result) => {
                  if (result.success) {
   ...
   */
  static async updateApplicationStage(applicationId: string, evaluation: PipelineStageEvaluation): Promise<void> {
    try {
      if (!evaluation.shouldUpdate) {
   ...
   */
  static async processApplicationStage(applicationId: string): Promise<PipelineStageEvaluation> {
    try {
   ...
   */
  static async onDocumentChange(applicationId: string, changeType: 'upload' | 'status_change' | 'delete'): Promise<void> {
    try {
   ...
   */
  static async processAllApplications(): Promise<void> {
    try {
