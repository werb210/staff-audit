...
type Plan = { key:string; name:string; limits: Limits; price_cents:number };
...
export async function getCurrentPlan(tenant=DEFAULT_TENANT): Promise<Plan>{
...
export async function summary(tenant=DEFAULT_TENANT){
  ...
  for (const row of (r.rows||[])) used[row.metric] = Number(row.value||0);
...
async function ensureCounter(tenant:string, metric:string, period:Date){
...
export async function meter(opts:{ tenant?:string; metric:string; qty?:number; ref_type?:string; ref_id?:string; meta?:any }){
  if (String(process.env.BILLING_ENABLED||"true").toLowerCase()!=="true") return { ok:true, disabled:true };
