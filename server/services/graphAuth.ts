...
export async function saveTokens(userId: string, accessToken: string, refreshToken: string | undefined, expiresOn: Date, scope: string) {
...
export async function getValidAccessToken(userId: string): Promise<string> {
  const rows = await db.select().from(graphTokens).where(eq(graphTokens.userId, userId));
  const row = rows[0];
  if (!row) throw new Error("No Graph tokens for user");
  const exp = row.expiresAt ? new Date(row.expiresAt) : new Date(Date.now() - 1000);
  if (exp.getTime() - Date.now() < 120000) {
    if (!row.refreshToken) throw new Error("No refresh token available");
    const result = await msal.acquireTokenByRefreshToken({
      refreshToken: row.refreshToken,
      scopes: (process.env.O365_SCOPES || "").split(" ").filter(Boolean),
    } as any);
    if (!result?.accessToken) throw new Error("Refresh failed");
    const expiresOn = (result.expiresOn as Date) ?? new Date(Date.now() + 55 * 60 * 1000);
