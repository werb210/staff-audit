...
export async function onInboundMessage(threadId: string) {
  // create/refresh SLAs for the thread
  const policies = await q<any>(`SELECT id, name, target_minutes FROM sla_policies WHERE active=true`);
  const now = new Date();
...
export async function onOutboundMessage(threadId: string) {
...
export async function evaluateSlasTick() {
  ...
  const rows = await q<any>(`
    ...
    WHERE ts.satisfied_at IS NULL AND ts.breached_at IS NULL 
