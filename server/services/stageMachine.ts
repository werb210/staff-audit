import { smsOnStage, smsOnFundsDisbursed, createFollowUpTask, logStageTransition } from './pipelineSms';
 ...
 */
export function hasStagePermission(stage: PipelineStage, userRole: string): boolean {
 ...
 */
export function evaluateAutoTransitions(app: Application): { shouldMove: boolean; targetStage?: PipelineStage; reason?: string } {
 ...
 */
export async function executeStageTransition(args: StageTransitionArgs): Promise<{ success: boolean; error?: string }> {
 ...
 */
async function executeStageEntryActions(stage: PipelineStage, app: Application, contact: Contact): Promise<void> {
  const smsArgs = {
 ...
 */
async function startStageTimers(stage: PipelineStage, appId: string): Promise<void> {
