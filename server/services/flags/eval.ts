...
function bucket(s: string): number { return hashStr(s) % 10000; } // 0..9999
function pctToThreshold(p: number): number { return Math.max(0, Math.min(100, p)) * 100; }

/* --------- Context extraction --------- */
export type FlagContext = { userId?: string; role?: string; tenantId?: string; ip?: string };
export function ctxFromReq(req: any): FlagContext {
  return {
...
/* --------- Flags --------- */
export async function isFlagEnabled(key: string, ctx: FlagContext): Promise<{value:boolean, reason:string}>{
  ...
  const by = (s:string)=> ovs.find(o=>o.scope===s);
  if (by('user'))   return { value: !!by('user')!.value, reason:"override:user" };
  if (by('role'))   return { value: !!by('role')!.value, reason:"override:role" };
  if (by('tenant')) return { value: !!by('tenant')!.value, reason:"override:tenant" };
  if (by('global')) return { value: !!by('global')!.value, reason:"override:global" };
...
/* --------- Experiments --------- */
export async function assignExperiment(expKey:string, ctx:FlagContext){
  ...
  const exp = er.rows?.[0];
  if (!exp) return { variant: process.env.FLAGS_DEFAULT_VARIANT || "control", reason: "missing" };

  if (exp.status !== 'running') return { variant: process.env.FLAGS_DEFAULT_VARIANT || "control", reason: exp.status };

  // optional gate by flag
  if (exp.exposure_flag_key) {
    const f = await isFlagEnabled(exp.exposure_flag_key, ctx);
    if (!f.value) return { variant: process.env.FLAGS_DEFAULT_VARIANT || "control", reason:`flag:${f.reason}` };
  }
  ...
  if (b >= alloc) return { variant: "control", reason:"outside_allocation" };
  ...
  const vars = vr.rows || [];
  if (!vars.length) return { variant:"control", reason:"no_variants" };

  const total = vars.reduce((s:any,v:any)=> s + Number(v.weight||0), 0) || 100;
  let acc = 0;
  for (const v of vars){
    const next = acc + Math.floor((Number(v.weight||0) / total) * alloc);
    if (b >= acc && b < next) return { variant: v.variant_key, reason:`bucket:${b}/${alloc}` };
    acc = next;
  }
