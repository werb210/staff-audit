...
export const canonicalEmail = (raw: string | null | undefined = '') => {
...
export function mergeContactsFromApplications(apps: App[]) {
  const groups = new Map<string, App[]>();
  for (const a of apps) {
    const em = canonicalEmail(a.contactEmail as string);
    if (!em) continue;
  ...
  for (const [email, arr] of groups) {
    ...
    const phones = Array.from(new Set(arr.flatMap(a => [a.mobile, a.phone].filter(Boolean) as string[])));
    const phone = phones[0] || '';
    // most-frequent company label
    const companyCounts = arr.reduce((m,a)=> {
      const key = (a.businessName||'').trim();
      if (!key) return m; m.set(key, (m.get(key)||0)+1); return m;
