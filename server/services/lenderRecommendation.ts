...
interface LenderMatchResult {
  ...
  lender_name: string;
  match_score: number;
  estimated_approval: 'High' | 'Medium' | 'Low';
  missing_criteria: string[];
  overrides_applied?: string[];
}

export function recommendLenders(
  ...
  for (const lender of lenderProducts) {
    ...
    // ✅ Documents
    const missingDocs = (lender.required_documents || []).filter(
      (doc) => !applicant.documents.includes(doc)
    );
