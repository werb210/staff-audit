...
export class BackupService {
  ...
  async createMonthlySnapshot(): Promise<void> {
  ...
  async createApplicationSnapshot(applicationId: string, month: string): Promise<string> {
    try {
  ...
  private async createZipFile(applicationId: string, docs: any[]): Promise<string> {
  ...
  private async recordBackupSummary(month: string, successCount: number, failureCount: number, duration: number): Promise<void> {
    try {
      await db.insert(documentBackups).values({
        backupDate: new Date(),
  ...
  async triggerManualBackup(applicationId?: string): Promise<{ success: boolean; message: string }> {
    try {
