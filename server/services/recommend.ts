import { ApplicantSignal, LenderProduct, ScoredProduct } from "../domain/lenders";

export function scoreProducts(app: ApplicantSignal, products: LenderProduct[]): ScoredProduct[] {
  const out: ScoredProduct[] = [];

  for (const p of products) {
    if (p.country !== app.country) continue;
    if (app.amountRequested < p.minAmount || app.amountRequested > p.maxAmount) {
      // we still include LOC later; skip others out of range
      if (p.category !== "LOC") continue;
    }
    ...
    // Category match
    if (p.category === app.category) { score += 20; reasons.push("Category match"); }

    // Amount fit
    if (app.amountRequested >= p.minAmount && app.amountRequested <= p.maxAmount) {
      score += 15; reasons.push("Within amount range");
    } else if (p.category === "LOC") {
      // LOC fallback inclusion (always include if within range)
      if (app.amountRequested >= p.minAmount && app.amountRequested <= p.maxAmount) {
        score += 10; reasons.push("LOC fallback in range");
      } else {
    ...
    if (p.minMonthsInBusiness && (app.monthsInBusiness ?? 0) >= p.minMonthsInBusiness) {
      score += 5; reasons.push("Meets months-in-business");
    }
    if (p.minMonthlyRevenue && (app.monthlyRevenue ?? 0) >= p.minMonthlyRevenue) {
      score += 5; reasons.push("Meets monthly revenue");
    }
    ...
    if (p.industriesExcluded?.length && app.industry) {
      if (p.industriesExcluded.includes(app.industry)) {
        score -= 40; reasons.push("Industry exclusion");
      }
    }
