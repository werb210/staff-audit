...
import { REQUIRED_FIELDS, RequiredField } from './requiredFieldsList';
import { db } from '../../db';
...
export interface ExtractedField {
  ...
  location?: string; // page/section where found
...
export class DocumentInsightsService {
  ...
  async extractRequiredFieldsRegex(text: string): Promise<ExtractedField[]> {
  ...
  async extractRequiredFieldsAI(text: string, missingFields: string[]): Promise<ExtractedField[]> {
...
Only include fields that are clearly found in the document. If a field is not found or unclear, don't include it.`;
      ...
      return (result.extractedFields || []).map((field: any) => ({
        label: field.label,
        value: field.value,
  ...
  async assessDocumentQuality(text: string, fileName: string): Promise<{
  ...
  async extractDocumentInsights(documentId: string): Promise<DocumentInsights> {
    try {
  ...
  private async saveInsightsToDocument(documentId: string, insights: DocumentInsights): Promise<void> {
    try {
      await db
        .update(documents)
        .set({
  ...
  async batchProcessDocuments(documentIds: string[]): Promise<{
    processed: DocumentInsights[];
    errors: { documentId: string; error: string }[];
  }> {
    const processed: DocumentInsights[] = [];
