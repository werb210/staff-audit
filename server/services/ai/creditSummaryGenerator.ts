...
export class CreditSummaryGeneratorService {
  private readonly SYSTEM_PROMPT = `You are a senior credit analyst preparing executive credit summaries for lending decisions. Create a comprehensive, professional one-page credit summary that includes:
...
Format the output in clean, professional markdown. Be concise but thorough. Focus on decision-relevant information for senior lenders.`;
  ...
  async generateCreditSummary(applicationId: string): Promise<CreditSummary> {
    try {
  ...
  private async gatherApplicationData(applicationId: string): Promise<CreditSummaryData> {
    try {
      // Get application
      const app = await db
        .select()
  ...
  private async generateAISummary(data: CreditSummaryData): Promise<string> {
    try {
...
Create a professional, one-page credit summary in markdown format with clear sections for Business Overview, Requested Funding, Financial Snapshot, Risk Assessment, and Recommendation.`;
  ...
  private generateFallbackSummary(data: CreditSummaryData): string {
...
*Note: AI summary generation failed - manual review required*`;
  ...
  private async generatePDF(summaryText: string, data: CreditSummaryData): Promise<Uint8Array> {
    try {
      const pdfOptions = {
  ...
  private extractFinancialHighlights(application: any, fieldAggregation: any): any {
  ...
  // Identify risk factors
  private identifyRiskFactors(application: any, discrepancyReport: any, bankingAnalysis: any): string[] {
    const risks: string[] = [];
  ...
  // Generate recommendations
  private generateRecommendations(application: any, riskFactors: string[], financialHighlights: any): string[] {
  ...
  private createMetadata(data: CreditSummaryData): CreditSummary['metadata'] {
    ...
    if (data.documentInsights.documentCount >= 3) confidence += 0.2;
    if (data.documentInsights.documentCount >= 5) confidence += 0.1;
    if (data.discrepancyReport?.discrepancies?.length === 0) confidence += 0.1;
