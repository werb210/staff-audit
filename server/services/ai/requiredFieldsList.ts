export interface RequiredField {
  label: string;
  category: 'business' | 'financial' | 'personal' | 'compliance';
  pattern?: string;
  aliases?: string[];
  required: boolean;
}
...
export const getFieldsByCategory = (category: RequiredField['category']): RequiredField[] => {
  return REQUIRED_FIELDS.filter(field => field.category === category);
};

// Get required fields only
export const getRequiredFields = (): RequiredField[] => {
  return REQUIRED_FIELDS.filter(field => field.required);
};

// Find field by label or alias
export const findField = (searchTerm: string): RequiredField | undefined => {
  const term = searchTerm.toLowerCase();
  return REQUIRED_FIELDS.find(field => 
    field.label.toLowerCase() === term || 
