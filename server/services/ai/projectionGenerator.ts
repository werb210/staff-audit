...
export interface FinancialProjectionPeriod {
  year: number;
  revenue: number;
  grossProfit: number;
...
export interface ProjectionAssumptions {
  ...
  businessMaturity: 'startup' | 'growth' | 'mature';
...
export interface FinancialProjections {
  ...
  baselineData: {
    ...
    currentMargins: number;
    currentExpenses: number;
    currentCashFlow: number;
  };
  assumptions: ProjectionAssumptions;
  oneYearProjection: FinancialProjectionPeriod;
  threeYearProjections: FinancialProjectionPeriod[];
  keyInsights: string[];
  riskFactors: string[];
  recommendations: string[];
  scenarioAnalysis: {
    optimistic: { revenueChange: number; profitChange: number };
    pessimistic: { revenueChange: number; profitChange: number };
  };
...
export class ProjectionGeneratorService {
  private readonly SYSTEM_PROMPT = `You are a senior financial analyst specializing in business projections for lending purposes. Analyze financial statements, bank statements, and business documents to create realistic, data-driven financial projections.
...
Be thorough, realistic, and conservative in your analysis.`;
  ...
  async generateFinancialProjections(applicationId: string): Promise<FinancialProjections> {
    try {
  ...
  private filterFinancialDocuments(docs: any[]): any[] {
  ...
  private async analyzeBaselineFinancials(financialData: string): Promise<FinancialProjections['baselineData']> {
    try {
...
Provide actual numbers in CAD. For growth rate, use percentage (e.g., 15 for 15%).`;
  ...
  private async generateAssumptions(
    ...
    baseline: FinancialProjections['baselineData']
...
Generate conservative assumptions:
{
  ...
  private async generateOneYearProjection(
    baseline: FinancialProjections['baselineData'],
    assumptions: ProjectionAssumptions
  ): Promise<FinancialProjectionPeriod> {
    ...
    switch (assumptions.grossMarginTrend) {
      case 'improving': marginAdjustment = 2; break;
      case 'declining': marginAdjustment = -2; break;
  ...
  private async generateThreeYearProjections(
    baseline: FinancialProjections['baselineData'],
    assumptions: ProjectionAssumptions
  ): Promise<FinancialProjectionPeriod[]> {
    const projections: FinancialProjectionPeriod[] = [];
    ...
    for (let year = 1; year <= 3; year++) {
      ...
      let marginAdjustment = 0;
      switch (assumptions.grossMarginTrend) {
        case 'improving': marginAdjustment = year * 1; break;
        case 'declining': marginAdjustment = -year * 1; break;
        default: marginAdjustment = 0;
      }
  ...
  private async generateInsights(
    baseline: FinancialProjections['baselineData'],
    oneYear: FinancialProjectionPeriod,
    threeYear: FinancialProjectionPeriod[]
  ): Promise<string[]> {
  ...
  private async identifyRiskFactors(financialData: string, assumptions: ProjectionAssumptions): Promise<string[]> {
    const risks: string[] = [];
  ...
  private async generateRecommendations(insights: string[], risks: string[]): Promise<string[]> {
  ...
  private generateScenarioAnalysis(
    oneYear: FinancialProjectionPeriod,
    assumptions: ProjectionAssumptions
  ): FinancialProjections['scenarioAnalysis'] {
  ...
  private calculateConfidence(docs: any[], baseline: FinancialProjections['baselineData']): number {
    let confidence = 0.3; // Base confidence

    // Document quality
    if (docs.length >= 3) confidence += 0.2;
    if (docs.length >= 5) confidence += 0.1;

    // Data availability
    if (baseline.currentRevenue > 0) confidence += 0.2;
    if (baseline.historicalGrowthRate !== 5) confidence += 0.1; // Not default value
    if (baseline.currentExpenses > 0) confidence += 0.1;

    return Math.min(1, confidence);
  }

  // Generate basic projections when no financial documents available
  private generateBasicProjections(applicationId: string): FinancialProjections {
    const baselineData = {
      currentRevenue: 0,
      historicalGrowthRate: 5,
      currentMargins: 20,
