...
export interface FieldMismatch {
  fieldName: string;
  applicationValue: string | number | null;
  ocrValue: string;
  source: string; // Document name/type where OCR value was found
...
export class ConflictCheckerService {
  ...
  async detectFieldMismatches(applicationId: string): Promise<ConflictReport> {
    try {
  ...
  private compareApplicationToOCR(application: any, aggregatedFields: any): FieldMismatch[] {
  ...
  private compareStringValues(appValue: string, ocrValue: string): boolean {
    ...
    // Exact match
    if (normalizedApp === normalizedOCR) return false;
  ...
  private compareNumericValues(appValue: any, ocrValue: string): boolean {
    const appNum = parseFloat(String(appValue).replace(/[^0-9.-]/g, ''));
    const ocrNum = parseFloat(ocrValue.replace(/[^0-9.-]/g, ''));

    if (isNaN(appNum) || isNaN(ocrNum)) return true; // Mismatch if can't parse
  ...
  private findFieldSource(fieldName: string, aggregatedFields: any): { documentName: string; confidence: number } {
    const fieldMap = aggregatedFields.fieldMap || {};
    const fieldEntries = fieldMap[fieldName] || [];

    if (fieldEntries.length > 0) {
      // Return the highest confidence source
      const bestEntry = fieldEntries.reduce((best: any, current: any) => 
  ...
  private generateFieldRecommendation(fieldName: string, severity: string): string {
  ...
  private calculateOverallRisk(mismatches: FieldMismatch[]): 'low' | 'medium' | 'high' | 'critical' {
    ...
    if (highCount >= 1 || mismatches.length >= 3) return 'medium';
    return 'low';
  }

  // Generate overall recommendations
  private generateRecommendations(mismatches: FieldMismatch[]): string[] {
  ...
  private getCheckedFields(application: any, aggregatedFields: any): string[] {
  ...
  async batchCheckConflicts(applicationIds: string[]): Promise<{
