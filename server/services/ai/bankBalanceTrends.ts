...
export interface MonthlyBalanceExtremes {
  month: string;
  year: number;
  maxBalance: number;
  minBalance: number;
  ...
  balanceRange: number;
  volatility: number; // Standard deviation of daily balances
  daysOverdraft: number;
  daysAbove10k: number;
...
export interface BalanceTrendAnalysis {
  ...
  monthlyExtremes: MonthlyBalanceExtremes[];
  overallTrends: {
    overallTrend: 'improving' | 'declining' | 'stable';
    averageVolatility: number;
    highestBalance: number;
    lowestBalance: number;
    monthsWithOverdraft: number;
    averageMonthlyRange: number;
  };
...
export class BankBalanceTrendsService {
  ...
  async extractMonthlyExtremes(documentId: string): Promise<BalanceTrendAnalysis> {
    try {
      ...
      const analysis: BalanceTrendAnalysis = {
        documentId,
  ...
  private processMonthlyStats(monthlyStats: any[]): MonthlyBalanceExtremes[] {
    return monthlyStats.map((monthStat, index) => {
      ...
      if (index > 0) {
        ...
        if (changePercent > 0.1) monthlyTrend = 'up';
        else if (changePercent < -0.1) monthlyTrend = 'down';
      }
  ...
  private calculateOverallTrends(monthlyExtremes: MonthlyBalanceExtremes[]): BalanceTrendAnalysis['overallTrends'] {
    ...
    if (monthlyExtremes.length >= 3) {
      ...
      if (change > 0.15) overallTrend = 'improving';
      else if (change < -0.15) overallTrend = 'declining';
    }
  ...
  // Helper functions
  private getEarliestDate(monthlyExtremes: MonthlyBalanceExtremes[]): string {
    if (monthlyExtremes.length === 0) return new Date().toISOString();
    const earliest = monthlyExtremes[0];
    return new Date(earliest.year, this.getMonthNumber(earliest.month), 1).toISOString();
  }

  private getLatestDate(monthlyExtremes: MonthlyBalanceExtremes[]): string {
    if (monthlyExtremes.length === 0) return new Date().toISOString();
  ...
  private generateEmptyAnalysis(documentId: string): BalanceTrendAnalysis {
    return {
      documentId,
  ...
  async batchAnalyzeBalanceTrends(documentIds: string[]): Promise<{
    analyses: BalanceTrendAnalysis[];
    summary: {
    ...
    const analyses: BalanceTrendAnalysis[] = [];
