...
export interface MultiDocumentSummary {
...
export class DocumentSummarizerService {
  private readonly SYSTEM_PROMPT = `You are a financial document analyst specializing in loan applications. Analyze documents comprehensively to identify:
...
Group similar document types and provide actionable insights for lending decisions. Be thorough but concise.`;
  ...
  async summarizeMultipleDocuments(applicationId: string): Promise<MultiDocumentSummary> {
    try {
      ...
      const summary: MultiDocumentSummary = {
        applicationId,
  ...
  private categorizeDocuments(docs: any[]): { [category: string]: any[] } {
  ...
  private determineDocumentCategory(docType: string): string {
  ...
  private async processCategoryDocuments(category: string, docs: any[]): Promise<DocumentSummarySection> {
    const processedDocs = [];
  ...
  // Analyze individual document
  private async analyzeIndividualDocument(doc: any, text: string): Promise<DocumentSummarySection['documents'][0]> {
    try {
...
Focus on financial data, business information, and lending-relevant details.`;
  ...
  private validateDataQuality(quality: any): 'excellent' | 'good' | 'fair' | 'poor' {
  ...
  private async analyzeCategoryDocuments(
    category: string, 
    docs: DocumentSummarySection['documents']
  ): Promise<{ insights: string[]; risks: string[] }> {
    try {
...
- Data quality patterns`;
  ...
  private async generateOverallAnalysis(docs: any[], sections: DocumentSummarySection[]): Promise<{
    insights: string[];
    criticalFindings: string[];
    recommendations: string[];
  }> {
...
- Action items for underwriters`;
  ...
  private calculateAvgQuality(docs: DocumentSummarySection['documents']): number {
  ...
  private calculateCompletenessScore(docs: any[], sections: DocumentSummarySection[]): number {
  ...
  private calculateRiskScore(sections: DocumentSummarySection[]): 'low' | 'medium' | 'high' | 'critical' {
    let riskFactors = 0;
    ...
    if (riskFactors >= 10) return 'critical';
  ...
  private identifyMissingDocuments(docs: any[]): string[] {
    ...
    for (const criticalDoc of criticalDocs) {
      const hasDoc = criticalDoc.keywords.some(keyword => 
        docTypes.some(docType => docType.includes(keyword))
      );
  ...
  private generateEmptySummary(applicationId: string): MultiDocumentSummary {
    return {
      applicationId,
      totalDocuments: 0,
      sections: [],
