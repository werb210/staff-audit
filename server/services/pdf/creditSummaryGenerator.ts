...
export class CreditSummaryGenerator {
  ...
  async generateAISummary(application: any, applicationDocs: any[]): Promise<string> {
    try {
      const documentSummary = applicationDocs.map(doc => 
        `- ${doc.documentType}: ${doc.status || 'uploaded'}`
      ).join('\n');
...
Format as professional text suitable for a PDF report. Be concise but thorough.`;
  ...
  private generateFallbackSummary(application: any): string {
...
`;
  ...
  async generateCreditSummaryPDF(applicationId: string): Promise<Buffer> {
    try {
  ...
  async generateEditableSummary(applicationId: string): Promise<{
    applicationData: any;
    aiSummary: string;
  ...
  private async extractRiskFactors(application: any, docs: any[]): Promise<string[]> {
    try {
...
Return only a JSON array of risk factors as strings.`;
  ...
  private async generateRecommendations(application: any, docs: any[]): Promise<string[]> {
    try {
