 ...
 */
export async function runOcrOnDocument(documentId: string): Promise<string> {
 ...
 */
async function processDocumentWithVision(imageUrl: string, documentType: string, fileName: string): Promise<OCRResult> {
  try {
 ...
 */
async function processPDFDocument(preSignedUrl: string, documentType: string, fileName: string): Promise<OCRResult> {
  try {
 ...
 */
function parseExtractedData(text: string, documentType: string): any {
  try {
 ...
 */
function extractBankInfo(text: string): any {
  const bankInfo: any = {};
 ...
 */
function extractFinancialInfo(text: string): any {
 ...
 */
function calculateConfidenceScore(extractedData: any, rawText: string): number {
  ...
  if (rawText.length > 500) score += 15;
 ...
 */
async function runBankingAnalysis(documentId: string, ocrText: string): Promise<void> {
  try {
 ...
 */
export async function processApplicationDocuments(applicationId: string): Promise<void> {
  try {
