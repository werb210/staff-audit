...
export class OCRService {
  ...
  private async extractDocumentDataWithAI(fileBuffer: Buffer, documentType: string): Promise<OCRResult> {
    try {
...
- confidence: Score from 0-1 indicating extraction quality`;
  ...
  private async extractDocumentDataFallback(fileBuffer: Buffer, documentType: string): Promise<OCRResult> {
  ...
  private async performAIBankingAnalysis(rawText: string): Promise<OCRResult['bankingInsights']> {
    try {
  ...
  async saveOCRResults(applicationId: string, documentId: string, results: OCRResult) {
    try {
