...
export async function generateApplicationPDF(applicationId: string): Promise<Buffer> {
  ...
  try {
    ...
    const canon: any = appResult.applicationCanon ?? {};
 ...
 */
export async function generateAllApplicationPDFs(): Promise<{
  total: number;
  successful: number;
  failed: number;
  ...
  try {
    ...
    const results = {
      ...
      successful: 0,
      failed: 0,
      errors: [] as { applicationId: string; error: string }[]
    };
 ...
 */
export async function testPDFGeneration(applicationId?: string): Promise<{
  success: boolean;
  applicationId?: string;
  filename?: string;
  fileSize?: number;
