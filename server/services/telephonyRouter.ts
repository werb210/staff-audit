...
export class TelephonyRouter {
  ...
  async handleIncomingCall(fromNumber: string, toNumber: string, callSid: string) {
    try {
  ...
  async handleCallAnswered(callSid: string, answeredByTenant: 'bf' | 'slf', answeredByUser: string, contactId?: string) {
    try {
  ...
  // Handle incoming SMS
  async handleIncomingSms(fromNumber: string, toNumber: string, body: string, messageSid: string) {
    try {
  ...
  async handleOutgoingCall(contactId: string, tenant: 'bf' | 'slf', userId: string, toNumber: string) {
    try {
