...
export async function runAutoMigrations() {
  ...
  try { await pool.query(`create table if not exists _migrations (name text primary key, applied_at timestamptz default now())`); } catch {}
  let files: string[] = [];
  ...
  try {
    ...
    for (const r of res.rows) applied.add(r.name);
  } catch {}
