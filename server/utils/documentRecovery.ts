...
/**
 * Document recovery utility to fix missing file paths in database
 */
 ...
 */
export async function recoverMissingDocuments(): Promise<RecoveryResult> {
 ...
 */
async function scanAllFiles(): Promise<string[]> {
  const files: string[] = [];
 ...
 */
async function findMatchingFile(doc: any, allFiles: string[]): Promise<string | null> {
 ...
 */
export async function previewRecovery(): Promise<{ canRecover: number; totalMissing: number }> {
  try {
    const allDocs = await db.select()
      .from(documents)
      .where(documents.filePath.like('uploads/documents/%'));
