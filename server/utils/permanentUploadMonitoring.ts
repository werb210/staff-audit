...
import { applications, documents } from '../../shared/schema.js';
 ...
 */
export async function findZeroDocumentApplications(hours: number = 24): Promise<any[]> {
  try {
 ...
 */
export async function checkUploadSystemHealth(): Promise<{
  totalApplications: number;
  totalDocuments: number;
  averageDocsPerApp: number;
  zeroDocumentApps: number;
  ...
  try {
    ...
    const totalApplications = parseInt(appCountResult.rows[0]?.count as string) || 0;
    const totalDocuments = parseInt(docCountResult.rows[0]?.count as string) || 0;
 ...
 */
export async function checkAdminAlertThresholds(): Promise<{
  shouldAlert: boolean;
  reasons: string[];
  metrics: any;
}> {
 ...
 */
export function logConnectionEvent(eventType: string, applicationId: string, details?: any): void {
