 ...
 */
export async function checkDocumentAccessible(filePath: string, applicationId?: string, fileName?: string): Promise<boolean> {
  ...
  try {
    ...
    const possibleAppId = applicationId || pathParts.find(part => 
 ...
 */
async function recursiveFileSearch(fileName: string, searchDir: string): Promise<string | null> {
  try {
 ...
 */
export async function filterAccessibleDocuments(documents: any[]): Promise<any[]> {
  const accessibilityChecks = await Promise.all(
    documents.map(async (doc) => {
      const isAccessible = await checkDocumentAccessible(
