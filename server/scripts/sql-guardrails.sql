...
ON contacts (phone, LOWER(company_name)) 
WHERE phone IS NOT NULL AND phone != '' AND company_name IS NOT NULL;

-- 3. APPLICATIONS Table Guardrails
-- Prevent duplicate business names within same tenant  
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_applications_tenant_business_unique 
ON applications (tenant_id, LOWER(business_name)) 
...
-- Prevent duplicate lender names within tenant
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_lender_tenant_name_unique 
ON "Lender" (tenant, LOWER(name));

-- 5. LENDER PRODUCT Table Guardrails (already in schema but ensuring)  
-- Prevent duplicate product names per lender
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_lender_product_name_unique 
ON "LenderProduct" ("lenderId", LOWER(name));
...
ON crm_contacts (tenant_id, company_id, LOWER(first_name), LOWER(last_name))
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO duplicate_attempt_log (table_name, attempted_data, conflict_reason, created_at)
...
WHERE indexname LIKE '%_unique' 
   OR indexname LIKE 'idx_%duplicate%'
...
COMMENT ON INDEX idx_applications_tenant_business_unique IS 'DupCheck Pack: Prevents duplicate business names per tenant';
COMMENT ON INDEX idx_lender_tenant_name_unique IS 'DupCheck Pack: Prevents duplicate lender names per tenant';
