...
async function q<T=any>(s:TemplateStringsArray,...v:any[]){ 
  return (await db.execute(sql.raw(String.raw(s,...v)))).rows as T[]; 
}

(async()=>{
  const out:any={ ts:new Date().toISOString(), ok:true, db:{} };
