...
export async function smsStaff(msg: string, fallbackOnly = false) {
...
export async function lastNDaysAds(tenant: string, days: number) {
  const d = new Date(); 
...
export async function lastActivityAt(opts: { contactId?: string; applicationId?: string }) {
