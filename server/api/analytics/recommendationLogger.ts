...
router.post("/recommendation-log", async (req, res) => {
  try {
    ...
    const parsed =
      typeof insertRecommendationLogSchema?.parse === "function"
...
router.get("/recommendation-dashboard", async (_req, res) => {
  try {
    ...
    const rejectionReasonsResult = await db.execute(
      sql/*sql*/ `
        SELECT
    ...
    const topLendersResult = await db.execute(
      sql/*sql*/ `
        SELECT
    ...
    const stats =
      (Array.isArray((statsResult as any).rows)
        ? (statsResult as any).rows[0]
        : Array.isArray(statsResult)
        ? (statsResult as any)[0]
        : statsResult) || {};

    const topRejectionReasons =
      (Array.isArray((rejectionReasonsResult as any).rows)
        ? (rejectionReasonsResult as any).rows
        : (rejectionReasonsResult as any)) || [];

    const mostRecommendedLenders =
      (Array.isArray((topLendersResult as any).rows)
        ? (topLendersResult as any).rows
        : (topLendersResult as any)) || [];
