...
export function productEligible(product: any, form: any): boolean {
  ...
  if (cat === "Purchase Order Financing" && 'industryRestriction' in rules && rules.industryRestriction) {
    const industry = lower(form.industry);
    const ok = rules.industryRestriction.some((i: string) => industry.includes(lower(i)));
    if (!ok) return false;
  }

  const revOk  = !rules.minMonthlyRevenue || (form.monthlyRevenue || 0) >= rules.minMonthlyRevenue;
...
export function getEligibleProducts(products: any[], applicationForm: any): any[] {
  return products.filter(product => productEligible(product, applicationForm));
}

// Category-specific validation for forms
export function validateCategoryRequirements(category: string, form: any): { valid: boolean; errors: string[] } {
  ...
  if (rules.minMonthlyRevenue && (form.monthlyRevenue || 0) < rules.minMonthlyRevenue) {
  ...
  if (category === "Purchase Order Financing" && 'industryRestriction' in rules && rules.industryRestriction) {
    const industry = lower(form.industry);
    const validIndustry = rules.industryRestriction.some((i: string) => industry.includes(lower(i)));
    if (!validIndustry) {
