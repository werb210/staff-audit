  ...
  return (
    ...
    >
      ...
      <div className="text-xs space-y-1 mb-2">
        {f.industry && (
          <div>
            <b>Industry:</b> {f.industry}
          </div>
        )}
        {f.businessType && (
          <div>
            <b>Type:</b> {f.businessType}
          </div>
        )}
        {f.annualRevenue && (
          <div>
            <b>Annual Revenue:</b> ${Number(f.annualRevenue).toLocaleString()}
          </div>
        )}
        {f.yearsInBusiness && (
          <div>
            <b>Years in Business:</b> {f.yearsInBusiness}
          </div>
        )}
        {f.numberOfEmployees && (
          <div>
            <b>Employees:</b> {f.numberOfEmployees}
          </div>
        )}
        {f.businessAddress && (
          <div>
            <b>Address:</b>{" "}
            <span className="truncate">{f.businessAddress}</span>
          </div>
        )}
        {f.website && (
          <div>
            <b>Website:</b> <span className="truncate">{f.website}</span>
          </div>
        )}
        {f.useOfFunds && (
          <div>
            <b>Use of Funds:</b>{" "}
            <span className="truncate">{f.useOfFunds}</span>
          </div>
        )}
      </div>
      ...
      <div className="text-xs flex justify-between items-center mb-2 pt-1 border-t">
        <div>
          ...
          {f.requestedAmount != null
            ? `$${Number(f.requestedAmount).toLocaleString()}`
            : "—"}
        </div>
        <div>
          <b>Created:</b>{" "}
          ...
          <div className="flex items-center gap-2">
            ...
            {c.missingDocuments && c.missingDocuments.length > 0 && (
              <span className="text-red-600">
                Missing: {c.missingDocuments.length}
              </span>
            )}
          </div>
        </div>
            ...
            <div className="space-y-0.5">
              ...
              {c.documentsUploaded.length > 3 && (
                <div className="text-xs text-gray-500">
                  +{c.documentsUploaded.length - 3} more
                </div>
              )}
            </div>
          </div>
