...
export async function reportFeatureMount(featureId: string) {
  try {
    ...
    console.warn(
      `[Feature Registry] Failed to report mount for ${featureId}:`,
      err,
    );
  }
}
...
export async function reportActionAvailable(featureId: string) {
  try {
    ...
    console.warn(
      `[Feature Registry] Failed to report action for ${featureId}:`,
      err,
    );
  }
}
...
export async function setFeatureOption(
  ...
  try {
    ...
    console.error(
      `[Feature Registry] Failed to set option ${key} for ${featureId}:`,
      err,
    );
    throw err;
  }
