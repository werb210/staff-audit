...
const KEY = (import.meta as any).env?.VITE_DIALER_FORCE_OPEN_KEY || "Shift+D";
function matches(e: KeyboardEvent) {
...
export function installDialerHotkey() {
  ...
  window.addEventListener("keydown", (e) => {
    if (matches(e)) {
      ...
      if (typeof anyWin.openDialer === "function") anyWin.openDialer();
      else window.dispatchEvent(new CustomEvent("dialer:open"));
