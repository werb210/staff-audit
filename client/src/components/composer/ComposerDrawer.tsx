...
export default function ComposerDrawer({
  ...
  useEffect(() => {
    if (open) {
      ...
      setEmailSubject(
        `Follow up - ${contact?.name || contact?.company || "Discussion"}`,
      );
      setEmailBody("");
      setSmsBody("");
      setNoteContent("");
    ...
    try {
      const response = await fetch("/api/composer/doc-kit", {
        ...
        body: JSON.stringify({
          contact: contact,
          files: [
            { name: "application_docs.pdf", contentType: "application/pdf" },
          ],
        }),
      });
      const data = await response.json();
  ...
  const handleGetLinkedInLinks = async () => {
    ...
    try {
      const response = await fetch(
        `/api/composer/linkedin/links?companyDomain=${encodeURIComponent(contact?.company || "")}&profileUrl=${encodeURIComponent(contact?.linkedinUrl || "")}`,
      );
      const data = await response.json();
  ...
  const handleCreateMeeting = async () => {
    ...
    try {
      ...
      const endISO = new Date(
        new Date(startISO).getTime() + 30 * 60000,
      ).toISOString(); // 30 min meeting
      ...
      <div className="absolute inset-y-0 right-0 w-[680px] max-w-[95vw] bg-white shadow-xl border-l flex flex-col">
        ...
        >
          <TabsList className="m-4 grid w-auto grid-cols-7">
            ...
            <TabsTrigger value="calllog" className="text-xs">
              Call Log
            </TabsTrigger>
            ...
            <TabsContent value="sms" className="p-4 space-y-4">
              <div className="space-y-4">
                <div className="text-sm text-gray-600">
                  To:{" "}
                  {contact?.phone || contact?.mobilePhone || "No phone number"}
                </div>
                <Textarea
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={handleGetLinkedInLinks}
                      disabled={isLoading}
                    >
                      Get LinkedIn Links
                    </Button>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        {linkedinLinks.salesNavigator && (
                          <a
                            href={linkedinLinks.salesNavigator}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center text-blue-600 hover:underline text-sm"
                          >
                        ...
                        {linkedinLinks.peopleAtCompany && (
                          <a
                            href={linkedinLinks.peopleAtCompany}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center text-blue-600 hover:underline text-sm"
                          >
            ...
            <TabsContent value="calllog" className="p-4 space-y-4">
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <Textarea placeholder="Call notes and summary..." rows={8} />
