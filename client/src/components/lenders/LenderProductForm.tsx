...
const lenderProductSchema = z.object({
  ...
  country: z.enum(['CA', 'US'], { required_error: 'Please select a country' }),
  category: z.enum(PRODUCT_CATEGORIES as [string, ...string[]], { required_error: 'Please select a category' }),
  amountMin: z.number().min(0, 'Minimum amount must be 0 or greater').optional(),
  amountMax: z.number().min(0, 'Maximum amount must be 0 or greater').optional(),
  termMin: z.number().min(1, 'Minimum term must be at least 1 month').max(120, 'Minimum term cannot exceed 120 months').optional(),
  termMax: z.number().min(1, 'Maximum term must be at least 1 month').max(120, 'Maximum term cannot exceed 120 months').optional(),
  rateFrom: z.number().min(0, 'Rate must be 0 or greater').max(100, 'Rate cannot exceed 100%').optional(),
  rateTo: z.number().min(0, 'Rate must be 0 or greater').max(100, 'Rate cannot exceed 100%').optional(),
  feesFlat: z.number().min(0, 'Flat fees must be 0 or greater').optional(),
  feesPct: z.number().min(0, 'Percentage fees must be 0 or greater').optional(),
...
export default function LenderProductForm({
  ...
  const { data: lenders } = useQuery({
    ...
    queryFn: async () => {
      ...
      return response as { id: string; name: string; countryCoverage?: string }[];
    }
  });
  ...
  const createMutation = useMutation({
    mutationFn: async (data: LenderProductFormData) => {
      ...
      const response = await api('/api/lender-products', { method: 'POST', body: data });
      return response;
    },
    onSuccess: (product) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: LenderProductFormData) => {
      ...
      const response = await api(`/api/lender-products/${productId}`, { method: 'PUT', body: data });
      return response;
    },
    onSuccess: (product) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['lender-products'] });
      queryClient.invalidateQueries({ queryKey: ['lender-product', productId] });
      onSaved(product);
      onClose();
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      ...
      const response = await api(`/api/lender-products/${productId}`, { method: 'DELETE' });
      return response;
    },
    onSuccess: () => {
  ...
  const handleDelete = () => {
    if (window.confirm('Are you sure you want to delete this lender product? This action cannot be undone.')) {
      setIsDeleting(true);
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Business Line of Credit" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
