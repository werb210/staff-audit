...
export default function LenderCatalogTable({
  className = "",
}: LenderCatalogTableProps) {
  const { data, isLoading, error } = useLenderCatalog();
  const [searchTerm, setSearchTerm] = useState("");
  const [countryFilter, setCountryFilter] = useState<"all" | "CA" | "US">(
    "all",
  );
  ...
  const filteredProducts = useMemo(() => {
    ...
    return data.products.filter((product) => {
      ...
      const matchesCountry =
        countryFilter === "all" || product.country === countryFilter;
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              {data?.total || 0} products available • {filteredProducts.length}{" "}
              displayed
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="flex gap-2">
            <select
              value={countryFilter}
              onChange={(e) =>
                setCountryFilter(e.target.value as "all" | "CA" | "US")
              }
              ...
              filteredProducts.map((product) => (
                <tr key={product.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    {(product.interest_rate_min !== null ||
                      product.interest_rate_max !== null) && (
                      <div className="text-xs text-gray-500">
                        Rate: {product.interest_rate_min ?? "N/A"} -{" "}
                        {product.interest_rate_max ?? "N/A"}%
                      </div>
                    )}
                  </td>
        ...
        <div className="px-6 py-4 bg-gray-50 border-t">
          <div className="flex justify-between text-sm text-gray-600">
            ...
            <div className="flex gap-4">
              <span>
                CA: {filteredProducts.filter((p) => p.country === "CA").length}
              </span>
              <span>
                US: {filteredProducts.filter((p) => p.country === "US").length}
              </span>
              <span>
                Active: {filteredProducts.filter((p) => p.active).length}
              </span>
            </div>
          </div>
        </div>
