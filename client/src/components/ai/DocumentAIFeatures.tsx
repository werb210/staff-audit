      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="matching" className="flex items-center gap-2">
            ...
            <Badge variant="outline" className="text-xs">Feature 5</Badge>
          </TabsTrigger>
          <TabsTrigger value="summary" className="flex items-center gap-2">
            <FileText className="h-4 w-4" />
            Document Summarizer
            <Badge variant="outline" className="text-xs">Feature 6</Badge>
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="matching" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {!documentMatchingMutation.data && !documentMatchingMutation.isPending && (
                <Alert>
                  <AlertDescription>
                    Click "Match Documents" to have AI analyze uploaded documents and match them 
                    to standard lending requirements.
                  </AlertDescription>
                </Alert>
              )}
                ...
                <div className="space-y-4">
                  {documentMatchingMutation.data.analysis?.matches && (
                    <div className="space-y-3">
                      ...
                      {documentMatchingMutation.data.analysis.matches.map((match: any, index: number) => (
                  ...
                  {documentMatchingMutation.data.analysis?.missingRequirements?.length > 0 && (
                    <div className="space-y-2">
                      <h4 className="font-medium text-red-700">Missing Requirements</h4>
                      <ul className="space-y-1">
                        {documentMatchingMutation.data.analysis.missingRequirements.map((req: string, index: number) => (
                  ...
                  {documentMatchingMutation.data.analysis?.unmatchedDocuments?.length > 0 && (
                    <div className="space-y-2">
                      <h4 className="font-medium text-yellow-700">Unmatched Documents</h4>
                      <ul className="space-y-1">
                        {documentMatchingMutation.data.analysis.unmatchedDocuments.map((doc: string, index: number) => (
              ...
              {documentMatchingMutation.error && (
                <Alert variant="destructive">
                  <AlertDescription>
                    Failed to analyze document matching: {documentMatchingMutation.error.message}
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
        ...
        <TabsContent value="summary" className="space-y-4">
          <Card>
            ...
            <CardContent>
              {!documentSummaryMutation.data && !documentSummaryMutation.isPending && (
                <Alert>
                  <AlertDescription>
                    Generate a comprehensive AI summary of all uploaded documents including 
                    key financial information, business details, and compliance notes.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {documentSummaryMutation.error && (
                <Alert variant="destructive">
                  <AlertDescription>
                    Failed to generate document summary: {documentSummaryMutation.error.message}
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
