...
export default function RiskScoreDisplay({ applicationId, autoLoad = false }: RiskScoreDisplayProps) {
  ...
  const { data: assessmentData, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(`/api/ai/applications/${applicationId}/risk-score`);
      if (!response.ok) {
  ...
  const calculateRiskMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['risk-assessment', applicationId] });
    }
  });
  ...
  const processDocumentsMutation = useMutation({
    mutationFn: async () => {
      const response = await fetch(`/api/ai/applications/${applicationId}/process`, {
        method: 'POST'
      });
      if (!response.ok) {
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-muted-foreground">Calculating AI risk assessment...</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="text-center">
            <div className={`text-6xl font-bold ${getScoreColor(assessment.riskScore)} mb-2`}>
              {assessment.riskScore}
            </div>
            <Badge className={`${getRiskLevelColor(assessment.riskLevel)} mb-4`}>
              {getRiskIcon(assessment.riskLevel)}
              <span className="ml-1 capitalize">{assessment.riskLevel} Risk</span>
            </Badge>
            <Progress 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <p className="text-sm font-medium text-muted-foreground mb-2">Primary Risk Factors</p>
              ...
              <div className="space-y-1">
                ...
                {assessment.recommendations.length > 2 && (
                  <button
                    ...
                    className="text-sm text-blue-600 hover:underline"
                  >
                    +{assessment.recommendations.length - 2} more recommendations
                  </button>
                )}
              </div>
            </div>
