...
export const uploadDoc = (id: string, file: File, category: string) => {
  ...
  return api(`/api/apps/${id}/documents`, { method: "POST", body: fd, headers: {} });
};

export const downloadAll = (id: string) => window.location.assign(`/api/apps/${id}/documents.zip`);

export const lookupPhone = (phone: string) => 
  api(`/api/lookup/phone?phone=${encodeURIComponent(phone)}`);

export const getMatches = (id: string) => 
  api(`/api/apps/${id}/lender/match`);

export const sendToLender = (id: string, lenderId: string, channel: "o365" | "api") =>
  api(`/api/apps/${id}/lender/send`, { 
    method: "POST", 
    body: JSON.stringify({ lenderId, method: channel === "o365" ? "email" : "api" })
  });