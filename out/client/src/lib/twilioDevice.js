...
export async function getTwilioDevice() {
    if (window.twilioDevice)
        return window.twilioDevice;
    if (window.twilioInit)
    ...
    window.twilioInit = (async () => {
        ...
        device.on("tokenWillExpire", async () => {
            ...
            if (r2.ok) {
                ...
                console.error("[TwilioDevice] Failed to refresh token", r2.status);
            }
        });
        try {
            ...
            if (outs[0])
                Device.audio.speakerDevices.set(outs[0].deviceId);
        }
        catch (err) {
            console.warn("[TwilioDevice] Could not enumerate audio devices", err);
        }
