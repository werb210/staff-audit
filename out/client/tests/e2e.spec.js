...
const MANIFEST = JSON.parse(fs.readFileSync(path.join(__dirname, 'tabs.manifest.json'), 'utf8'));
...
async function attachGuards(page) {
    ...
    page.on('console', msg => {
        if (msg.type() === 'error' || msg.type() === 'warning') {
            ...
            if (!ignoreConsole.some(rx => rx.test(t))) {
                ...
                page.evaluate((m) => window.__TEST_ERRORS__ = (window.__TEST_ERRORS__ || []).concat(m), t);
            }
        }
    });
    ...
    await page.addInitScript(() => {
        ...
        window.addEventListener('dialer:open', (e) => {
            window.__TEST_EVENTS__.push({ type: 'dialer:open', detail: e?.detail || null });
        }, { once: false, passive: true });
    });
}
async function assertNoConsoleErrors(page) {
    const errs = await page.evaluate(() => window.__TEST_ERRORS__ || []);
    expect.soft(errs, 'Unexpected console errors').toEqual([]);
}
function label(tab, feature) {
    return `${tab} › ${feature}`;
}
                ...
                try {
                    ...
                    if (f.action === 'click') {
                        ...
                        for (const step of f.form || []) {
                            ...
                            if (step.select)
                                await page.locator(step.select).selectOption(step.value);
                            if (step.check)
                                await page.locator(step.check).setChecked(true);
                        }
                    ...
                    if (f.expect?.event === 'dialer:open') {
                        const seen = await page.waitForFunction(() => window.__TEST_EVENTS__?.some((e) => e.type === 'dialer:open'), null, { timeout: 5000 }).catch(() => false);
