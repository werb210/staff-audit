...
async function pushToClientBase(baseUrl, payload) {
    ...
    for (const lender of payload.lenders) {
        // upsert lender
        await axios.post(`${baseUrl}/api/admin/lenders/upsert`, { name: lender.name, is_active: true }, { headers, timeout: 20000 });
        // upsert products in batch
        await axios.post(`${baseUrl}/api/admin/lender-products/upsertMany`, {
...
async function run() {
    ...
    console.log(`📤 Pushing ${payload.totalLenders} lenders & ${payload.totalProducts} products to ${apps.length} client app(s)...`);
    for (const baseUrl of apps) {
        ...
        console.log(`   ✅ ${baseUrl} – mode: ${res.mode} (totalLenders=${payload.totalLenders}, totalProducts=${payload.totalProducts})`);
    }
