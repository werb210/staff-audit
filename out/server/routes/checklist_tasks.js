...
r.post("/applications/:id/tasks/from-requirements", async (req, res) => {
    const { id } = req.params;
...
async function createPeriodSpecificTasks(app, reqs) {
    // Fetch coverage
    const cov = await fetch(`http://localhost:5000/api/docs/coverage?applicationId=${app.id}`)
        .then(r => r.json())
        .catch(() => ({ coverage: {} }));
    const makeTask = async (title, tags) => {
        const dueAt = new Date();
