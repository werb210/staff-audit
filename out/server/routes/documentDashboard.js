...
router.get('/enhanced-status', async (req, res) => {
    try {
        ...
        const documentStats = await db.select({
            ...
            withChecksums: sql `COUNT(CASE WHEN checksum IS NOT NULL AND checksum != '' THEN 1 END)`,
            withStorageKeys: sql `COUNT(CASE WHEN storage_key IS NOT NULL AND storage_key != '' THEN 1 END)`,
            totalSize: sql `COALESCE(SUM(size), 0)`,
            avgSize: sql `COALESCE(AVG(size), 0)`,
            recentUploads: sql `COUNT(CASE WHEN createdAt > NOW() - INTERVAL '24 hours' THEN 1 END)`
        }).from(documents);
        ...
        const recentDocuments = await db.select({
            ...
            hasChecksum: sql `checksum IS NOT NULL AND checksum != ''`,
            hasStorageBackup: sql `storage_key IS NOT NULL AND storage_key != ''`,
            enhancementLevel: sql `
        CASE 
