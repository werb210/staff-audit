...
router.get('/contacts', requireAuth, async (req, res) => {
    try {
        ...
        const filteredContacts = searchQuery
            ? mockContacts.filter(contact => contact.firstName.toLowerCase().includes(searchQuery.toLowerCase()) ||
...
router.post('/contacts/auto-create', async (req, res) => {
    try {
...
router.get('/companies', requireAuth, async (req, res) => {
    ...
    `);
        ...
        if (companies.length === 0) {
            companies.push({ id: '1', name: 'Acme Corp', contactCount: 0, industry: 'Technology' }, { id: '2', name: 'Tech Solutions', contactCount: 0, industry: 'Software' }, { id: '3', name: 'Global Industries', contactCount: 0, industry: 'Manufacturing' });
        }
        res.json(companies);
    }
    catch (error) {
