...
router.post("/:templateId/versions/:versionId/submit", async (req, res) => {
    const { versionId } = req.params;
    await q(`UPDATE comm_template_versions SET status='pending' WHERE id = $1 AND status='draft'`, [versionId]);
    res.json({ ok: true });
});
router.post("/:templateId/versions/:versionId/approve", async (req, res) => {
    const { versionId } = req.params;
    await q(`
...
router.post("/:templateId/versions/:versionId/reject", async (req, res) => {
    const { versionId } = req.params;
