 ...
 */
router.get('/o365/start', async (req, res) => {
    try {
 ...
 */
router.get('/o365/callback', async (req, res) => {
    try {
 ...
 */
router.get('/googleads/start', async (req, res) => {
    try {
 ...
 */
router.get('/googleads/callback', async (req, res) => {
    try {
        const { code, state } = req.query;
        if (!code) {
            return res.redirect('/staff/settings/integrations?error=oauth_failed');
...
router.post('/sync/calendar', async (req, res) => {
    try {
        ...
        const [token] = await db
            .select()
            .from(oauthTokens)
            .where(and(eq(oauthTokens.userId, userId), eq(oauthTokens.provider, 'office365'), eq(oauthTokens.isActive, true)));
        if (!token) {
            return res.status(400).json({
                success: false,
