...
import { aiComposeEmail, aiComposeSMS } from "../services/ai";
import { EmailComposeSchema, SMSComposeSchema } from "../lib/validate";
const r = Router();
...
function fail(res, code, detail, status = 422) {
    res.status(status).json({ ok: false, error: code, detail });
}
async function writeTimeline(appId, type, payload) {
    try {
...
r.post("/report-issue", async (req, res) => {
    try {
...
r.post("/request-human", async (req, res) => {
    try {
...
r.get("/docs/scan", async (req, res) => {
    try {
        const applicationId = String(req.query.applicationId || "");
        if (!applicationId)
            return fail(res, "missing_applicationId");
        ...
        for (const d of docs) {
            ...
            let key = "";
            if (t.includes("bank"))
                key = "Bank Statements (last 6 months)";
            else if (t.includes("t2"))
                key = "T2 Corporate Return (latest)";
            else if (t.includes("noa"))
                key = "NOA (latest)";
            else if (t.includes("aging") && t.includes("ar"))
                key = "AR Aging";
            else if (t.includes("aging") && t.includes("ap"))
                key = "AP Aging";
            else if (t.includes("financial") || t.includes("pl") || t.includes("balance"))
                key = "Financials (YTD + last FY)";
            if (key)
                present.add(key);
            ...
            if (!q.ok)
                quality.push({ ...q, reason: dpi < 150 ? "low_dpi" : "unknown" });
        }
...
r.post("/docs/validate", async (req, res) => {
    try {
...
r.post("/financials/score", async (req, res) => {
    try {
        const { applicationId } = req.body || {};
        if (!applicationId)
            return fail(res, "missing_applicationId");
...
r.post("/approval-prob", async (req, res) => {
    try {
...
r.post("/ops/priority", async (req, res) => {
    try {
        const { applicationId } = req.body || {};
        if (!applicationId)
            return fail(res, "missing_applicationId");
...
r.post("/credit-summary/generate", async (req, res) => {
    try {
        const { applicationId } = req.body || {};
        if (!applicationId)
            return fail(res, "missing_applicationId");
...
// --- 8) Benchmarks (NAICS) -------------------------------------------
r.get("/benchmarks/:naics", async (req, res) => {
...
r.post("/compose/email", async (req, res) => {
    try {
...
r.post("/compose/sms", async (req, res) => {
    try {
...
r.post("/compliance/screen", async (req, res) => {
...
r.post("/lender-qa", async (req, res) => {
