...
async function fetchTwilioMedia(url) {
    ...
    if (!res.ok)
        throw new Error("twilio_media_fetch_failed");
    return Buffer.from(await res.arrayBuffer());
}
// OpenAI transcription
async function transcribeAudioMp3(buf) {
    ...
    if (!res.ok)
        throw new Error("openai_transcribe_failed: " + JSON.stringify(j));
    return j.text;
}
...
Return a JSON object {summary, disposition, nextActions:[{title,dueDays}], tags:[…]}.`;
    ...
    const j = await res.json();
    if (!res.ok)
        throw new Error("openai_summary_failed: " + JSON.stringify(j));
    return JSON.parse(j.choices?.[0]?.message?.content || "{}");
}
r.post("/voice/transcribe", async (req, res) => {
    try {
