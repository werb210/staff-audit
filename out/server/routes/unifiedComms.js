...
router.get("/", requireAnyRole(["admin", "manager", "agent", "marketing"]), async (req, res) => {
...
router.get("/contacts/search", requireAnyRole(["admin", "manager", "agent", "marketing"]), async (req, res) => {
    const { phone, unified } = req.query;
    if (!phone) {
        return res.status(400).json({ error: "phone_required" });
    }
    try {
        const contacts = await findContactAcrossTenants(phone, req.app.locals.db);
        res.json({ contacts });
    }
    catch (error) {
