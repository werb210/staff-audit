 ...
 */
router.post('/analyze/:documentId', simpleAuth, async (req, res) => {
 ...
 */
router.post('/cross-application/:applicationId', simpleAuth, async (req, res) => {
    const startTime = Date.now();
    try {
 ...
 */
router.get('/results/:documentId', simpleAuth, async (req, res) => {
    try {
        const documentId = parseInt(req.params.documentId);
        if (isNaN(documentId)) {
            return res.status(400).json({
 ...
 */
router.get('/flagged', simpleAuth, async (req, res) => {
    try {
