...
router.patch('/lender-products/:id', requireLenderAuth, async (req, res) => {
    try {
        ...
        const existingProduct = await db.execute(sql `SELECT id, product_name, lender_name FROM lender_products WHERE id = ${productId}`);
...
router.post('/lender-products', requireLenderAuth, async (req, res) => {
    try {
...
router.delete('/lender-products/:id', requireLenderAuth, async (req, res) => {
    try {
        const productId = req.params.id;
        const lenderUser = req.lenderUser;
        ...
        const deleteResult = await db.execute(sql `DELETE FROM lender_products WHERE id = ${productId} RETURNING id, product_name`);
