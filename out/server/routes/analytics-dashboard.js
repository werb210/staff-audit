...
import { db } from '../db';
import { applications, documents, smsMessages, callLogs, emailMessages } from '../../shared/schema';
...
router.get('/dashboard-summary', developmentAuth, async (req, res) => {
    try {
...
router.get('/pipeline-activity', developmentAuth, async (req, res) => {
    try {
        ...
        const allApps = await db
            ...
            .from(applications)
            .where(and(gte(applications.createdAt, start), lte(applications.createdAt, end)));
...
router.get('/document-status', developmentAuth, async (req, res) => {
    try {
...
router.get('/conversion', developmentAuth, async (req, res) => {
    try {
        const { startDate, endDate } = req.query;
...
router.get('/communication-stats', developmentAuth, async (req, res) => {
    try {
