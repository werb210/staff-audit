...
router.post("/:id/tasks", async (req, res) => {
    ...
    if (!job_key || !(job_key in Jobs))
        return res.status(400).json({ error: "invalid job_key" });
...
router.post("/tasks/:taskId/run", async (req, res) => {
    ...
    if (!fn)
        return res.status(400).json({ error: "job not registered" });
...
router.post("/dismiss/:releaseId", async (req, res) => {
    const uid = req.user?.id || null;
    if (!uid)
        return res.status(401).json({ error: "login required to dismiss" });
