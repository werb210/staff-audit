...
router.get('/health', async (req, res) => {
    ...
    `;
        ...
        const previewStats = previewLogResult.rows.reduce((acc, row) => {
            acc[row.status] = {
        ...
        const totalPreviews = Object.values(previewStats).reduce((sum, stat) => sum + stat.count, 0);
...
router.get('/preview-logs', async (req, res) => {
    try {
