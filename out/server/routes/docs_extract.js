 ...
 */
r.post("/ai/docs/extract", async (req, res) => {
    ...
    let extracted = { docType, fields: {}, arrays: {} };
    let pii = {};
    if (docType === "tax_return") {
        extracted.fields.business_legal_name = payload.business_legal_name || payload.legal_name || null;
        // tax id is PII
        if (payload.taxpayer_id)
            pii.taxpayer_id = String(payload.taxpayer_id);
        const years = Array.isArray(payload.years) ? payload.years : [];
        extracted.arrays.years = years.map((y) => ({
    ...
    if (docType === "bank_statement") {
        ...
        if (payload.account_number)
            pii.account_number = String(payload.account_number);
        const months = Array.isArray(payload.months) ? payload.months : [];
        extracted.arrays.months = months.map((m) => ({
        ...
        extracted.arrays.transactions = tx.slice(0, 5000).map((t) => ({
 ...
 */
r.get("/docs/coverage", async (req, res) => {
    ...
    const cov = {
        ...
        tax_return: new Set(),
        financial_statements: new Set()
    };
    rows.forEach(r => {
        const ex = r.extracted_json || {};
        if (r.docType === "bank_statement") {
            (ex.arrays?.months || []).forEach((m) => {
                const ym = String(m.month || "").slice(0, 7);
                if (ym)
                    cov.bank_statement.add(ym);
            });
        }
        if (r.docType === "tax_return") {
            (ex.arrays?.years || []).forEach((y) => cov.tax_return.add(String(y.year).slice(0, 4)));
        }
        if (r.docType === "financial_statements") {
            (ex.arrays?.years || []).forEach((y) => cov.financial_statements.add(String(y.year).slice(0, 4)));
        }
    });
    const ret = Object.fromEntries(Object.entries(cov).map(([k, v]) => [k, Array.from(v).sort().reverse()]));
