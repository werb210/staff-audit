...
router.get('/pipeline-activity', async (req, res) => {
    try {
        ...
        const applicationsByDate = await db
            ...
            .from(applications)
            .where(gte(applications.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)))
...
router.get('/document-status', async (req, res) => {
    try {
        ...
        const processingTimes = await db
            .select({
            type: documents.type,
            avgProcessingDays: sql `AVG(EXTRACT(EPOCH FROM (${documents.updatedAt} - ${documents.createdAt})) / 86400)`
        })
            .from(documents)
            .where(and(eq(documents.status, 'accepted'), ...(conditions.length > 0 ? conditions : [])))
            .groupBy(documents.type);
...
router.get('/conversion', async (req, res) => {
    try {
        ...
        const funnelData = await db
            ...
            status: applications.status,
            count: count(),
            avgLoanAmount: sql `AVG(${applications.loanAmount})`
        })
            .from(applications)
            .where(conditions.length > 0 ? and(...conditions) : undefined)
            .groupBy(applications.status);
        ...
        const conversionBySource = await db
            ...
            leadSource: applications.leadSource,
            total: count(),
            approved: sql `COUNT(CASE WHEN ${applications.stage} = 'New' THEN 1 END)`,
            rejected: sql `COUNT(CASE WHEN ${applications.status} = 'draft' THEN 1 END)`
        })
            .from(applications)
            .where(conditions.length > 0 ? and(...conditions) : undefined)
            .groupBy(applications.leadSource);
        ...
        const timeToCompletion = await db
            .select({
            avgDays: sql `AVG(EXTRACT(EPOCH FROM (${applications.updatedAt} - ${applications.createdAt})) / 86400)`,
            stage: applications.stage
        })
            .from(applications)
            .where(and(sql `${applications.stage} = 'New'`, ...(conditions.length > 0 ? conditions : [])))
...
router.get('/dashboard-summary', async (req, res) => {
    try {
        ...
        const [totalApplications, activeApplications, completedApplications, pendingDocuments, recentActivity] = await Promise.all([
            ...
            db.select({ count: count() })
                .from(applications)
                .where(and(gte(applications.createdAt, thirtyDaysAgo), sql `${applications.stage} = 'New'`)),
            // Pending documents
            db.select({ count: count() })
                .from(documents)
...
router.get('/analytics/ga4/pageviews', async (req, res) => {
    try {
...
router.get('/analytics/ga4/users', async (req, res) => {
    try {
