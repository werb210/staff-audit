...
r.get("/applications/:id/documents.zip", async (req, res) => {
    const appId = String(req.params.id);
    const { rows } = await pool.query(`SELECT id, name as filename, storage_key as object_key, file_type as content_type, sha256, status
       FROM documents
      WHERE applicationId = $1
        AND status IN ('pending','accepted')
      ORDER BY createdAt ASC, id ASC`, [appId]);
    res.status(200);
