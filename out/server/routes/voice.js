...
function requireEnv(name) {
    ...
    if (!v)
        throw new Error(`[voice] Missing env: ${name}`);
    return v;
}
const clients = new Map();
function send(ev, res) {
    res.write(`data: ${JSON.stringify(ev)}\n\n`);
}
function broadcast(tenant, ev) {
...
// ----- SSE endpoint -----
r.get("/sse", (req, res) => {
    const tenant = (String(req.query.tenant || "bf").toLowerCase() === "slf" ? "slf" : "bf");
...
r.get("/token", async (req, res) => {
    try {
        ...
        const identity = String(req.query.identity || req?.user?.id || "staff-anon");
...
r.post("/simulate", (req, res) => {
    const body = req.body || {};
    const tenant = (String(body.tenant || "bf").toLowerCase() === "slf" ? "slf" : "bf");
...
r.post("/status-callback", (req, res) => {
...
r.post("/call", async (req, res) => {
    try {
        ...
        if (!to)
            return res.status(400).json({ ok: false, error: "Missing 'to' number" });
        const call = await client.calls.create({
            to,
            from: callerId,
