...
function signAccessToken(user, amr) {
    return jwt.sign({ id: user.id, email: user.email, roles: user.roles || [], amr }, process.env.JWT_SECRET, { expiresIn: ACCESS_TTL });
}
...
router.post('/auth/refresh', async (req, res) => {
    ...
    if (!refresh)
        return res.status(400).json({ error: 'missing refresh' });
    try {
        const p = jwt.verify(refresh, process.env.JWT_SECRET);
...
router.get('/auth/whoami', authJwt, (req, res) => {
