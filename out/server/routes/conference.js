...
const router = Router();
const { PUBLIC_BASE_URL, TWILIO_AUTH_TOKEN, CONF_ROOM_NAME = 'bf-bridge-1', CONF_MOD_PIN = '5463', CONF_GUEST_PIN = '1234', CONF_RECORDING = 'true', TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN: T_AUTH } = process.env;
function verify(req, path) {
...
router.post('/enter', (req, res) => {
    if (!verify(req, '/api/conference/enter'))
...
router.post('/role', (req, res) => {
    if (!verify(req, '/api/conference/role'))
        return res.status(403).send('Forbidden');
    ...
    if (CONF_RECORDING === 'true')
        baseOpts.record = 'record-from-start';
...
router.post('/participants', async (req, res) => {
    const { roomName, fromE164, list } = req.body;
    if (!roomName || !fromE164 || !Array.isArray(list))
