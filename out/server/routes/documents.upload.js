...
r.post("/api/applications/:id/documents/upload", requireStaff, upload.single("file"), async (req, res) => {
    try {
        ...
        if (!documentType)
            return res.status(400).json({ ok: false, error: "NO_DOCUMENT_TYPE" });
        ...
        const insert = await req.app.locals.db.query(`insert into documents (applicationId, name, size, document_type, status, file_key, uploaded_by)
       values ($1,$2,$3,$4,'pending',$5,$6) returning id`, [appId, f.originalname, f.size, documentType, key, "staff-ui"]);
