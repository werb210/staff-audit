...
router.post('/process/:documentId', async (req, res) => {
    ...
    `);
        ...
        const result = await processDocument(document.id, document.applicationId, document.name, document.document_type);
        res.json({
            success: true,
 ...
 */
router.post('/process-application/:applicationId', async (req, res) => {
    try {
