...
export function mountDocumentRoutes(app, basePath = "/api") {
    const pathFor = (suffix) => joinPaths(basePath, suffix);
    // POST /api/documents/presign
    app.post(pathFor("/documents/presign"), async (req, res) => {
        ...
        const confirmToken = jwt.sign({ applicationId, key, contentType, size: size || 0, filename }, DOCS_JWT_SECRET, { expiresIn: "15m" });
        if (isProd && USE_AWS) {
            try {
    ...
    if (!isProd) {
        import("fs/promises").then(({ mkdir, writeFile }) => {
            app.put(pathFor("/dev/mock-upload/:key"), express.raw({ type: "*/*", limit: "25mb" }), async (req, res) => {
                try {
    ...
    app.post(pathFor("/documents/complete"), async (req, res) => {
        ...
        let payload;
        try {
            payload = jwt.verify(token, DOCS_JWT_SECRET);
        }
        catch {
    ...
    app.get(pathFor("/documents/list"), async (req, res) => {
