...
router.get("/", async (req, res) => {
    try {
        ...
        if (country) {
            whereConditions.push(eq(lenderProducts.country, country));
        }
        if (productCategory) {
            whereConditions.push(eq(lenderProducts.category, productCategory));
        }
...
router.post("/", async (req, res) => {
    try {
        ...
        const [newProduct] = await db.insert(lenderProducts).values({
            ...
            category: validatedData.category, // Type assertion for enum
