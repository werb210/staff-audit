...
router.post("/applications", express.json(), async (req, res) => {
    try {
        ...
        const { firstName, lastName, email, phone, businessName, businessType, amountRequested, productType, purpose } = req.body;
...
router.post("/applications/:id/documents", upload.single('document'), async (req, res) => {
    try {
        ...
        const result = await documentsService.uploadDocument(applicationId, req.file.buffer, {
            fileName: req.file.originalname,
            documentType: req.body.documentType,
            fileSize: req.file.size,
            mimeType: req.file.mimetype
