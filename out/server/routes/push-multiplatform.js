...
if (vapidKeys.publicKey && vapidKeys.privateKey) {
    webpush.setVapidDetails('mailto:admin@boreal.financial', vapidKeys.publicKey, vapidKeys.privateKey);
...
router.post('/register', async (req, res) => {
    try {
        ...
        const existingDevice = await db.select()
            ...
            .where(and(eq(deviceRegistrations.userId, userId), eq(deviceRegistrations.platform, registrationData.platform), eq(deviceRegistrations.token, registrationData.token)))
            .limit(1);
        let deviceReg;
...
router.post('/unregister', async (req, res) => {
    try {
        ...
        const result = await db.update(deviceRegistrations)
            ...
            .where(and(eq(deviceRegistrations.userId, userId), eq(deviceRegistrations.platform, platform), eq(deviceRegistrations.token, token)))
            .returning();
        if (result.length === 0) {
...
router.post('/broadcast', async (req, res) => {
    try {
        ...
        const { title, body, silo = 'bf', userIds = [], url = '/portal', data = {}, platforms = ['webpush', 'fcm', 'apns', 'windows'] } = req.body;
        if (!title || !body) {
            return res.status(400).json({
                success: false,
        ...
        if (userIds.length > 0) {
            ...
            targetDevices = await db.select({
                ...
                .where(and(inArray(deviceRegistrations.userId, userIds), eq(deviceRegistrations.silo, silo), eq(deviceRegistrations.isActive, true), inArray(deviceRegistrations.platform, platforms)));
        }
        else {
            ...
            targetDevices = await db.select({
                ...
                .from(deviceRegistrations)
                .innerJoin(users, eq(users.id, deviceRegistrations.userId))
                .where(and(eq(deviceRegistrations.silo, silo), eq(deviceRegistrations.isActive, true), inArray(deviceRegistrations.platform, platforms), sql `${users.role} IN ('staff', 'admin')`));
        }
...
async function sendWindowsNotification(token, payload) {
    try {
