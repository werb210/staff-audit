...
router.post('/trigger-detection', async (req, res) => {
    try {
        ...
        for (const alert of result.alerts) {
            await recoveryLogger.logMissingDetected(alert.documentId, alert.fileName, alert.applicationId, alert.businessName, alert.expectedPath);
        }
        res.json({
            success: true,
...
router.get('/application/:id/missing', async (req, res) => {
    try {
...
router.post('/log-recovery', async (req, res) => {
    try {
        ...
        if (status === 'RECOVERED') {
            await recoveryLogger.logRecoverySuccess(documentId, fileName, applicationId, businessName, req.body.originalPath || 'Unknown', req.body.newPath || 'Unknown', req.body.fileSize || 0, req.body.uploadedBy || 'staff');
        }
        else if (status === 'RECOVERY_INITIATED') {
            await recoveryLogger.logRecoveryInitiated(documentId, fileName, applicationId, businessName);
        }
        else if (status === 'RECOVERY_FAILED') {
            await recoveryLogger.logRecoveryFailure(documentId, fileName, applicationId, businessName, details || 'Unknown error');
        }
        res.json({
            success: true,
            automation: 'AUTO-RECOVERY 2',
...
router.get('/validate/application/:id', async (req, res) => {
    try {
        const { id: applicationId } = req.params;
...
router.post('/run-complete-recovery/:applicationId', async (req, res) => {
    try {
        ...
        for (const doc of missingDocs) {
            await recoveryLogger.logMissingDetected(doc.documentId, doc.fileName, doc.applicationId, doc.businessName, doc.expectedPath);
        }
...
router.post('/recover-document/:documentId', async (req, res) => {
    try {
        const { documentId } = req.params;
...
router.post('/recover-batch', async (req, res) => {
    try {
...
router.post('/recover-application/:applicationId', async (req, res) => {
    try {
        const { applicationId } = req.params;
