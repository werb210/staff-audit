...
r.use(requireAuth);
// Get all budgets
r.get("/marketing/budgets", async (req, res) => {
    try {
...
r.post("/marketing/budgets", async (req, res) => {
    try {
...
r.get("/marketing/budget/forecast", async (req, res) => {
    try {
      ...
      GROUP BY source
    `).catch(() => ({ rows: [] }));
        const spent = Object.fromEntries(costs.map((c) => [c.source, Number(c.cents || 0)]));
