...
export function mountVerifyRoutes(app) {
    app.post("/api/auth/verify/start", limiter, requireAuth, async (req, res) => {
        try {
    ...
    app.post("/api/auth/verify/check", limiter, requireAuth, async (req, res) => {
        try {
    ...
    app.get("/api/auth/verify/status", requireAuth, (req, res) => {
        res.json({
            mfaRequired: !!req.user?.mfaRequired,
...
export function requireAdminWithMfa(req, res, next) {
    if (!req.user)
        return res.sendStatus(401);
