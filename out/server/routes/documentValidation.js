...
router.get('/db-compare', async (req, res) => {
    try {
        ...
        const dbEnumResult = await db.execute(sql `SELECT unnest(enum_range(NULL::document_type)) AS document_type_value ORDER BY document_type_value`);
        const dbTypes = dbEnumResult.rows.map((row) => row.document_type_value).sort();
