...
const store = {};
r.get("/marketing/linkedin/oauth/start", requireAuth, (req, res) => {
...
r.get("/marketing/linkedin/oauth/callback", async (req, res) => {
...
r.get("/marketing/linkedin/pages", requireAuth, async (_req, res) => {
    ...
    const orgs = await (await fetch(`${apiBase()}/organizationalEntityAcls?q=roleAssignee&role=ADMINISTRATOR&state=APPROVED&count=50&start=0&projection=(elements*(organizationalTarget~(id,localizedName)))`, { headers: { Authorization: `Bearer ${tk}` } })).json();
    res.json({ ok: true, actor: me, pages: (orgs.elements || []).map((e) => e["organizationalTarget~"]) });
});
// Create a page post
r.post("/marketing/linkedin/page-post", requireAuth, async (req, res) => {
    const tk = store.linkedin?.access_token;
    if (!tk)
        return res.status(401).json({ ok: false, error: "not_connected" });
...
r.get("/marketing/linkedin/page-analytics", requireAuth, async (req, res) => {
    const tk = store.linkedin?.access_token;
    if (!tk)
        return res.status(401).json({ ok: false, error: "not_connected" });
