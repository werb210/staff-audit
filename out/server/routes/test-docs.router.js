...
const b64toBuf = (b64) => Buffer.from(b64.replace(/\s+/g, ''), 'base64');
...
const router = Router();
let LAST_DIR = "";
function traceBase(req) { const tid = (req.header("X-Trace-Id") || (req.body?._trace?.id) || ("trace-" + Date.now())); const base = path.join(process.cwd(), "reports", "staff-docs-e2e-live", tid); ensureDir(base); LAST_DIR = base; return { tid, base }; }
function ocrText(buf, mimetype) {
    try {
        ...
        if (mimetype === "application/pdf" && fs.existsSync("/usr/bin/pdftotext")) {
            ...
            const txt = fs.readFileSync(tmpOut, "utf8");
            fs.unlinkSync(tmpIn);
...
function analyzeBankCsv(txt) {
    if (!txt || !/date,.*amount/i.test(txt))
        return null;
    ...
    for (const ln of lines) {
        ...
        if (cols.length < 4)
            continue;
        const amt = parseFloat(cols[2]);
        const bal = parseFloat(cols[3]);
        if (!isNaN(amt)) {
            if (amt >= 0)
                credits += amt;
            else
...
router.post("/upload", (req, res, next) => {
    ...
    if (multer && /multipart\/form-data/i.test(ct)) {
        ...
        return m.single("file")(req, res, () => {
            try {
                const { tid, base } = traceBase(req);
                const uploads = path.join(base, "docs");
                ensureDir(uploads);
                ...
                const ocrDir = path.join(base, "ocr");
                ensureDir(ocrDir);
                const text = ocrText(file?.buffer || Buffer.alloc(0), file?.mimetype || "application/octet-stream");
                let ocrPath;
                if (text) {
                    ocrPath = path.join(ocrDir, id + ".txt");
                    fs.writeFileSync(ocrPath, text);
                }
                const rec = { id, filename: (file?.originalname || "unknown"), mimetype: (file?.mimetype || "application/octet-stream"), category: (req.body?.category || undefined), size: (file?.size || 0), paths: { bin: binPath, ocr: ocrPath } };
                const idx = path.join(base, "index.json");
                let list = [];
                try {
                    list = JSON.parse(fs.readFileSync(idx, 'utf8'));
                }
                catch { }
                list.push(rec);
    ...
    try {
        ...
        const rec = { id, filename: (body.filename || "unknown"), mimetype: (body.mimetype || "application/octet-stream"), category: body.category, size: buf.length, paths: { bin, ocr: ocrPath } };
        const idx = path.join(base, "index.json");
        let list = [];
        try {
            list = JSON.parse(fs.readFileSync(idx, 'utf8'));
...
router.post("/submit", (req, res) => {
    ...
    let analysis = { docs_total: picked.length, bank: null };
    ...
    const keysOf = (o) => Object.keys(flatten(o));
    const coverage = {
