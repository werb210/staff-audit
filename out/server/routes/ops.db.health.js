...
async function tryFixPermissions(tables) {
...
r.get("/ops/db/self-test", async (_req, res) => {
    const report = { env: {}, connectivity: {}, search_path: {}, tables: [], rls: {}, advice: [] };
    // ENV
    ...
    try {
        ...
        report.connectivity.ok = false;
        report.connectivity.error = e.message;
    ...
    try {
        ...
        report.search_path.error = e.message;
    }
    // Table counts
    ...
    for (const t of report.tables) {
        if (!t.ok && /does not exist/i.test(t.error || ""))
            report.advice.push(`Table missing: ${t.table} — run migrations.`);
        if (!t.ok && /permission denied/i.test(t.error || ""))
            report.advice.push(`Permissions issue on ${t.table} — GRANT privileges to your app role.`);
    }
    if (!String(report.search_path.value || "").includes("public"))
        report.advice.push("search_path does not include 'public' — we force it per-connection, but set it at DB level for consistency.");
    res.json(report);
});
r.post("/ops/db/auto-fix", async (_req, res) => {
    ...
    try {
        ...
        results.search_path = "ok";
    }
    catch (e) {
        results.search_path = e.message;
    }
