...
router.post('/voicemail-redirect', async (req, res) => {
...
router.post('/voicemail-notify', async (req, res) => {
...
router.get('/voicemails', async (req, res) => {
    ...
    try {
        ...
        if (silo) {
            query = query.where(eq(voicemails.silo, silo));
        }
        if (unreadOnly === 'true') {
...
router.post('/voicemails/:id/read', async (req, res) => {
    const { id } = req.params;
    try {
        await db
            .update(voicemails)
...
// Delete voicemail
router.delete('/voicemails/:id', async (req, res) => {
    const { id } = req.params;
    try {
        await db
            .delete(voicemails)
