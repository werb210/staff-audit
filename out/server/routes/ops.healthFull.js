...
async function tryCatch(name, fn, fatal = false) {
    try {
        const detail = await fn();
        return pass(name, detail);
    }
    catch (e) {
        return fatal ? fail(name, e?.message || String(e)) : warn(name, e?.message || String(e));
    }
}
