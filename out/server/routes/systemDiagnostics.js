...
import { testS3BucketAccess, testS3Upload, testS3PreSignedUrl, runComprehensiveS3Diagnostic } from '../utils/s3Diagnostic';
const router = Router();
/**
 * GET /api/system/test-s3-basic
 * Basic S3 bucket access test
 */
router.get('/test-s3-basic', async (req, res) => {
    try {
 ...
 */
router.get('/test-s3-presigned', async (req, res) => {
    try {
        const bucket = process.env.AWS_S3_BUCKET_NAME || 'boreal-documents';
 ...
 */
router.get('/test-s3-comprehensive', async (req, res) => {
    try {
        const bucket = process.env.AWS_S3_BUCKET_NAME || 'boreal-documents';
