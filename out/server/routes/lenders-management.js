...
router.get('/', async (req, res) => {
    try {
        ...
        if (search && typeof search === 'string') {
            conditions.push(or(like(lenders.name, `%${search}%`), like(lenders.contactName, `%${search}%`), like(lenders.contactEmail, `%${search}%`), like(lenders.companyBio, `%${search}%`)));
        }
        if (active !== undefined) {
    ...
    try {
        ...
        if (updates.name && updates.name !== existingLender.name) {
            const duplicateLender = await db
                .select()
                .from(lenders)
                .where(and(eq(lenders.name, updates.name), eq(lenders.id, id)));
