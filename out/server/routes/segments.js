...
router.post('/', limiter, async (req, res) => {
    ...
    if (!body?.name || !body?.filter)
        return res.status(400).json({ ok: false, reason: 'missing_name_or_filter' });
...
router.get('/:id/run', limiter, async (req, res) => {
    const { id } = req.params;
    const seg = await segmentsDb.findById(id);
    if (!seg)
        return res.status(404).json({ ok: false, reason: 'segment_not_found' });
    try {
    ...
    `);
        ...
        if (seg.filter.state) {
            filtered = filtered.filter((c) => c.status === seg.filter.state);
        }
