...
export function lendersRouter() {
    const r = makeRouter();
    // LIST (filters: active, country, q; pagination)
    r.get("/lenders", async (req, res) => {
        const { active, country, q, limit = "100", offset = "0" } = req.query;
        const where = [];
        const args = [];
        if (active === "true") {
        ...
        if (q) {
            ...
            where.push(`(lower(name) LIKE $${args.length} OR lower(legal_name) LIKE $${args.length})`);
        }
    ...
    // READ
    r.get("/lenders/:lenderId", async (req, res) => {
        const client = await pool.connect();
        try {
    ...
    // CREATE (admin)
    r.post("/lenders", requireAdminToken, async (req, res) => {
        const { id, name, legal_name, slug, website, contact_email, contact_phone, country, is_active = true } = req.body || {};
        if (!id || !name)
            return res.status(400).json({ ok: false, error: "id_and_name_required" });
        const client = await pool.connect();
        try {
    ...
    r.patch("/lenders/:lenderId", requireAdminToken, async (req, res) => {
    ...
    r.delete("/lenders/:lenderId", requireAdminToken, async (req, res) => {
        const client = await pool.connect();
        try {
    ...
    r.put("/lenders/:lenderId/self", requireLenderAuth, async (req, res) => {
