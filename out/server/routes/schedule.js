...
r.post("/schedule/links", async (req, res) => {
    const b = req.body || {};
...
r.get("/schedule/links", async (_req, res) => { const { rows } = await db.execute(sql `select * from schedule_links where active=true order by createdAt desc`); res.json({ ok: true, items: rows }); });
// PUBLIC: show slots for a given date (now returns human-readable labels + buffers)
r.get("/public/s/:slug/slots", async (req, res) => {
    ...
    for (let i = startIdx; i <= endIdx && i < view.length; i++) {
        if (view[i] === "0")
            slots.push({ startIndex: i, startMinutes: i * link.duration_minutes });
    }
    res.json({
        ok: true,
...
// PUBLIC: book
r.post("/public/s/:slug/book", async (req, res) => {
    const { rows } = await db.execute(sql `select * from schedule_links where slug=${req.params.slug} and active=true limit 1`);
    const link = rows?.[0];
    if (!link)
        return res.status(404).json({ ok: false });
...
function offsetOf(_tz) { return "+00:00"; } // simple: treat times as UTC; UI shows correct TZ
// Find next available slot within N days
r.get("/public/s/:slug/next", async (req, res) => {
