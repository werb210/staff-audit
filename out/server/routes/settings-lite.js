...
r.post("/users", (req, res) => {
    ...
    const u = { id: crypto.randomUUID(), email, name, roles: ["agent"], active: true, createdAt: new Date().toISOString() };
    d.users.push(u);
    save(d);
    res.json(u);
});
...
r.delete("/users/:id", (req, res) => {
    ...
    if (i < 0)
        return res.status(404).json({ error: "not found" });
    const [rm] = d.users.splice(i, 1);
    save(d);
    res.json({ ok: true, id: rm.id });
});
