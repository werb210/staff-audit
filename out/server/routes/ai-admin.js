...
// Update feature flag
router.put('/flags/:flagKey', adminOnly, async (req, res) => {
    try {
...
router.put('/prompts/:promptKey', adminOnly, async (req, res) => {
    try {
...
router.get('/usage/users', adminOnly, async (req, res) => {
    try {
        const { days = 7 } = req.query;
        const tenantId = req.user?.tenant_id || 'bf';
        const startDate = new Date();
      ...
      LIMIT 20
    `);
        const formattedUsage = (usage || []).map((row) => ({
            userId: row.user_id,
...
router.post('/emergency-disable', adminOnly, async (req, res) => {
    try {
        const userId = req.user?.id || req.user?.claims?.sub;
        await db.execute(sql `
      UPDATE ai_feature_flags 
