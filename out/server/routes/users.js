...
function col(...names) { return names.find(Boolean); }
...
r.patch("/:id", async (req, res) => {
    try {
        ...
        const updateVals = [];
        if (name !== undefined) {
            updateSets.push(`first_name=$${updateSets.length + 1}`);
            updateVals.push(name);
        }
        if (email !== undefined) {
            updateSets.push(`email=$${updateSets.length + 1}`);
            updateVals.push(email);
        }
        if (phone !== undefined) {
            updateSets.push(`phone=$${updateSets.length + 1}`);
            updateVals.push(phone);
        }
        if (role !== undefined) {
            updateSets.push(`role=$${updateSets.length + 1}`);
