...
r.post("/marketing/costs/upload", upload.single('csvFile'), async (req, res) => {
    try {
        if (!req.file) {
...
r.get("/marketing/costs/summary", async (req, res) => {
    try {
    ...
    `;
        const conditions = [];
        if (experiment_id)
            conditions.push(sql `experiment_id = ${experiment_id}`);
        if (variant)
            conditions.push(sql `variant = ${variant}`);
...
r.get("/marketing/roi/enhanced", async (req, res) => {
    try {
    ...
    `;
        ...
        const roiData = costs.map((cost) => {
            const revenue = revenues.find((r) => r.experiment_id === cost.experiment_id && r.variant === cost.variant);
