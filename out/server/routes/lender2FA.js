...
router.post('/send-2fa/:lenderId', async (req, res) => {
    try {
...
router.post('/verify-2fa/:lenderId', async (req, res) => {
    try {
        const { lenderId } = req.params;
        const { code } = req.body;
        if (!lenderId || !code) {
...
router.get('/status/:lenderId', async (req, res) => {
    try {
        const { lenderId } = req.params;
        if (!lenderId) {
            return res.status(400).json({
...
router.patch('/mobile-phone/:lenderId', async (req, res) => {
    try {
        const { lenderId } = req.params;
