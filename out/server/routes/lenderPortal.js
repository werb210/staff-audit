...
const requireLenderAuth = async (req, res, next) => {
    try {
...
router.get('/dashboard', async (req, res) => {
    try {
        ...
        const [productCount] = await db
            ...
            .from(lenderProducts)
            .where(and(eq(lenderProducts.lenderId, lenderId), eq(lenderProducts.isActive, true)));
...
router.patch('/products/:productId', async (req, res) => {
    try {
        ...
        const [existingProduct] = await db
            .select()
            .from(lenderProducts)
            .where(and(eq(lenderProducts.id, productId), eq(lenderProducts.lenderId, lenderId)))
            .limit(1);
        if (!existingProduct) {
