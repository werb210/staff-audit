...
const router = Router();
router.get("/", requireAnyRole(["lender", "manager", "admin"]), (req, res) => {
...
router.get("/export.csv", requireAnyRole(["lender", "manager", "admin"]), (req, res) => {
    const lenderId = req.user?.lenderId || req.user?.id;
    const items = req.app.locals.db.lenderReports.listForLender(lenderId);
    const csv = toCSV(items.map((r) => ({ id: r.id, name: r.name, type: r.type, url: r.url || "", embedUrl: r.embedUrl || "", createdAt: r.createdAt })));
