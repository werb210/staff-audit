...
try {
    ...
    enforceProductionSecurity = (phone, environment = "production") => {
        const isTestNumber = (p) => ["+15005550000", "+15005550006", "+15005550008", "+15005550009"].includes(p);
        if (environment === "production" && isTestNumber(phone)) {
            throw new Error("Refusing to send to Twilio test number in production");
        }
    };
}
const { TWILIO_CONTENT_SID, TWILIO_MESSAGING_SERVICE_SID, BF_SMS_FROM_E164, BF_WHATSAPP_FROM, ENVIRONMENT, } = process.env;
