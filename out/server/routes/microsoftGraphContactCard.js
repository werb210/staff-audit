...
import { db } from '../db';
import { emailAccounts, emailMessages2, contactLogs } from '../../shared/schema';
    ...
    try {
        ...
        if (fromAccountId) {
            const accounts = await db
                .select()
                .from(emailAccounts)
                .where(and(eq(emailAccounts.id, fromAccountId), eq(emailAccounts.staffUserId, staffUserId)))
                .limit(1);
    ...
    try {
        ...
        for (const account of emailAccounts) {
            try {
                ...
                // Fetch recent emails
                const inboxResponse = await fetch(`https://graph.microsoft.com/v1.0/me/messages?$top=${limit}&$orderby=receivedDateTime desc`, {
                    headers: {
 ...
 * Track email opens
 */
router.get('/track/email-open/:trackingId', async (req, res) => {
    try {
