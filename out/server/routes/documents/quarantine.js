...
r.post("/scan", upload.single("file"), async (req, res) => {
    if (!req.file) {
...
r.get("/quarantined", async (_req, res) => {
    const quarantinedDocs = await db.select().from(documents).where(eq(documents.status, 'quarantined'));
    res.json({ ok: true, documents: quarantinedDocs });
});
r.post("/release/:documentId", async (req, res) => {
