...
router.post('/escalation', async (req, res) => {
    try {
        ...
        const { applicationId, userId, transcript, priority = 'medium', category = 'general_inquiry' } = req.body;
        if (!transcript) {
...
router.post('/submit', async (req, res) => {
    try {
        ...
        const { sessionId, applicationId, userId, messages, escalationId } = req.body;
...
router.get('/transcripts', async (req, res) => {
    try {
        ...
        if (applicationId) {
            query = query.where(eq(chatTranscripts.applicationId, applicationId));
        }
        if (sessionId) {
            query = query.where(eq(chatTranscripts.sessionId, sessionId));
        }
