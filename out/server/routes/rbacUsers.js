...
import { requireAdmin, logAuditEvent } from '../middleware/rbacAuth';
import { db } from '../db';
...
router.get('/', async (req, res) => {
    try {
        const { page = '1', limit = '50', search = '', role = '', status = 'all' } = req.query;
        ...
        if (search) {
            ...
            if (sanitizedSearch.length > 0) {
                conditions.push(ilike(users.email, `%${sanitizedSearch}%`));
            }
        }
        if (role && role !== 'all') {
...
router.post('/', async (req, res) => {
    try {
        const { email, username, password, role, firstName, lastName, phone, department, tenantId } = req.body;
        // Validation
...
router.patch('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { email, role, password, firstName, lastName, phone, department, isActive } = req.body;
        // Get existing user
        const [existingUser] = await db
            .select()
            .from(users)
