...
r.post("/api/voice/record", async (req, res) => {
    try {
        const { callSid, action } = req.body;
        if (!process.env.VOICE_ALLOW_RECORDING)
            return res.status(403).json({ error: "recording_disabled" });
...
r.post("/api/voice/add-participant", async (req, res) => {
    try {
...
r.post("/api/voice/hold", async (req, res) => {
    try {
        const { conferenceName, participantSid, hold } = req.body;
...
r.post("/api/voice/transfer", async (req, res) => {
    try {
...
r.post("/api/voice/digits", async (req, res) => {
    try {
...
r.post("/api/voice/merge", async (req, res) => {
    try {
        const { conferenceName } = req.body;
        if (!conferenceName)
            return res.status(400).json({ error: "missing_conference_name" });
