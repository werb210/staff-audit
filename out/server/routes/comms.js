...
function bad(res, code = 400, error = "Bad Request") { return res.status(code).json({ ok: false, error }); }
const DEV = process.env.NODE_ENV !== "production";
// ----- SMS -----
router.post("/sms/send", async (req, res) => {
    const { to, body, contactId } = req.body || {};
    if (!to || !body)
        return bad(res, 400, "to and body required");
    try {
...
router.post("/email/send", async (req, res) => {
    ...
    if (!to || !subject || (!text && !html))
        return bad(res, 400, "to, subject and text|html required");
    try {
