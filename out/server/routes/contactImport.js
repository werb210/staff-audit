...
router.post("/clone", requireAnyRole(["admin", "manager"]), async (req, res) => {
    try {
...
router.post("/bulk-import", requireAnyRole(["admin", "manager"]), async (req, res) => {
    try {
...
router.post("/preview", requireAnyRole(["admin", "manager"]), async (req, res) => {
    try {
        const { contacts, targetTenant } = req.body;
        if (!Array.isArray(contacts) || !targetTenant) {
            return res.status(400).json({ error: "invalid_request" });
        ...
        contacts.forEach((contact, index) => {
            ...
            if (!contact.email?.trim() && !contact.phone?.trim())
                errors.push("Missing email or phone");
            if (errors.length === 0) {
