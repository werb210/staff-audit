...
async function findOrCreateUserByPhone(e164) {
  ...
  `);
    ...
    r = await pool.query("INSERT INTO users (full_name, phone_e164, roles) VALUES ($1,$2,$3) RETURNING *", ['New User', e164, ['user']]);
    return r.rows[0];
}
...
router.post("/request", async (req, res) => {
    try {
        ...
        if (!verifySid)
            return res.status(500).json({ error: "verify_service_missing" });
...
router.post("/check", async (req, res) => {
    try {
        ...
        if (r.status !== "approved")
            return res.status(401).json({ ok: false, error: "code_not_approved", status: r.status });
