...
router.post("/subscription", async (req, res) => {
    ...
    if (!plan_key)
        return res.status(400).json({ error: "plan_key required" });
    await db.execute(sql `
  ...
  `);
    ...
    if (!r.rows?.[0])
        await db.execute(sql `INSERT INTO subscriptions(tenant_key, plan_key) VALUES ('default', ${plan_key})`);
    res.json({ ok: true });
});
...
router.post("/meter", async (req, res) => {
    ...
    if (out?.over_limit)
        return res.status(402).json(out); // "Payment Required" when hard-capped
    res.json(out);
});
