...
r.post("/recommendations", async (req, res) => {
    ...
    const tenant = req.query.tenant || "bf";
 ...
 */
r.post("/recommendations/:id/request-approval", async (req, res) => {
    ...
    if (!approverPhone)
        return res.status(500).json({ ok: false, error: "no_approver_configured" });
    ...
    // Send SMS
    const changes = rec.plan_json?.changes || [];
 ...
 */
r.post("/approve/:token", async (req, res) => {
