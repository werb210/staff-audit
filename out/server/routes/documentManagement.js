...
router.post('/staff-upload/:applicationId', async (req, res) => upload.single('file'), async (req, res) => {
    try {
...
router.post('/:id/replace', async (req, res) => upload.single('file'), async (req, res) => {
    try {
        const { id } = req.params;
        const file = req.file;
...
router.get('/:id/preview', async (req, res) => {
    ...
    try {
        ...
        if (docResult.rows.length === 0) {
            ...
            // Log preview attempt
            await pool.query('INSERT INTO document_preview_log (document_id, status, error_message, file_exists, user_agent, ip_address) VALUES ($1, $2, $3, $4, $5, $6)', [id, previewStatus, errorMessage, false, userAgent, ipAddress]);
        ...
        if (document.storage_key) {
            try {
                ...
                const preSignedUrl = await generatePreSignedDownloadUrl(document.storage_key, 3600, // 1 hour expiration
                document.name);
                previewStatus = 200;
                fileExists = true;
                // Log successful S3 access
                await pool.query('INSERT INTO document_preview_log (document_id, status, error_message, file_exists, checksum_valid, user_agent, ip_address) VALUES ($1, $2, $3, $4, $5, $6, $7)', [id, previewStatus, null, true, true, userAgent, ipAddress]);
        ...
        if (document.checksum) {
            try {
                ...
                if (currentChecksum !== document.checksum) {
                    ...
                    await pool.query('INSERT INTO document_preview_log (document_id, status, error_message, file_exists, checksum_valid, user_agent, ip_address) VALUES ($1, $2, $3, $4, $5, $6, $7)', [id, previewStatus, errorMessage, true, false, userAgent, ipAddress]);
                ...
                await pool.query('INSERT INTO document_preview_log (document_id, status, error_message, file_exists, checksum_valid, user_agent, ip_address) VALUES ($1, $2, $3, $4, $5, $6, $7)', [id, previewStatus, errorMessage, true, null, userAgent, ipAddress]);
                return res.status(500).json({ error: errorMessage });
            }
        }
        else {
        ...
        await pool.query('INSERT INTO document_preview_log (document_id, status, error_message, file_exists, checksum_valid, user_agent, ip_address) VALUES ($1, $2, $3, $4, $5, $6, $7)', [id, 200, null, true, checksumValid, userAgent, ipAddress]);
        ...
        try {
            await pool.query('INSERT INTO document_preview_log (document_id, status, error_message, file_exists, user_agent, ip_address) VALUES ($1, $2, $3, $4, $5, $6)', [req.params.id, 500, errorMessage, fileExists, userAgent, ipAddress]);
        }
        catch (logError) {
...
router.get('/applications/:appId/zip', async (req, res) => {
    try {
...
router.get('/audit/application/:id', async (req, res) => {
    try {
        const { id } = req.params;
...
router.get('/:applicationId/download-all', async (req, res) => {
    const { applicationId } = req.params;
        ...
        for (const doc of documents) {
            try {
                ...
                if (s3Response.Body) {
                    ...
                    // Add file to ZIP
                    archive.append(s3Response.Body, { name: zipPath });
                    successCount++;
