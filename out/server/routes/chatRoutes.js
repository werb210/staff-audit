...
`;
        ...
        try {
            ...
            escalated = escalationKeywords.some(keyword => message.toLowerCase().includes(keyword) || aiResponse.toLowerCase().includes('escalate'));
        }
        catch (aiError) {
...
router.get('/escalations', async (req, res) => {
    try {
        ...
        if (status) {
            query = query.where(eq(chatEscalations.status, status));
        }
...
router.get('/issue-reports', async (req, res) => {
    try {
        ...
        const conditions = [];
        if (status) {
            conditions.push(eq(chatIssueReports.status, status));
        }
        if (severity) {
            conditions.push(eq(chatIssueReports.severity, severity));
        }
 ...
 */
router.patch('/escalations/:id/status', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.post('/log-contact', async (req, res) => {
    try {
