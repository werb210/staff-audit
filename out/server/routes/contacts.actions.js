...
export function mountContactActions(app) {
    // POST /api/contacts/:id/email  { subject, body }
    app.post("/api/contacts/:id/email", async (req, res) => {
    ...
    app.post("/api/contacts/:id/meetings", async (req, res) => {
    ...
    app.post("/api/contacts/:id/files", async (req, res) => {
    ...
    app.post("/api/contacts/:id/create-application", async (req, res) => {
        const row = await maybe(`insert into applications (contact_id, requested_amount, status, createdAt)
       values($1,$2,'draft',now()) returning id`, [req.params.id, req.body.amount || 0]);
