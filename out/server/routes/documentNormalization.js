 ...
 */
router.post("/bulk-normalize", async (req, res) => {
    try {
    ...
    try {
        ...
        res.json({
            ...
            validation: {
                ...
                unmapped: normalization.raw.filter(raw => !normalization.normalized.includes(raw) &&
        ...
        for (const product of products) {
            ...
            if (normalization.normalized.length > 0 && Object.keys(normalization.mappings).length === 0) {
                ...
                normalization.raw.forEach(raw => {
                    if (!normalization.normalized.includes(raw) && !normalization.mappings[raw]) {
