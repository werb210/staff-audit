...
function validateTwilio(req) {
    ...
    if (!sig)
        return true; // Loosen for local development
    ...
    try {
        ...
        if (code) {
            [ar] = await db.select()
                ...
                .where(and(eq(approvalRequests.code, code), eq(approvalRequests.status, 'queued'), eq(approvalRequests.approverPhone, approver)))
            ...
            [ar] = await db.select()
                .from(approvalRequests)
                .where(and(eq(approvalRequests.status, 'queued'), eq(approvalRequests.approverPhone, approver)))
                .orderBy(approvalRequests.createdAt)
                .limit(1);
        }
        if (!ar) {
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const [item] = await db.select()
            .from(approvalRequests)
            .where(and(eq(approvalRequests.id, id), eq(approvalRequests.tenant, tenant)))
            .limit(1);
        if (!item) {
