    ...
    try {
        ...
        if (search) {
            ...
            if (searchTerm.length > 0) {
                ...
                conditions.push(or(ilike(users.firstName, searchPattern), ilike(users.lastName, searchPattern), ilike(users.username, searchPattern), ilike(users.email, searchPattern)));
            }
        }
        if (role && role !== 'all') {
...
// Revoke user session
router.post('/users/:userId/sessions/:sessionId/revoke', async (req, res) => {
    try {
...
router.post('/users/:userId/sessions/revoke-all', async (req, res) => {
    try {
        const { userId } = req.params;
        await db
            .update(userSessions)
...
router.get('/users/:userId/login-attempts', async (req, res) => {
    try {
        const { userId } = req.params;
...
router.patch('/users/:userId', async (req, res) => {
    try {
        ...
        await db.insert(userAuditLog).values({
            ...
            resource: 'user',
            resourceId: userId,
            oldValues: Object.fromEntries(Object.entries(changes).map(([k, v]) => [k, v.from])),
            newValues: Object.fromEntries(Object.entries(changes).map(([k, v]) => [k, v.to])),
            ipAddress: req.ip,
            userAgent: req.headers['user-agent'],
        });
...
router.get('/users/:userId/audit-log', async (req, res) => {
    try {
        const { userId } = req.params;
        ...
        const auditEntries = await db
            .select()
            .from(userAuditLog)
            .where(or(eq(userAuditLog.userId, userId), eq(userAuditLog.resourceId, userId)))
...
router.get('/audit-log', async (req, res) => {
    try {
        ...
        const conditions = [];
        if (action) {
            conditions.push(eq(userAuditLog.action, action));
        }
        if (resource) {
            conditions.push(eq(userAuditLog.resource, resource));
        }
        if (conditions.length > 0) {
            query = query.where(and(...conditions));
        }
