 ...
 */
function calculateMatchScore(application, product, docStatus) {
    let score = 0;
    const reasons = [];
    ...
    const productIndustries = (product.industries || []).map((i) => i.toLowerCase());
    if (productIndustries.length === 0 || productIndustries.includes('all') || productIndustries.includes(appIndustry)) {
        ...
        const hasPartialMatch = productIndustries.some(pi => industryKeywords.some(keyword => pi.includes(keyword) || keyword.includes(pi)));
        if (hasPartialMatch) {
            score += 8;
    ...
    const productGeography = (product.geography || ['US']).map((g) => g.toUpperCase());
 ...
 */
router.post('/transmit/:applicationId/:productId', async (req, res) => {
    try {
      ...
      RETURNING id, createdAt`;
        ...
        await pool.query(`UPDATE applications SET status = 'submitted_to_lenders', updatedAt = NOW() WHERE id = $1`, [applicationId]);
        res.json({
            success: true,
            transmissionId,
            applicationId,
