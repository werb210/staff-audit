...
router.get("/api/admin/users/audit", requireAdmin, async (req, res) => {
    try {
...
async function logUserAudit(userId, action, targetUserId, details) {
    try {
        await db.execute(sql `
...
router.get("/api/admin/users/roles", requireAdmin, async (req, res) => {
    try {
        const roles = [
