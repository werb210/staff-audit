...
router.get("/lenders/:lenderId/report", allowPublicAccess, async (req, res) => {
    try {
...
router.get("/lenders/:lenderId/report/download", allowPublicAccess, async (req, res) => {
    try {
        const { lenderId } = req.params;
    ...
    `);
        if (format === 'csv') {
            ...
            const csvRows = applications.map(app => `${app.id},${app.business_name || 'N/A'},${app.amount},${app.status},${app.createdAt},${app.updatedAt},${app.commission}`).join('\n');
...
router.get("/lenders/matching-summary", allowPublicAccess, async (req, res) => {
    try {
...
router.get("/lenders/:id/matches", allowPublicAccess, async (req, res) => {
    try {
        const { id } = req.params;
