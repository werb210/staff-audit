...
router.get("/lenders/:id/settings", allowPublicAccess, async (req, res) => {
    try {
        const { id } = req.params;
        // Get lender details
...
router.put("/lenders/:id/settings", allowPublicAccess, async (req, res) => {
    try {
        const { id } = req.params;
        const { contact_email, phone, website, commission_rate, country_access } = req.body;
        const updateData = {};
        if (contact_email)
            updateData.contactEmail = contact_email;
        if (phone)
        ...
        if (country_access && country_access.length > 0)
            updateData.country = country_access[0];
...
router.get("/lenders/:id/messages", allowPublicAccess, async (req, res) => {
    try {
        const { id } = req.params;
...
router.post("/lenders/:id/messages", allowPublicAccess, async (req, res) => {
    try {
        const { id } = req.params;
