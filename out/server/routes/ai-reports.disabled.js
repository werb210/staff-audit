...
router.post('/session/start', authenticateClient, async (req, res) => {
    try {
...
router.post('/session/log', authenticateClient, async (req, res) => {
    try {
...
router.get('/sessions', authenticateStaff, async (req, res) => {
    try {
        ...
        if (status && status !== 'all') {
            conditions.push(eq(applicantSessions.status, status));
        }
        if (businessName) {
...
router.get('/sessions/:sessionId', authenticateStaff, async (req, res) => {
    try {
...
router.get('/stats/summary', authenticateStaff, async (req, res) => {
    try {
        // Total sessions
        ...
        const messagesResult = await db
            .select({ total: sql `SUM(${applicantSessions.totalMessages})` })
            .from(applicantSessions);
...
router.patch('/sessions/:sessionId/complete', authenticateStaff, async (req, res) => {
    try {
        const { sessionId } = req.params;
        await db
            .update(applicantSessions)
