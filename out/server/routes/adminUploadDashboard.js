...
router.get('/metrics', async (req, res) => {
    ...
    `);
        // Calculate metrics
        const totalUploads = uploadStats.rows.reduce((sum, stat) => sum + parseInt(stat.count), 0);
        const successCount = uploadStats.rows.find((stat) => stat.status === 'success')?.count || 0;
        const fallbackCount = uploadStats.rows.find((stat) => stat.status === 'fallback')?.count || 0;
        const failureCount = uploadStats.rows.find((stat) => stat.status === 'failure')?.count || 0;
        const fallbackFilesExist = fallbackDocs.rows.filter((doc) => doc.file_status === 'file_exists').length;
...
router.get('/health', async (req, res) => {
    try {
        ...
        try {
            ...
            console.warn('⚠️ [ADMIN] S3 health check failed:', s3Error.message);
        }
...
router.post('/retry-upload/:documentId', async (req, res) => {
    try {
    ...
    `);
        ...
        const result = await recovery.retryFallbackUpload(doc.rows[0]);
        if (result.success) {
            res.json({
                success: true,
...
router.post('/retry-all-fallbacks', async (req, res) => {
    try {
