...
router.post("/policies", async (req, res) => {
    ...
    if (!target || !days)
        return res.status(400).json({ error: "target and days required" });
...
router.post("/erasure/:id/decide", async (req, res) => {
...
router.get("/erasure/:id/dry-run", async (req, res) => {
  ...
  `);
    ...
    res.json({ counts, apps, total: Object.values(counts).reduce((sum, count) => sum + count, 0) });
});
router.get("/erasure/:id/export", async (req, res) => {
    const reqData = await db.execute(sql `
    SELECT * FROM erasure_requests WHERE id = ${req.params.id} LIMIT 1
  `);
    const request = reqData.rows?.[0];
...
router.post("/erasure/:id/purge", async (req, res) => {
    const reqData = await db.execute(sql `
  ...
  `);
    ...
    `);
        ...
        res.status(500).json({ error: 'Purge failed: ' + error.message });
    }
});
  ...
  `);
    ...
    for (const policy of (policies.rows || [])) {
        try {
            ...
            results[policy.target] = { error: error.message };
        }
    }
