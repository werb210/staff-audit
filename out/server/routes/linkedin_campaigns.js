...
r.post("/", async (req, res) => {
    const { tenant = "bf", templateId, audienceId, mode = "ads", scheduleAt, budgetCents, adAccountUrn } = req.body || {};
...
r.put("/:id", async (req, res) => {
    const { templateId, audienceId, mode, scheduleAt, budgetCents, status, adAccountUrn, campaignUrn, meta } = req.body || {};
...
r.post("/:id/launch", async (req, res) => {
    ...
    if (!campaign.template)
        return res.status(400).json({ ok: false, error: "no_template" });
    if (!campaign.audience)
        return res.status(400).json({ ok: false, error: "no_audience" });
