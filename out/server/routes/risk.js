...
router.post("/analyze/:applicationId", simpleAuth, async (req, res) => {
    try {
        ...
        const revenue = Number(financial?.annualRevenue ||
            ocrData?.extractedRevenue ||
            formData?.annualRevenue ||
            0);
        const expenses = Number(financial?.monthlyExpenses ||
        ...
        const riskInput = {
            applicationId,
            creditScore: Number(financial?.creditScore ||
        ...
        const savedAssessment = await riskEngine.saveRiskAssessment(applicationId, result, riskInput, "openai", "gpt-4o", processingTime);
        res.json({
            success: true,
            data: result,
            metadata: {
...
/* --- RISK METRICS --------------------------------------------------------- */
router.get("/metrics", simpleAuth, async (req, res) => {
    try {
        const assessments = await db.select().from(riskAssessments);
        const metrics = {
            totalAssessments: assessments.length,
            ...
            trends: {
                last30Days: assessments.filter(a => a.createdAt &&
                    new Date(a.createdAt) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)).length
            }
        };
...
/* --- GET RISK ASSESSMENT -------------------------------------------------- */
router.get("/assessment/:applicationId", simpleAuth, async (req, res) => {
    try {
        const applicationId = req.params.applicationId;
...
/* --- STRESS TEST ---------------------------------------------------------- */
router.post("/stress-test", simpleAuth, async (req, res) => {
    try {
        ...
        for (const scenario of testScenarios) {
            ...
            const estimatedNewScore = Math.min(100, baseAssessment.riskScore * (2 - impactMultiplier) + (rateImpact * 100));
            results.push({
...
/* --- COMPLIANCE CHECK ----------------------------------------------------- */
router.get("/compliance-check/:applicationId", simpleAuth, async (req, res) => {
    try {
        const applicationId = req.params.applicationId;
...
/* --- INDUSTRY BENCHMARK --------------------------------------------------- */
router.post("/industry-benchmark", simpleAuth, async (req, res) => {
    try {
        ...
        const benchmark = industryBenchmarks[normalizedIndustry]
...
router.get("/assessments", simpleAuth, async (req, res) => {
    try {
