...
import { validateChecksum, logRecoveryEvent, validateMultipleDocuments } from '../utils/documentValidation.js';
import { createDocumentVersion, getDocumentVersionHistory, restoreDocumentVersion, cleanupOldVersions } from '../utils/documentVersioning.js';
...
import { generateHealthReport, getApplicationHealthReport, exportHealthReportToCSV, getRecoveryEventHistory } from '../utils/documentHealthDashboard.js';
...
router.post('/versions/:documentId/upload', async (req, res) => {
    try {
        const { documentId } = req.params;
        ...
        const result = await createDocumentVersion(documentId, tempFilePath, req.user?.id || 'unknown', notes || 'Document updated via API');
        res.json({
            success: true,
...
router.post('/versions/:documentId/restore/:versionNumber', async (req, res) => {
    try {
        const { documentId, versionNumber } = req.params;
        const { notes } = req.body;
        console.log(`🔄 [API] Restoring document ${documentId} to version ${versionNumber}`);
        const result = await restoreDocumentVersion(documentId, parseInt(versionNumber), req.user?.id || 'unknown', notes || `Restored to version ${versionNumber}`);
        if (result.success) {
            res.json({
                success: true,
...
router.post('/versions/:documentId/cleanup', async (req, res) => {
    try {
        const { documentId } = req.params;
...
router.get('/recovery-events/:documentId?', async (req, res) => {
    try {
        const { documentId } = req.params;
        ...
        if (documentId) {
            ...
            const events = await getRecoveryEventHistory(parseInt(limit));
            res.json({
                success: true,
                events
            });
...
// Retry specific item
router.post('/retry-queue/:queueId/retry', async (req, res) => {
    try {
...
router.post('/retry-queue/clear-completed', async (req, res) => {
    try {
...
router.get('/health/application/:applicationId', async (req, res) => {
    try {
...
router.get('/health/report/export', async (req, res) => {
    try {
...
router.post('/preview-status/:documentId', async (req, res) => {
    try {
        const { documentId } = req.params;
        ...
        await logRecoveryEvent(documentId, previewStatus, req.user?.id || null, notes || `Preview status changed to ${previewStatus}`);
        res.json({
            success: true,
            documentId,
            previewStatus,
