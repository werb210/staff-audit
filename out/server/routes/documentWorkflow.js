 ...
 */
router.post("/api/applications/:id/nudge-documents", bearerAuth, async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
router.get("/api/public/applications/:id/required-docs", async (req, res) => {
    try {
        const { id } = req.params;
 ...
 * Compute required documents based on application details
 */
async function computeRequiredDocs(application) {
