...
const tenantOf = (req) => (String(req.headers["x-tenant"] || "bf").toLowerCase() === "slf" ? "slf" : "bf");
async function pickTable(cands) {
    const { rows } = await pool.query(`SELECT table_name FROM information_schema.tables WHERE table_schema='public' AND table_name = ANY($1::text[])`, [cands]);
    for (const c of cands)
        if (rows.find(r => r.table_name === c))
            return c;
    return null;
}
function asNum(x) { if (x == null || x === "")
    return null; const n = Number(x); return isFinite(n) ? n : null; }
function asBool(x, def = true) { if (x == null || x === "")
    return def; const s = String(x).toLowerCase(); return ["1", "true", "yes", "y"].includes(s); }
