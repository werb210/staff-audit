...
router.get('/:contactId/twilio-logs', async (req, res) => {
    try {
        ...
        const searchFilteredLogs = q
            ? filteredLogs.filter(log => log.message?.toLowerCase().includes(q.toLowerCase()) ||
...
router.get('/:contactId/twilio-logs/stats', async (req, res) => {
    try {
        const { contactId } = req.params;
...
router.post('/:contactId/twilio-logs/quick-reply', async (req, res) => {
    try {
        const { contactId } = req.params;
