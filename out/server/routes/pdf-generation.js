...
import { generateApplicationPDF, generateAllApplicationPDFs, testPDFGeneration, saveDocumentToS3AndDB } from '../services/pdfGeneratorService';
const router = Router();
/**
...
router.post('/generate/:applicationId', async (req, res) => {
    try {
        ...
        const documentId = await saveDocumentToS3AndDB(applicationId, pdfBuffer, filename, 'generated_application_summary', 'accepted');
        res.json({
            success: true,
