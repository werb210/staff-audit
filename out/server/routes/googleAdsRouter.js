...
import { getCampaignPerformance, getSearchTerms, getPMaxAssets, getRecommendations, detectAnomalies, suggestBudgetRebalance, clusterSearchTerms } from "../lib/googleAds";
...
r.get("/oauth/callback", async (req, res) => {
    ...
    try {
        ...
        req.session.gadsConnected = true;
...
r.get("/status", (req, res) => {
    res.json({ connected: !!req.session.gadsConnected });
});
// Overview endpoint with performance cards
r.get("/overview", async (req, res, next) => {
    try {
        const { customerId = "", range = "7d" } = req.query;
        if (!customerId) {
            return res.json({
                connected: false,
        ...
        const totalSpend = campaigns.results?.reduce((sum, c) => sum + (c.metrics?.cost_micros || 0), 0) / 1000000 || 0;
        const totalConversions = campaigns.results?.reduce((sum, c) => sum + (c.metrics?.conversions || 0), 0) || 0;
        const totalClicks = campaigns.results?.reduce((sum, c) => sum + (c.metrics?.clicks || 0), 0) || 0;
        const totalImpressions = campaigns.results?.reduce((sum, c) => sum + (c.metrics?.impressions || 0), 0) || 0;
        const ctr = totalImpressions > 0 ? (totalClicks / totalImpressions) * 100 : 0;
        const cpa = totalConversions > 0 ? totalSpend / totalConversions : 0;
        const roas = totalSpend > 0 ? (campaigns.results?.reduce((sum, c) => sum + (c.metrics?.conversions_value || 0), 0) || 0) / totalSpend : 0;
        // Detect anomalies
        ...
        res.json({
            ...
            health: {
                disapprovedAds: recommendations.results?.filter((r) => r.recommendation?.type === 'AD_DISAPPROVED').length || 0,
                limitedBudget: campaigns.results?.filter((c) => c.campaign?.status === 'BUDGET_LIMITED').length || 0,
                learningCampaigns: campaigns.results?.filter((c) => c.campaign?.learning_status === 'LEARNING').length || 0
            },
            anomalies,
            budgetSuggestions,
...
// Campaigns deep-dive
r.get("/campaigns", async (req, res, next) => {
    try {
        const { customerId = "", range = "7d" } = req.query;
        const campaigns = await getCampaignPerformance(customerId, range);
        res.json(campaigns);
...
r.get("/search-terms", async (req, res, next) => {
    try {
        const { customerId = "", range = "7d" } = req.query;
        const searchTerms = await getSearchTerms(customerId, range);
...
r.get("/assets", async (req, res, next) => {
    try {
        ...
        const coverage = assetTypes.map(type => {
            const typeAssets = assets.results?.filter((a) => a.asset_group_asset?.field_type === type) || [];
            return {
                type,
                count: typeAssets.length,
                required: type === 'HEADLINE' ? 5 : type === 'DESCRIPTION' ? 5 : 1,
                performance: typeAssets.reduce((sum, a) => sum + (a.metrics?.clicks || 0), 0)
            };
        });
        res.json({
...
// AI endpoints
r.post("/ai/explain", async (req, res) => {
    try {
...
r.post("/ai/rebalance", async (req, res) => {
    try {
...
r.post("/negatives/apply", async (req, res) => {
    try {
