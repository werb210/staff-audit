...
async function tableExists(name) {
    const { rows } = await db.query(`select 1 from information_schema.tables where table_schema='public' and table_name=$1`, [name]);
...
r.get("/api/_int/auth/first-signin/diag", async (req, res) => {
    ...
    try {
        ...
        if (!dryRun || sendTest) {
            ...
            const { rows } = await db.query(`insert into invites (email, phone_e164, role, token, expires_at)
         values ($1,$2,'lender_admin',$3,$4) returning id`, [email || null, phone || null, token, exp]);
    ...
    const hardFails = [
        ...
        checks.user_columns.some((c) => !c.ok),
        checks.invite_columns.some((c) => !c.ok),
    ];
    ...
    const narrative = `First-time sign-in & verification ${ok ? "is READY" : "is NOT READY"}.
What Replit checked:
• Database: users, invites, user_lenders tables present; required columns ${hardFails.includes(true) ? "have gaps" : "are OK"}.
• Messaging: email (SendGrid) and SMS (Twilio) ${messagingOK ? "configured" : "missing config"}.
• Routes: invite accept + 2FA start/verify ${routesOK ? "responding" : "not responding"}.
• Token: ${checks.token_ready ? "generated successfully" : "failed"}${checks.sample_accept_url ? `\nSample accept URL (dry-run): ${checks.sample_accept_url}` : ""}
