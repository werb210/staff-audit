...
router.get("/", requireRole(["lender", "manager", "admin"]), async (req, res) => {
...
router.post("/:appId/accept", requireRole(["lender", "manager", "admin"]), async (req, res) => {
    const { appId } = req.params;
    const { amount } = req.body || {};
    const app = await req.app.locals.db.apps.get(appId);
    if (!app)
        return res.status(404).json({ error: "not_found" });
    if (app.stage !== "Off to Lender")
        return res.status(409).json({ error: "bad_stage" });
...
router.post("/:appId/decline", requireRole(["lender", "manager", "admin"]), async (req, res) => {
    const { appId } = req.params;
