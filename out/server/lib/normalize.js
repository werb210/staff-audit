...
export function parseMoney(val) {
    if (val == null)
        return { cents: null, decimal: null, raw: null };
...
const key = (() => {
    ...
    if (env.startsWith("base64:"))
        return Buffer.from(env.replace("base64:", ""), "base64");
    if (env.length === 32)
...
export function encryptPII(obj) {
...
export function decryptPII(buf) {
