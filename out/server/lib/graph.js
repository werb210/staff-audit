...
export async function getAppToken(scope = "https://graph.microsoft.com/.default") {
    ...
    if (!res?.accessToken)
        throw new Error("GRAPH_APP_TOKEN_FAILED");
    return res.accessToken;
}
async function graphFetch(path, init, token) {
    ...
    if (!res.ok)
        throw new Error(`GRAPH ${init?.method || "GET"} ${url} -> ${res.status}: ${txt}`);
    return data;
}
export async function sendMail({ userAccessToken, fromUserPrincipal, to, subject, html, cc = [], bcc = [], attachments = [] }) {
