 ...
 */
export const LEGACY_DOCUMENT_MAPPING = {
...
export function normalizeDocumentType(docType) {
    ...
    if (CANONICAL_DOCUMENT_TYPES.includes(cleaned)) {
        return cleaned;
    }
 ...
 */
export function normalizeDocumentRequirements(requirements) {
    const raw = [];
    const normalized = [];
    const mappings = {};
 ...
 */
export function isCanonicalDocumentType(docType) {
    return CANONICAL_DOCUMENT_TYPES.includes(docType);
}
/**
 * Gets all legacy variants that map to a canonical type
 */
export function getLegacyVariants(canonicalType) {
 ...
 */
export async function bulkNormalizeLenderProducts(db, lenderProducts) {
    const results = {
        processed: 0,
        updated: 0,
        errors: 0,
