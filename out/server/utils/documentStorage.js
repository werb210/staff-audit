...
import { enqueueRecoveryAlert } from './safeFileWriter.js';
 ...
 */
export async function saveDocumentToDiskAndDB(applicationId, sourceFilePath, originalFileName, documentType, uploadedBy = 'system') {
 ...
 */
export async function verifyDocumentOnDisk(filePath) {
    try {
 ...
 */
export async function validateApplicationHasDocuments(applicationId) {
    try {
 ...
 */
export async function checkDocumentsBeforeAppDeletion(applicationId) {
    try {
        const appDocuments = await db.select().from(documents).where(eq(documents.applicationId, applicationId));
 ...
 */
export async function checkUploadFailureAlert() {
    try {
        const { pool } = await import('../db.js');
 ...
 */
export async function getDocumentFromDisk(filePath) {
 ...
 */
async function findDocumentAnywhere(documentId, extension) {
    try {
        const uploadsDir = path.join(BASE_DIR, 'uploads');
 ...
 */
export function generateTimestampedFileName(originalName, prefix) {
 ...
 */
export async function uploadToStorage(filePath, storageKey) {
    try {
