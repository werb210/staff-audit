 ...
 */
export async function generatePreSignedDownloadUrl(s3Key, expiresIn = 14400, // 4 hours default (14400 seconds)
fileName) {
 ...
 */
export async function generatePreSignedUploadUrl(s3Key, contentType = 'application/pdf', expiresIn = 14400 // 4 hours default (14400 seconds)
) {
 ...
 */
export function validateS3PreSignedConfig() {
 ...
 */
export async function generateBatchPreSignedUrls(s3Keys, expiresIn = 3600) {
    console.log(`🔗 [BATCH-SIGNED] Generating ${s3Keys.length} pre-signed URLs`);
    const results = await Promise.all(s3Keys.map(async ({ key, fileName }) => ({
        key,
        fileName,
