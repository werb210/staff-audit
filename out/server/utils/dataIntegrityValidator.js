...
export async function validateApplicationSubmission(applicationId) {
    const result = {
        isValid: true,
        errors: [],
        warnings: [],
...
export async function validateS3Storage(documentId) {
    try {
        const [document] = await db.select().from(documents).where(eq(documents.id, documentId));
        if (!document) {
            return { isValid: false, errors: [`Document ${documentId} not found in database`] };
        }
        const s3Key = document.storageKey || document.storageKey || document.objectStorageKey;
        if (!s3Key) {
...
export async function validateDataSafety() {
    const warnings = [];
    const violations = [];
    try {
...
export async function getSystemIntegrityStatus() {
    try {
