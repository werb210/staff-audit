...
function expandRules(rule, dateLocal) {
...
export function getAvailability(username, fromISO, days = 21) {
    ...
    for (let i = 0; i < days; i++) {
        ...
        const daySlots = rules
            ...
            .filter(s => !black.some(b => !(parseISO(s.endUtc) <= parseISO(b.startUtc) || parseISO(s.startUtc) >= parseISO(b.endUtc))));
