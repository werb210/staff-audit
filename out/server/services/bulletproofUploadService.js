 ...
 */
export async function saveFileToDisk(buffer, originalName, documentId) {
 ...
 */
async function queueForBackup(documentId) {
 ...
 */
async function performBackup(documentId) {
    if (!s3Client) {
 ...
 */
async function updateBackupStatus(documentId, status, objectKey) {
    try {
        await db
            .update(documents)
            .set({
 ...
 */
export async function bulletproofUploadDocument(buffer, originalName, documentType, applicationId, uploadedBy) {
