    ...
    for (const reminder of due) {
        try {
            if (reminder.channel === "sms") {
                ...
                const [contact] = await q(`SELECT id, phone, sms_opt_out FROM contacts WHERE id=$1 LIMIT 1`, [reminder.target_id]);
                ...
                try {
                    const [lender] = await q(`SELECT email FROM lender_users WHERE org_id=$1 AND is_admin=true ORDER BY createdAt LIMIT 1`, [reminder.target_id]);
