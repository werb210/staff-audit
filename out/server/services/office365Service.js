...
const O365_SCOPES = process.env.O365_SCOPES ||
    [
        "offline_access",
        "openid",
        "profile",
...
async function readToken(userId) {
    ...
    if (db) {
        try {
            const rows = await db.execute(sql `select user_id, access_token, refresh_token, expires_at, scope, tenant from o365_tokens where user_id=${userId} limit 1`);
            const r = Array.isArray(rows) ? rows[0] : rows?.rows?.[0];
            if (r)
                return { ...r, user_id: String(r.user_id) };
        }
        catch { }
    }
    return mem.get(userId) || null;
}
async function writeToken(row) {
    const db = await getDb();
    if (db) {
        try {
            await db.execute(sql `insert into o365_tokens (user_id, access_token, refresh_token, expires_at, scope, tenant, createdAt, updatedAt)
...
async function getAuthUrl(state) {
    if (!hasCredentials())
        throw new Error("o365_not_configured");
...
async function graph(userId, path, init) {
...
async function listMessages(userId, top = 25) {
    const data = await graph(userId, `/me/messages?$top=${encodeURIComponent(String(top))}&$select=id,subject,from,receivedDateTime,isRead`);
    return data;
}
async function sendMail(userId, input) {
...
async function listEvents(userId, range = {}) {
    ...
    if (range.start && range.end) {
        const url = `/me/calendarView?startDateTime=${encodeURIComponent(range.start)}&endDateTime=${encodeURIComponent(range.end)}&$select=subject,organizer,start,end,location,id`;
