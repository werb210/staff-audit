...
export async function safeSend(fn) {
    try {
        return await fn();
    }
    catch (e) {
...
export async function sendClientSMS(to, body, silo = "BF") {
    const from = silo === "BF" ? BF_FROM : SLF_VOICE;
    return safeSend(() => client.messages.create({ to, from, body }));
}
// Legacy compatibility
export async function sendSmsSafe(to, msg, brand = "BF") {
    const now = new Date();
...
export async function sendSms(to, body, brand = "BF") {
