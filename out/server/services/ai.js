...
function msTimeout(p, ms = TIMEOUT) {
...
async function runJSON(prompt, schema, modelEnvKey) {
    const model = process.env[modelEnvKey] || "gpt-4o-mini";
    const start = Date.now();
    try {
        const response = await msTimeout(openai.chat.completions.create({
            model,
...
/* ---------------- PROMPTS ---------------- */
function sysDocScan(applicationId) {
...
/* ---------------- EXPORTS (real calls) ---------------- */
export async function aiScanDocs(input) {
...
export async function aiValidate(input) {
...
export async function aiRedFlags(input) {
...
export async function aiFinancialHealth(input) {
...
export async function aiApprovalProbability(input) {
...
export async function aiRouting(input) {
...
export async function aiCreditSummary(input) {
...
export async function aiBenchmarks(input) {
...
export async function aiComposeSMS(input) {
...
export async function aiRequestMissing(input) {
...
export async function aiAML(input) {
...
export async function aiLenderQA(input) {
...
export async function visionJson(prompt, imageUrl) {
    const r = await fetch(API, {
        method: "POST",
        headers: { "Authorization": `Bearer ${KEY}`, "Content-Type": "application/json" },
        body: JSON.stringify({
