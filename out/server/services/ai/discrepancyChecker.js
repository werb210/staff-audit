...
export class DiscrepancyCheckerService {
    ...
    async compareFieldsToApplication(applicationId) {
        try {
    ...
    async getDocumentInsights(docs) {
        const insights = {};
    ...
    async performFieldComparisons(application, documentInsights) {
        const discrepancies = [];
        ...
        for (const mapping of fieldMappings) {
            ...
            if (!appValue)
                continue; // Skip if no application value
            ...
            for (const [documentId, fields] of Object.entries(documentInsights)) {
                ...
                if (!docField)
                    continue; // Skip if field not found in document
    ...
    async performStandardComparison(appValue, docValue) {
        if (!appValue || !docValue)
            return false;
    ...
    async performCustomComparison(fieldName, appValue, docValue) {
        switch (fieldName) {
    ...
    compareAmountToRevenue(loanAmount, revenueStr) {
    ...
    async detectAIDiscrepancies(application, documentInsights) {
        try {
...
- Unusual patterns or red flags`;
            ...
            return aiDiscrepancies.map((item) => ({
    ...
    calculateConfidence(discrepancies, documentInsights) {
    ...
    generateRecommendations(discrepancies, application) {
    ...
    generateEmptyReport(applicationId) {
        return {
            applicationId,
            discrepancies: [],
            overallRisk: 'low',
