...
export class RiskScorerService {
    SYSTEM_PROMPT = `You are a senior lending risk analyst with 20+ years of experience. Assess lending risk based on business information, financial data, document analysis, and banking history.
...
Consider: business stability, financial performance, documentation quality, banking behavior, compliance issues, and market conditions.`;
    ...
    async generateRiskScore(applicationId) {
        try {
    ...
    async calculateComponentScores(data) {
    ...
    calculateBusinessRisk(application) {
        let risk = 5; // Base risk
        // Time in business
        if (application.timeInBusiness) {
            if (application.timeInBusiness >= 60)
                risk -= 2; // 5+ years
            else if (application.timeInBusiness >= 24)
                risk -= 1; // 2+ years
            else if (application.timeInBusiness < 12)
                risk += 2; // Less than 1 year
        }
        ...
        if (application.amountRequested && application.timeInBusiness) {
            ...
            if (monthlyRisk > 10000)
                risk += 1; // High loan relative to business age
        }
        return Math.max(0, Math.min(10, risk));
    }
    // Calculate financial risk (0-10)
    calculateFinancialRisk(application, fieldAggregation) {
        let risk = 5; // Base risk
        // Monthly revenue analysis
        if (application.monthlyRevenue) {
            if (application.monthlyRevenue >= 100000)
                risk -= 2; // Strong revenue
            else if (application.monthlyRevenue >= 50000)
                risk -= 1; // Good revenue
            else if (application.monthlyRevenue < 10000)
                risk += 2; // Low revenue
        }
        else {
        ...
        if (application.amountRequested && application.monthlyRevenue) {
            ...
            if (ratio > 0.3)
                risk += 2; // High debt service ratio
            else if (ratio > 0.2)
                risk += 1; // Moderate ratio
            else if (ratio < 0.1)
                risk -= 1; // Low ratio
        }
    ...
    calculateDocumentRisk(documents, discrepancyReport) {
        let risk = 5; // Base risk
        // Document quantity
        if (documents.length >= 5)
            risk -= 1; // Good documentation
    ...
    calculateBankingRisk(bankingAnalysis) {
        if (!bankingAnalysis)
            return 6; // Higher risk if no banking data
        let risk = 5; // Base risk
        ...
        if (summary) {
            ...
            if (summary.totalNSFIncidents === 0)
                risk -= 1;
            else if (summary.totalNSFIncidents >= 3)
                risk += 2;
            else if (summary.totalNSFIncidents >= 1)
                risk += 1;
            // Overdraft frequency
            if (summary.overdraftFrequency > 20)
                risk += 2; // Frequent overdrafts
            else if (summary.overdraftFrequency > 10)
                risk += 1;
            else if (summary.overdraftFrequency < 5)
                risk -= 1;
            // Cash flow trend
            ...
            if (summary.averageMonthlyBalance > 50000)
                risk -= 1;
            else if (summary.averageMonthlyBalance < 5000)
                risk += 1;
        }
        return Math.max(0, Math.min(10, risk));
    }
    // Calculate compliance risk (0-10)
    calculateComplianceRisk(application, fieldAggregation) {
    ...
    calculateOverallScore(components) {
        ...
        const weightedScore = components.businessRisk * weights.businessRisk +
    ...
    async generateAIAnalysis(data, components, overallScore) {
        try {
...
Focus on specific, actionable insights for lending decisions.`;
    ...
    calculateConfidence(data) {
        ...
        // Data availability
        if (data.application.monthlyRevenue)
            confidence += 0.1;
        if (data.application.timeInBusiness)
            confidence += 0.1;
        ...
        if (data.bankingAnalysis)
            confidence += 0.1;
        if (data.fieldAggregation?.documentsSummary?.documentsProcessed > 0)
            confidence += 0.1;
        return Math.min(1, confidence);
    }
    // Generate simple heuristic score when AI fails
    generateHeuristicScore(applicationId, basicData) {
        // Simplified scoring based on available data
        let score = 5; // Default medium risk
        if (basicData.timeInBusiness < 12)
            score += 2;
        if (basicData.amountRequested > 100000)
            score += 1;
        if (!basicData.monthlyRevenue)
            score += 1;
