...
export class NSFAnalyzerService {
    ...
    async analyzeNSFTrends(documentId) {
        try {
    ...
    extractMonthlyNSFData(monthlyStats) {
    ...
    calculateOverallSummary(monthlyNSFData) {
        ...
        // Find peak NSF month
        const peakMonth = monthlyNSFData.reduce((peak, current) => current.nsfCount > peak.nsfCount ? current : peak);
    ...
    determineOverallTrend(monthlyNSFData) {
        if (monthlyNSFData.length < 3)
            return 'STABLE';
        ...
        if (change >= 1)
            return 'WORSENING'; // More than 1 additional NSF per month
        if (change <= -0.5)
            return 'IMPROVING'; // At least 0.5 fewer NSF per month
        return 'STABLE';
    }
    // Determine severity based on NSF patterns
    determineSeverity(totalIncidents, averageMonthly, monthsWithNSF) {
        // Critical: Very frequent NSF incidents
        if (averageMonthly >= 3 || totalIncidents >= 15)
            return 'CRITICAL';
        // High: Regular NSF incidents
        if (averageMonthly >= 1.5 || totalIncidents >= 8)
            return 'HIGH';
        // Moderate: Occasional NSF incidents
        if (averageMonthly >= 0.5 || totalIncidents >= 3)
            return 'MODERATE';
        // Low: Rare or no NSF incidents
        return 'LOW';
    }
    // Perform detailed trend analysis
    performTrendAnalysis(monthlyNSFData) {
    ...
    assessConsistency(monthlyNSFData) {
    ...
    assessRecentTrend(monthlyNSFData) {
        if (monthlyNSFData.length < 3)
            return 'STABLE';
    ...
    identifyRiskFactors(summary, trends) {
        const risks = [];
        // High NSF frequency
