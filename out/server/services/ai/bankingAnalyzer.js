...
export class BankingAnalyzerService {
    ...
    async analyzeBankingStatement(documentId) {
        try {
            ...
            const text = await documentInsightsService.extractTextFromPDF(document.filePath || document.storageKey);
            if (!text) {
    ...
    async extractBasicBankInfo(text) {
    ...
    async extractMonthlyStats(text) {
        const monthlyStats = [];
        ...
        for (const line of lines) {
            ...
            if (balanceMatch) {
                const amounts = balanceMatch.map(amount => parseFloat(amount.replace(/[\$,\s]/g, ''))).filter(amount => !isNaN(amount));
                if (amounts.length > 0) {
                    const balance = amounts[amounts.length - 1]; // Assume last amount is balance
                    monthlyData[monthYear].minBalance = Math.min(monthlyData[monthYear].minBalance || Infinity, balance);
                    monthlyData[monthYear].maxBalance = Math.max(monthlyData[monthYear].maxBalance || -Infinity, balance);
                    if (balance < 0) {
        ...
        for (const [monthYear, data] of Object.entries(monthlyData)) {
            if (data.minBalance === Infinity)
                data.minBalance = 0;
            if (data.maxBalance === -Infinity)
                data.maxBalance = 0;
        ...
        return monthlyStats.sort((a, b) => new Date(`${a.month} ${a.year}`).getTime() - new Date(`${b.month} ${b.year}`).getTime());
    }
    // Enhance statistics using AI for more accurate parsing
    async enhanceWithAI(text, basicStats) {
        try {
...
Focus on accuracy. Extract actual NSF incidents, fees, and balance information from the statement.`;
    ...
    calculateOverallSummary(monthlyStats) {
        ...
        const averageMonthlyBalance = monthlyStats.reduce((sum, month) => sum + month.averageBalance, 0) / monthlyStats.length;
        ...
        // Generate risk flags
        const riskFlags = [];
        if (totalNSFIncidents > 0)
            riskFlags.push(`${totalNSFIncidents} NSF incidents found`);
        if (overdraftFrequency > 10)
            riskFlags.push('Frequent overdrafts detected');
        if (lowestBalance < -1000)
            riskFlags.push('Significant negative balances');
        if (cashflowTrend === 'declining')
            riskFlags.push('Declining cashflow trend');
        return {
    ...
    async generateInsights(monthlyStats, summary) {
        const insights = [];
        // Balance insights
    ...
    calculateConfidence(monthlyStats, text) {
        let confidence = 0.5; // Base confidence
        // More months = higher confidence
        if (monthlyStats.length >= 12)
            confidence += 0.3;
        else if (monthlyStats.length >= 6)
            confidence += 0.2;
        else if (monthlyStats.length >= 3)
            confidence += 0.1;
    ...
    // Save analysis to document metadata
    async saveBankingAnalysis(documentId, analysis) {
        try {
            await db
                .update(documents)
                .set({
    ...
    async batchAnalyzeBankingStatements(documentIds) {
        const analyses = [];
        const errors = [];
