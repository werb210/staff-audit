...
async function sendSystemSMS({ to, message, source }) {
    console.log(`[SMS] ${source}: ${to} - ${message.slice(0, 50)}...`);
    // TODO: Implement with Twilio service
}
async function sendSystemEmail({ to, subject, html, source }) {
...
function isBusinessHours(date) {
    ...
    if (!bizDays.includes(dow))
        return false;
    const time = d.format("HH:mm");
    return time >= bizStart && time <= bizEnd;
}
function nextBusinessHour(from) {
...
export async function scheduleReminder({ targetType, targetId, channel, templateId, delayHours = 24, vars = {} }) {
  ...
  `, [now]);
    for (const reminder of pending) {
        try {
            ...
            // Get template
            const [tpl] = await q(`SELECT * FROM comm_templates WHERE id=$1 LIMIT 1`, [reminder.template_id]);
            if (!tpl) {
            ...
            if (reminder.target_type === "application") {
                const [app] = await q(`SELECT contact_id FROM applications WHERE id=$1 LIMIT 1`, [reminder.target_id]);
                contactId = app?.contact_id;
            }
            else if (reminder.target_type === "thread") {
                const [thread] = await q(`SELECT contact_id FROM comm_threads WHERE id=$1 LIMIT 1`, [reminder.target_id]);
            ...
            if (reminder.channel === "sms") {
                const [contact] = await q(`SELECT sms_opt_out FROM contacts WHERE id=$1 LIMIT 1`, [contactId]);
