...
export class AnalyticsService {
    ...
    async getTrafficSourcesReport(startDate, endDate) {
        return await this.analytics.properties.runReport({
            property: `properties/${this.propertyId}`,
            requestBody: {
                dateRanges: [{
    ...
    extractMetric(report, metricName, defaultValue) {
        try {
            const rows = report.data?.rows;
            if (!rows || rows.length === 0)
                return defaultValue;
            const metricIndex = report.data.metricHeaders.findIndex((header) => header.name === metricName);
            if (metricIndex === -1)
                return defaultValue;
