...
import { attributionEvents } from '../../shared/marketing-schema';
...
export class AttributionService {
    ...
    async getContactJourney(contactId) {
        try {
    ...
    async getChannelPerformance(startDate, endDate) {
        try {
            const events = await db.select()
                .from(attributionEvents)
                .where(and(gte(attributionEvents.eventTime, startDate), lte(attributionEvents.eventTime, endDate)));
    ...
    async getTopConvertingPaths(startDate, endDate, limit = 10) {
        try {
            const events = await db.select()
                .from(attributionEvents)
                .where(and(gte(attributionEvents.eventTime, startDate), lte(attributionEvents.eventTime, endDate)))
            ...
            for (const [contactId, journey] of contactJourneys) {
                const hasConversion = journey.some((e) => e.eventType === 'conversion');
                if (!hasConversion)
                    continue;
                const path = journey
                    .reverse() // Start from first touch
                    .map((e) => `${e.source}/${e.medium || 'unknown'}`)
                    .join(' → ');
                ...
                const totalRevenue = journey.reduce((sum, e) => sum + parseFloat(e.revenue || '0'), 0);
    ...
    async trackLanding(req, contactId) {
        try {
            const { utm_source, utm_medium, utm_campaign, utm_content, utm_term, gclid, gbraid, fbclid, li_fat_id } = req.query;
    ...
    getSourceFromClickId(gclid, gbraid, fbclid, liClickId) {
        if (gclid || gbraid)
            return 'google';
        if (fbclid)
            return 'facebook';
    ...
    async generateReport(startDate, endDate) {
        try {
            const [channelPerformance, topPaths, totalEvents, totalRevenue] = await Promise.all([
                this.getChannelPerformance(startDate, endDate),
                this.getTopConvertingPaths(startDate, endDate),
                db.select().from(attributionEvents)
                    .where(and(gte(attributionEvents.eventTime, startDate), lte(attributionEvents.eventTime, endDate))),
                db.select().from(attributionEvents)
                    .where(and(gte(attributionEvents.eventTime, startDate), lte(attributionEvents.eventTime, endDate), eq(attributionEvents.eventType, 'conversion')))
            ]);
            const report = {
                period: {
