...
const processScheduledNotifications = cron.schedule('* * * * *', async () => {
    try {
        ...
        const tableCheckResult = await db.execute(sql `SELECT EXISTS (
        ...
        const dueNotifications = await db.select()
            ...
            .where(and(lte(scheduledNotifications.scheduledAt, new Date()), eq(scheduledNotifications.sent, false)));
...
const cleanupOldNotifications = cron.schedule('0 3 * * 0', async () => {
    try {
        ...
        const deleted = await db.delete(scheduledNotifications)
            .where(and(eq(scheduledNotifications.sent, true), lte(scheduledNotifications.sentAt, thirtyDaysAgo)));
...
export { processScheduledNotifications, createMonthlyBackup, cleanupOldNotifications };
