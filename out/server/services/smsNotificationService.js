 ...
 */
function generateSMSMessage(trigger, data) {
 ...
 * Log SMS to database
 */
async function logSMSToDatabase(logEntry) {
    try {
 ...
 */
export async function checkAllDocumentsAccepted(applicationId) {
    try {
 ...
 */
export async function checkAnalysisCompleted(applicationId) {
    try {
    ...
    `);
      ...
      WHERE applicationId = ${applicationId}
    `);
        const ocrCount = parseInt(ocrResult.rows[0].ocr_count);
        const bankingCount = parseInt(bankingResult.rows[0].banking_count);
