...
export async function getVariantWeights(variant) {
...
export async function runEngine11(applicationId, variant = "prod") {
  ...
  `)).rows || [];
    ...
    for (const r of policies) {
        (policyByScope[r.scope] ||= []).push(r.rule);
    }
    const results = [];
    const rulesApplied = [];
    ...
    for (const p of products) {
        ...
        function fail(cond, why) { if (cond) {
            eligible = false;
            reasons.push(why);
        } }
        ...
        if (p.industries_allowed && p.industries_allowed.length)
            fail(!p.industries_allowed.includes(String(a.industry || "")), "industry not allowed");
        if (p.industries_blocked && p.industries_blocked.length)
            fail(p.industries_blocked.includes(String(a.industry || "")), "industry blocked");
        // policy overrides
        const scopes = ["global", `product:${p.key}`, `application:${a.id}`];
        for (const sc of scopes) {
            const { hits, eligibleOverride } = evalPolicies(policyByScope[sc] || [], a, p);
            if (hits.length)
                rulesApplied.push({ scope: sc, product: p.key, hits });
