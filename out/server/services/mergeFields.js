...
export async function getContactMergeVars(contactId, db) {
  ...
  `, [contactId]);
    ...
    `, [application.id]);
        missingDocs = docs.filter((x) => x.status === "uploaded"); // uploaded but not accepted -> still missing
    ...
    return {
        ...
        MissingDocs: missingDocs.map((x) => ({ Id: x.id, Category: x.category, Filename: x.filename })),
        RejectedDocsCount: rejectedDocs.length,
        RejectedDocs: rejectedDocs.map((x) => ({ Id: x.id, Category: x.category, Filename: x.filename })),
        Now: new Date().toISOString(),
        ClientPortalUrl: `${process.env.CLIENT_PORTAL_BASE || 'https://portal.boreal.financial'}/login`
    };
}
