...
export async function runDocumentStartupAudit() {
    ...
    `;
        ...
        for (const doc of missingStorageDocs) {
            const filePath = path.join("uploads", "documents", `${doc.id}.${doc.name.split('.').pop() || 'txt'}`);
            if (!fs.existsSync(filePath)) {
...
export async function getSystemValidationStatus() {
    try {
        ...
        const total = totalDocs[0].count;
        const missingHashes = missingChecksums[0].count;
        const missingStorage = missingStorageKeys[0].count;
        return {
            summary: {
