...
export async function setCampaignStatus(campaignId, enabled) {
    ...
    if (!res.ok)
        throw new Error("linkedin_status_failed " + res.status);
    return true;
}
export async function setCampaignDailyBudget(campaignId, amountCents, currency = "USD") {
    const { token } = cfg();
    const url = `https://api.linkedin.com/v2/adCampaignsV2/${campaignId}`;
    const res = await fetch(url, {
        method: "PATCH",
    ...
    if (!res.ok)
        throw new Error("linkedin_budget_failed " + res.status);
    return true;
}
