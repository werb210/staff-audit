...
ops.get("/kill-sw.js", (_req, res) => {
    res.type("application/javascript").send(`self.addEventListener('install',e=>self.skipWaiting());
...
function mountSpa(app) {
    ...
    const inject = (body, baseHref) => body.replace("<head>", `<head><base href="${baseHref}"><meta name="app-base" content="${baseHref}">`)
...
app.post("/api/sync/lender-products", async (req, res) => {
    try {
...
app.listen(PORT, () => console.log(`✅ [CLEAN BOOT] Server on :${PORT}, client=${CLIENT_DIR}`));
