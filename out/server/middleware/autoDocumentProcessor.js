 ...
 */
export async function processDocument(documentId, applicationId, fileName, documentType) {
    const result = {
        documentId,
        fileName,
        ocrSuccess: false,
            ...
            if (data && data.success && data.documentId && data.applicationId) {
                ...
                setImmediate(async () => {
                    try {
                        ...
                        const result = await processDocument(data.documentId, data.applicationId, data.fileName || 'Unknown File', data.documentType || 'bank_statements');
