...
export function deletionProtectionMiddleware(req, res, next) {
 ...
 */
export function documentRouteProtection(req, res, next) {
    const isLocked = process.env.DOCUMENT_DELETION_LOCKED === 'true';
    if (isLocked) {
        ...
        const isDangerous = dangerousRoutes.some(route => req.originalUrl.toLowerCase().includes(route));
        if (isDangerous) {
