...
export async function loadStaff(req, _res, next) {
    const sid = req.cookies?.[CNAME];
    if (!sid)
        return next();
    try {
...
export function getIP(req) { return req.headers["x-forwarded-for"]?.split(",")[0]?.trim() || req.socket.remoteAddress || null; }
