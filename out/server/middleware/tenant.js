import { resolveTenantFromHeaders } from '../tenancy/tenantMap';
// Tenant detection middleware
export function detectTenant(req, res, next) {
    // Start with host-based tenant resolution (hardened)
    let tenant = resolveTenantFromHeaders(req.headers);
    // Allow tenant switching via query params (for admin/testing)
    if (req.query.tenant && ['bf', 'slf'].includes(req.query.tenant)) {
...
export function withTenant(baseQuery, tenant, unified = false) {
    if (unified)
        return baseQuery; // Skip tenant filter for unified views
...
export function enforceTenantMutation(req, res, next) {
    // STAFF APP FIX: Bypass tenant requirement for client API calls
    const authHeader = req.headers["authorization"] || "";
    const clientToken = authHeader.replace("Bearer ", "").trim();
    const clientApiTokens = [
