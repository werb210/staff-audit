...
export function mountStatic(app) {
    ...
    if (!indexExists)
        console.warn("[static] index.html missing in", clientDir);
    ...
    app.get("*", (req, res) => {
        const p = req.path;
        if (p.startsWith("/api") || p.startsWith("/_next") || p.startsWith("/static"))
            return res.status(404).end();
        if (!indexExists)
            return res.status(500).send("Client build missing index.html");
        ...
        if (!/ <base /i.test(html)) {
            html = html.replace(/<head>/i, `<head><base href="${base}/"><meta name="app-base" content="${base}">`);
        }
        res.setHeader("Cache-Control", "no-store, must-revalidate");
