...
export function shouldBypassAuth(req) {
    ...
    if (u.startsWith("/api/auth/"))
        return true;
    if (req.method === "GET" && (u === "/login" || u.startsWith("/login?")))
        return true;
    if (u === "/__version")
        return true;
    return false;
...
export function requireAuthUnless(patterns) {
    return (req, res, next) => {
...
export function htmlRequiresAuth(req, res, next) {
    const p = req.path.toLowerCase();
    if (p.startsWith("/login") || p.startsWith("/verify") || p === "/api/health")
        return next();
    if (isAuthed(req))
        return next();
