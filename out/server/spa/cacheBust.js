...
import fs from "fs";
function noStore(res) { res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate, proxy-revalidate, max-age=0"); res.setHeader("Pragma", "no-cache"); res.setHeader("Expires", "0"); res.setHeader("Surrogate-Control", "no-store"); }
export function mountCacheBustingSpa(app) {
    const DIST = path.resolve(process.cwd(), "client", "dist");
    const ASSETS = path.join(DIST, "assets");
    const BUILD_ID = String(process.env.BUILD_ID || Math.floor(Date.now() / 1000));
    const PREFIX = `/assets-${BUILD_ID}`;
