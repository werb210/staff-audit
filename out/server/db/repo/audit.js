...
export const auditRepo = {
    async log(tenantId, actor, action, details) {
        // Use existing audit_log table
        return q(`INSERT INTO audit_log (user_id, tenant_id, endpoint, method, request_data, timestamp) 
              VALUES ($1,$2,$3,$4,$5,NOW())`, [actor || null, tenantId || null, action, 'POST', JSON.stringify(details || {})]);
    },
    async recent(tenantId, limit = 200) {
        if (tenantId) {
