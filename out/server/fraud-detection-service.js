...
Focus on detecting document manipulation, template usage, or suspicious patterns.`;
    ...
    async analyzeApplication(applicationId) {
      ...
      `, { applicationId: applicationId });
            ...
            // Determine risk level
            let riskLevel = 'Low';
            if (overallFraudScore > 85)
                riskLevel = 'Critical';
            else if (overallFraudScore > 70)
                riskLevel = 'High';
            else if (overallFraudScore > 50)
                riskLevel = 'Medium';
            const result = {
                applicationId,
                overallFraudScore,
     ...
     */
    async findSimilarDocuments(targetDocumentId, documentFingerprint) {
        try {
     ...
     */
    async calculateSimilarityScore(fingerprint1, content2) {
        try {
     ...
     */
    async saveSimilarityResults(documentId, similarities, analysis) {
        try {
            // Validate documentId
            const validDocumentId = Math.floor(Number(documentId));
            if (isNaN(validDocumentId) || validDocumentId <= 0) {
     ...
     */
    async saveApplicationFraudResult(result) {
        try {
            await db.execute(sql `
