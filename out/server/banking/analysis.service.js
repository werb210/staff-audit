...
function classifyFrequency(dates) {
    ...
    if (Math.abs(avg - 7) <= 2)
        return 'Weekly';
    if (Math.abs(avg - 14) <= 3)
        return 'Biweekly';
    if (Math.abs(avg - 15) <= 3)
        return 'Semimonthly';
    if (Math.abs(avg - 30) <= 4)
        return 'Monthly';
    if (Math.abs(avg - 90) <= 7)
        return 'Quarterly';
    if (Math.abs(avg - 365) <= 14)
        return 'Annual';
    return 'Irregular';
}
function categorizeRecurringName(name) {
    const s = name.toLowerCase();
    if (/(loan|lender|capital|bank|interest|mortgage|payment)/.test(s))
        return 'Loan Payment';
    if (/(lease)/.test(s))
        return 'Lease';
    if (/(adobe|microsoft|google|aws|shopify|netflix|subscription|saas)/.test(s))
        return 'Subscription';
    if (/(hydro|water|electric|power|utility|gas|telus|rogers|bell|internet)/.test(s))
        return 'Utilities';
    if (/(insurance|insur)/.test(s))
        return 'Insurance';
    return 'Other Recurring';
}
function trendFromSeries(byMonth) {
    if (byMonth.length < 2)
        return 'flat';
...
function monthlyAndAnnualTotals(amount, freq) {
...
// ---- Core service ----
export async function buildBankingAnalysis(applicationId, periodStart, periodEnd) {
