...
export async function processDocumentOCR(documentId, applicationId, filePath, documentType) {
        ...
        if (documentType === 'bank_statements') {
            try {
                ...
                if (application && application.legal_business_name?.toLowerCase().includes('test')) {
                    ...
                    const ocrText = extractedData.text || JSON.stringify(extractedData);
...
async function processImageWithVision(filePath, documentType) {
    try {
...
async function processTextDocument(filePath, documentType) {
    try {
...
async function processPDFDocument(filePath, documentType) {
...
function createExtractionPrompt(documentType) {
...
export async function getOcrResultsByDocument(documentId) {
    const { pool } = await import('./db');
    const client = await pool.connect();
...
export async function getOcrResultsByApplication(applicationId) {
    const { pool } = await import('./db');
    const client = await pool.connect();
...
export async function processS3DocumentOCR(documentId, applicationId, storageKey, documentType) {
    const startTime = Date.now();
    try {
