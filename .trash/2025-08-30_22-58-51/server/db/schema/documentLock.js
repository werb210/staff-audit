...
exports.validateEnumModification = validateEnumModification;
exports.displayStartupEnumLockWarning = displayStartupEnumLockWarning;
// Check if enum modifications are allowed
if (process.env.ALLOW_ENUM_EDITS !== "true") {
    console.warn("[LOCKED] document_type enum is frozen. Set ALLOW_ENUM_EDITS=true to modify.");
 ...
 */
function validateEnumModification(enumName = 'document_type') {
    if (process.env.ALLOW_ENUM_EDITS !== "true") {
 ...
 */
function displayStartupEnumLockWarning() {
    if (process.env.ALLOW_ENUM_EDITS !== "true") {
