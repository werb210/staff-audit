...
class FallbackRecoveryService {
  ...
  async retryFallbackUpload(document: FallbackDocument): Promise<{
    success: boolean;
    error?: string;
...
async function retryFallbackUploads(documentId?: string) {
  ...
  try {
    ...
    let successCount = 0;
    let failureCount = 0;
    const results: Array<{ doc: any; success: boolean; error?: string }> = [];

    // Process each document
    for (const doc of fallbackDocs.rows) {
      const result = await recovery.retryFallbackUpload(doc as any);
