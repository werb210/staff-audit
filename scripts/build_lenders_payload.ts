...
type ProductRow = {
  ...
  interestRateMaximum: number | null;
  rateType: string | null;
  rateFrequency: string | null;
  termMinimum: number | null;
  termMaximum: number | null;
  minimumAverageMonthlyRevenue: number | null;
...
function toSnakeProduct(
  ...
  const freq =
    p.rateFrequency?.toLowerCase().startsWith("ann")
      ? "Annual"
...
export async function buildLendersPayload() {
  ...
  const lendersMap = new Map<string, any>();
  allLenders.rows.forEach((l) => lendersMap.set(l.id, l));

  const grouped = new Map<
    string,
    { name: string; products: ReturnType<typeof toSnakeProduct>[] }
  >();
